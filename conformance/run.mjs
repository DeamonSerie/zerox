...
if (
  process.env.ZERO_NATIVE_TEST_SANDBOX !== "1" &&
  process.env.ZERO_NATIVE_TEST_ALLOW_LOCAL !== "1"
) {
  console.error(
    "conformance emits native test artifacts; run `pnpm run conformance` for Vercel Sandbox execution or set ZERO_NATIVE_TEST_ALLOW_LOCAL=1 to opt into local artifacts.",
  );
  process.exit(1);
}
...
async function assertCheckTimeoutOrDiagnostic(fixture, expectedCodes) {
  const result = await execFileAsync(zero, ["check", "--json", fixture], {
  ...
  assert(
    ...
    `expected one of ${expectedCodes.join(", ")}, got ${code}`,
  );
...
async function assertElf64Object(path, exportedName) {
  ...
  assert(symtab);
  const strtab = [...sections.values()].find(
    (section) => section.index === symtab.link,
  );
  assert(strtab);
  let sawExport = false;
  for (
    let offset = symtab.offset;
    offset < symtab.offset + symtab.size;
...
async function assertElfAarch64Object(path, exportedName) {
  ...
  assert(
    bytes.includes(
      Buffer.from([0x40, 0x05, 0x80, 0x52, 0xc0, 0x03, 0x5f, 0xd6]),
    ),
  );
  assert(
    bytes.includes(
      Buffer.concat([Buffer.from(exportedName), Buffer.from([0])]),
    ),
  );
}
...
async function assertElf64Executable(path) {
  ...
  assert.equal(
    Number(bytes.readBigUInt64LE(phoff + 32)),
    Number(bytes.readBigUInt64LE(phoff + 40)),
  );
}
...
async function assertElfAarch64Executable(path) {
  ...
  assert(
    bytes.includes(
      Buffer.from([0xa8, 0x0b, 0x80, 0xd2, 0x01, 0x00, 0x00, 0xd4]),
    ),
  );
}
...
async function assertVoidArm64Object(path, exportedName) {
  ...
  assert(
    bytes.includes(
      Buffer.concat([Buffer.from(`_${exportedName}`), Buffer.from([0])]),
    ),
  );
  return bytes;
}
...
async function assertVoidArm64Executable(path) {
  ...
  for (let offset = 32, i = 0; i < ncmds; i++) {
    ...
    if (cmd === 0x1b) {
      ...
      assert(
        !bytes.subarray(offset + 8, offset + 24).every((byte) => byte === 0),
      );
      sawUuid = true;
    }
    offset += cmdsize;
...
for (const fixture of [
  ...
  "conformance/check/pass/control-flow-if.0",
  "conformance/check/pass/arithmetic-basic.0",
  "conformance/check/pass/logic-operators.0",
  "conformance/check/pass/not-operator.0",
  "conformance/check/pass/nested-combined.0",
...
const checkJsonSuccess = await execFileAsync(zero, [
  "check",
  "--json",
  "conformance/native/pass/explicit-casts.0",
]);
...
assert(
  checkJsonSuccessBody.compilerPhases.some(
    (item) => item.name === "parse" && item.cacheable === true,
  ),
);
assert(
  checkJsonSuccessBody.compilerPhases.some(
    (item) => item.name === "check" && item.cacheable === true,
  ),
);
assert(
  checkJsonSuccessBody.compilerCaches.some(
    (item) =>
      item.name === "parseTree" &&
      item.stored === true &&
      item.invalidatesOn === "source",
  ),
);
assert(
  checkJsonSuccessBody.compilerCaches.some(
    (item) =>
      item.name === "interface" &&
      item.invalidatesOn.includes("public symbols"),
  ),
);
assert.ok(checkJsonSuccessBody.incrementalInvalidation.targetDependency);
assert.equal(
  checkJsonSuccessBody.incrementalInvalidation.profileDependency,
  "release",
);
assert.equal(
  checkJsonSuccessBody.incrementalInvalidation.recheckStrategy,
  "fingerprint changed modules and dependent bodies",
);

const agentSurfaceClassification = JSON.parse(
  await readFile("conformance/agent-surface/classification.json", "utf8"),
);
assert.equal(agentSurfaceClassification.schema, 1);
assert.deepEqual(
  agentSurfaceClassification.fixtures.map((item) => item.id),
  [
...
async function assertAgentSurfaceGenericShadowing(path, actualPattern) {
  const result = await execFileAsync(zero, ["check", "--json", path]).catch(
    (error) => error,
  );
  assert.notEqual(result.code, 0);
  const body = JSON.parse(result.stdout);
...
await assertAgentSurfaceGenericShadowing(
  "conformance/agent-surface/fixtures/generic-type-shadowing.0",
  /already names a shape/,
);
await assertAgentSurfaceGenericShadowing(
  "conformance/agent-surface/fixtures/builtin-generic-type-shadowing.0",
  /already names a built-in type/,
);
await assertAgentSurfaceGenericShadowing(
  "conformance/agent-surface/fixtures/shape-generic-type-shadowing.0",
  /already names a shape/,
);
await assertAgentSurfaceGenericShadowing(
  "conformance/agent-surface/fixtures/shape-generic-self-shadowing.0",
  /reserved for method Self types/,
);
await assertAgentSurfaceGenericShadowing(
  "conformance/agent-surface/fixtures/method-generic-type-shadowing.0",
  /already names a shape/,
);
await assertAgentSurfaceGenericShadowing(
  "conformance/agent-surface/fixtures/method-generic-outer-shadowing.0",
  /outer generic scope/,
);
await assertAgentSurfaceGenericShadowing(
  "conformance/agent-surface/fixtures/method-generic-self-shadowing.0",
  /reserved for method Self types/,
);

const agentSurfaceInterfaceMethodGeneric = await execFileAsync(zero, [
  "check",
  "--json",
  "conformance/agent-surface/fixtures/interface-method-generic-binding.0",
]);
const agentSurfaceInterfaceMethodGenericBody = JSON.parse(
  agentSurfaceInterfaceMethodGeneric.stdout,
);
...
const agentSurfaceDirectGenericCheck = await execFileAsync(zero, [
  "check",
  "--json",
  "conformance/agent-surface/fixtures/direct-generic-recursion.0",
]);
const agentSurfaceDirectGenericCheckBody = JSON.parse(
  agentSurfaceDirectGenericCheck.stdout,
);
...
const agentSurfaceDirectGenericReadinessBody = JSON.parse(
  agentSurfaceDirectGenericReadiness.stdout,
);
...
assert.equal(
  agentSurfaceDirectGenericReadinessBody.targetReadiness.buildable,
  true,
);
assert.equal(
  agentSurfaceDirectGenericReadinessBody.targetReadiness.diagnostics.length,
  0,
);
...
const agentSurfaceDirectGenericCollisionReadinessBody = JSON.parse(
  agentSurfaceDirectGenericCollisionReadiness.stdout,
);
assert.equal(agentSurfaceDirectGenericCollisionReadinessBody.ok, true);
assert.equal(
  agentSurfaceDirectGenericCollisionReadinessBody.targetReadiness.ok,
  false,
);
assert.equal(
  agentSurfaceDirectGenericCollisionReadinessBody.targetReadiness.buildable,
  false,
);
assert.equal(
  agentSurfaceDirectGenericCollisionReadinessBody.targetReadiness.diagnostics[0]
    .code,
  "BLD004",
);
assert.match(
  agentSurfaceDirectGenericCollisionReadinessBody.targetReadiness.diagnostics[0]
    .message,
  /specialization name collides/,
);

const agentSurfacePolymorphicRecursion = await execFileAsync(zero, [
  "check",
  "--json",
  "conformance/agent-surface/fixtures/polymorphic-recursion-growth.0",
]).catch((error) => error);
assert.notEqual(agentSurfacePolymorphicRecursion.code, 0);
const agentSurfacePolymorphicRecursionBody = JSON.parse(
  agentSurfacePolymorphicRecursion.stdout,
);
assert.equal(
  agentSurfacePolymorphicRecursionBody.diagnostics[0].code,
  "TYP027",
);
assert.match(
  agentSurfacePolymorphicRecursionBody.diagnostics[0].message,
  /recursive generic call/,
);

const agentSurfaceMutualPolymorphicRecursion = await execFileAsync(zero, [
  "check",
  "--json",
  "conformance/agent-surface/fixtures/mutual-polymorphic-recursion-growth.0",
]).catch((error) => error);
assert.notEqual(agentSurfaceMutualPolymorphicRecursion.code, 0);
const agentSurfaceMutualPolymorphicRecursionBody = JSON.parse(
  agentSurfaceMutualPolymorphicRecursion.stdout,
);
assert.equal(
  agentSurfaceMutualPolymorphicRecursionBody.diagnostics[0].code,
  "TYP027",
);
assert.match(
  agentSurfaceMutualPolymorphicRecursionBody.diagnostics[0].message,
  /recursive generic call/,
);

const agentSurfaceMutualInferredPolymorphicRecursion = await execFileAsync(
  zero,
  [
    "check",
    "--json",
    "conformance/agent-surface/fixtures/mutual-polymorphic-recursion-inferred-growth.0",
  ],
).catch((error) => error);
assert.notEqual(agentSurfaceMutualInferredPolymorphicRecursion.code, 0);
const agentSurfaceMutualInferredPolymorphicRecursionBody = JSON.parse(
  agentSurfaceMutualInferredPolymorphicRecursion.stdout,
);
assert.equal(
  agentSurfaceMutualInferredPolymorphicRecursionBody.diagnostics[0].code,
  "TYP027",
);
assert.match(
  agentSurfaceMutualInferredPolymorphicRecursionBody.diagnostics[0].message,
  /recursive generic call/,
);

const compilerMetrics = await execFileAsync("node", [
...
assert.equal(
  compilerMetricsBody.stdlib.mainHelperCount,
  compilerMetricsBody.stdlib.checkerReturnCount,
);
assert.equal(
  compilerMetricsBody.stdlib.mainHelperCount,
  compilerMetricsBody.stdlib.checkerArgCountCount,
);
...
assert.deepEqual(
  compilerMetricsBody.stdlib.checkerReturnsMissingFromMainHelpers,
  [],
);
assert.deepEqual(
  compilerMetricsBody.stdlib.mainHelpersMissingFromCheckerReturns,
  [],
);
assert.deepEqual(
  compilerMetricsBody.stdlib.checkerArgCountsMissingFromMainHelpers,
  [],
);
assert.deepEqual(
  compilerMetricsBody.stdlib.mainHelpersMissingFromCheckerArgCounts,
  [],
);
assert.deepEqual(compilerMetricsBody.stdlib.argCountMismatches, []);
assert.equal(
  compilerMetricsBody.stdlib.sharedSignatureLookup.checkerReturnTypes,
  true,
);
assert.equal(
  compilerMetricsBody.stdlib.sharedSignatureLookup.checkerArgCounts,
  true,
);
assert.equal(compilerMetricsBody.backendFormats.elf.sharedWriter, true);
assert.equal(
  compilerMetricsBody.backendFormats.elf.x86ObjectUsesSharedWriter,
  true,
);
assert.equal(
  compilerMetricsBody.backendFormats.elf.x86ExecutableUsesSharedWriter,
  true,
);
assert.equal(
  compilerMetricsBody.backendFormats.elf.aarch64ObjectUsesSharedWriter,
  true,
);
assert.equal(
  compilerMetricsBody.backendFormats.elf.aarch64ExecutableUsesSharedWriter,
  true,
);
assert.deepEqual(
  compilerMetricsBody.backendFormats.elf.archFilesWithLocalSectionWriters,
  [],
);
assert.equal(compilerMetricsBody.backendFormats.elf.patchStateModule, true);
assert.equal(
  compilerMetricsBody.backendFormats.elf.x86UsesPatchStateModule,
  true,
);
assert.deepEqual(
  compilerMetricsBody.backendFormats.elf.archFilesWithLocalPatchState,
  [],
);
assert.equal(compilerMetricsBody.backendFormats.coff.sharedWriter, true);
assert.equal(
  compilerMetricsBody.backendFormats.coff.objectUsesSharedWriter,
  true,
);
assert.equal(
  compilerMetricsBody.backendFormats.coff.executableUsesSharedWriter,
  true,
);
assert.deepEqual(
  compilerMetricsBody.backendFormats.coff.archFilesWithLocalContainerWriters,
  [],
);
assert.equal(compilerMetricsBody.backendFormats.coff.patchStateModule, true);
assert.equal(
  compilerMetricsBody.backendFormats.coff.x64UsesPatchStateModule,
  true,
);
assert.deepEqual(
...
assert.equal(
  compilerMetricsBody.backendFormats.x64.sharedEncodingPrimitives,
  true,
);
assert.equal(
  compilerMetricsBody.backendFormats.x64.elfUsesSharedEncodingPrimitives,
  true,
);
assert.equal(
  compilerMetricsBody.backendFormats.x64.coffUsesSharedEncodingPrimitives,
  true,
);
assert.deepEqual(
  compilerMetricsBody.backendFormats.x64.formatFilesWithLocalEncodingPrimitives,
  [],
);
assert.equal(
  compilerMetricsBody.backendFormats.aarch64.sharedEncodingPrimitives,
  true,
);
assert.equal(
  compilerMetricsBody.backendFormats.aarch64.elfUsesSharedEncodingPrimitives,
  true,
);
assert.equal(
...
assert.deepEqual(
  compilerMetricsBody.backendFormats.aarch64
...
const agentSurfaceBorrowLifetime = await execFileAsync(zero, [
  "check",
  "--json",
  "conformance/agent-surface/fixtures/borrow-lexical-lifetime.0",
]).catch((error) => error);
assert.notEqual(agentSurfaceBorrowLifetime.code, 0);
const agentSurfaceBorrowLifetimeBody = JSON.parse(
  agentSurfaceBorrowLifetime.stdout,
);
assert.equal(agentSurfaceBorrowLifetimeBody.diagnostics[0].code, "BOR001");
assert.match(
  agentSurfaceBorrowLifetimeBody.diagnostics[0].actual,
  /data already has shared borrow/,
);
assert.equal(
  agentSurfaceBorrowLifetimeBody.diagnostics[0].repair.id,
  "end-conflicting-borrow",
);
assert.equal(
  agentSurfaceBorrowLifetimeBody.diagnostics[0].borrowTrace.rule,
  "lexical",
);
assert.equal(
  agentSurfaceBorrowLifetimeBody.diagnostics[0].borrowTrace.activeBorrows[0]
    .root,
  "data",
);
assert.equal(
...
assert.match(
  agentSurfaceBorrowLifetimeBody.diagnostics[0].borrowTrace.repair,
  /inner block|lexical scope/,
);

const agentSurfaceBorrowMultiple = await execFileAsync(zero, [
  "check",
  "--json",
  "conformance/agent-surface/fixtures/borrow-multiple-active-borrows.0",
]).catch((error) => error);
assert.notEqual(agentSurfaceBorrowMultiple.code, 0);
const agentSurfaceBorrowMultipleBody = JSON.parse(
  agentSurfaceBorrowMultiple.stdout,
);
assert.equal(agentSurfaceBorrowMultipleBody.diagnostics[0].code, "BOR001");
assert.equal(
  agentSurfaceBorrowMultipleBody.diagnostics[0].borrowTrace.activeBorrows
    .length,
  2,
);
assert.deepEqual(
  agentSurfaceBorrowMultipleBody.diagnostics[0].borrowTrace.activeBorrows.map(
    (borrow) => borrow.binding,
  ),
  ["first", "second"],
);
assert.deepEqual(
  agentSurfaceBorrowMultipleBody.diagnostics[0].borrowTrace.activeBorrows.map(
    (borrow) => borrow.bindingDecl.line,
  ),
  [3, 4],
);
assert.equal(
  agentSurfaceBorrowMultipleBody.diagnostics[0].borrowTrace.truncated,
  false,
);

const agentSurfaceBorrowExplain = await execFileAsync(zero, [
  "explain",
  "--json",
  "BOR001",
]);
const agentSurfaceBorrowExplainBody = JSON.parse(
  agentSurfaceBorrowExplain.stdout,
);
...
const agentSurfaceUnresolvedImport = await execFileAsync(zero, [
  "check",
  "--json",
  "conformance/agent-surface/fixtures/unresolved-package-import",
]).catch((error) => error);
assert.notEqual(agentSurfaceUnresolvedImport.code, 0);
const agentSurfaceUnresolvedImportBody = JSON.parse(
  agentSurfaceUnresolvedImport.stdout,
);
assert.equal(agentSurfaceUnresolvedImportBody.diagnostics[0].code, "IMP001");
assert.match(
  agentSurfaceUnresolvedImportBody.diagnostics[0].expected,
  /missing\.utility\.0 or missing\.utility\/mod\.0/,
);

const agentSurfaceMalformedUse = await execFileAsync(zero, [
  "check",
  "--json",
  "conformance/agent-surface/fixtures/malformed-use-current.0",
]).catch((error) => error);
assert.notEqual(agentSurfaceMalformedUse.code, 0);
const agentSurfaceMalformedUseBody = JSON.parse(
  agentSurfaceMalformedUse.stdout,
);
assert.equal(agentSurfaceMalformedUseBody.diagnostics[0].code, "PAR100");
assert.match(
  agentSurfaceMalformedUseBody.diagnostics[0].message,
  /expected import module segment/,
);
...
await writeFile(
  agentSurfaceMalformedLocalUseFixture,
  'use local.\n\npub fn main Void world World !\n  check world.out.write "malformed local use parser fixture\\n"\n',
);
const agentSurfaceMalformedLocalUse = await execFileAsync(zero, [
  "check",
  "--json",
  agentSurfaceMalformedLocalUseFixture,
]).catch((error) => error);
assert.notEqual(agentSurfaceMalformedLocalUse.code, 0);
const agentSurfaceMalformedLocalUseBody = JSON.parse(
  agentSurfaceMalformedLocalUse.stdout,
);
assert.equal(agentSurfaceMalformedLocalUseBody.diagnostics[0].code, "PAR100");
assert.match(
  agentSurfaceMalformedLocalUseBody.diagnostics[0].message,
  /expected import module segment/,
);
...
await writeFile(
  agentSurfaceSplitUseFixture,
  'use std.\ncodec\n\npub fn main Void world World !\n  check world.out.write "split use parser fixture\\n"\n',
);
const agentSurfaceSplitUse = await execFileAsync(zero, [
  "check",
  "--json",
  agentSurfaceSplitUseFixture,
]).catch((error) => error);
...
assert.match(
  agentSurfaceSplitUseBody.diagnostics[0].message,
  /expected import module segment/,
);
...
await writeFile(
  agentSurfaceKeywordUseFixture,
  'use pub\n\npub fn main Void world World !\n  check world.out.write "keyword use parser fixture\\n"\n',
);
const agentSurfaceKeywordUse = await execFileAsync(zero, [
  "check",
  "--json",
  agentSurfaceKeywordUseFixture,
]).catch((error) => error);
...
assert.match(
  agentSurfaceKeywordUseBody.diagnostics[0].message,
...
const agentSurfaceOwnedDropCheck = await execFileAsync(zero, [
  "check",
  "--json",
  "conformance/agent-surface/fixtures/owned-drop-direct-backend-unsupported.0",
]);
const agentSurfaceOwnedDropCheckBody = JSON.parse(
  agentSurfaceOwnedDropCheck.stdout,
);
...
const agentSurfaceOwnedDropReadinessBody = JSON.parse(
  agentSurfaceOwnedDropReadiness.stdout,
);
...
assert.equal(
  agentSurfaceOwnedDropReadinessBody.targetReadiness.buildable,
  false,
);
assert.equal(
  agentSurfaceOwnedDropReadinessBody.targetReadiness.languageOk,
  true,
);
assert.equal(
  agentSurfaceOwnedDropReadinessBody.targetReadiness.diagnostics[0].code,
  "BLD004",
);
assert.deepEqual(
  agentSurfaceOwnedDropReadinessBody.targetReadiness.diagnostics[0]
    .backendBlocker,
  {
...
assert.equal(
  directCallExeReadinessBody.targetReadiness.diagnostics[0].code,
  "BLD004",
);
assert.equal(
  directCallExeReadinessBody.targetReadiness.diagnostics[0].backendBlocker
    .stage,
  "buildability",
);
assert.match(
  directCallExeReadinessBody.targetReadiness.diagnostics[0].message,
...
const directCallReadinessDiag =
  directCallExeReadinessBody.targetReadiness.diagnostics[0];
...
assert.equal(
  directCallExeGraphBody.targetReadiness.diagnostics[0].code,
  "BLD004",
);
for (const key of [
  "code",
  ...
  assert.equal(
    directCallExeGraphBody.targetReadiness.diagnostics[0][key],
    directCallReadinessDiag[key],
  );
}
assert.equal(
  directCallExeGraphBody.targetReadiness.diagnostics[0].backendBlocker.stage,
  "buildability",
);
...
const coffDynamicSliceReadinessBody = JSON.parse(
  coffDynamicSliceReadiness.stdout,
);
...
assert.equal(
  coffDynamicSliceReadinessBody.targetReadiness.diagnostics[0].code,
  "BLD004",
);
assert.equal(
...
assert.match(
  coffDynamicSliceReadinessBody.targetReadiness.diagnostics[0].message,
  /constant start/,
);
...
const coffDynamicSliceReadinessDiag =
  coffDynamicSliceReadinessBody.targetReadiness.diagnostics[0];
...
for (const key of [
  ...
  assert.equal(
    coffDynamicSliceBuildDiag[key],
    coffDynamicSliceReadinessDiag[key],
  );
}
...
async function assertVoidObjectBuildabilityBlocked(
  ...
  assert.match(
    readinessBody.targetReadiness.diagnostics[0].message,
    expectedMessage,
  );
  const build = await execFileAsync(zero, [
    "build",
...
const directCallArm64ObjReadinessBody = JSON.parse(
  directCallArm64ObjReadiness.stdout,
);
...
assert.equal(
  directCallArm64ObjReadinessBody.targetReadiness.diagnostics[0].code,
  "BLD004",
);
assert.equal(
  directCallArm64ObjReadinessBody.targetReadiness.diagnostics[0].backendBlocker
    .backend,
  "zero-elf-aarch64",
);
assert.equal(
...
assert.match(
  directCallArm64ObjReadinessBody.targetReadiness.diagnostics[0].message,
  /without parameters|small integer literal/,
);
...
const directCallArm64ObjReadinessDiag =
  directCallArm64ObjReadinessBody.targetReadiness.diagnostics[0];
...
for (const key of [
  ...
  assert.equal(
    directCallArm64ObjBuildDiag[key],
    directCallArm64ObjReadinessDiag[key],
  );
}
assert.equal(
  directCallArm64ObjBuildDiag.backendBlocker.backend,
  "zero-elf-aarch64",
);
...
async function assertAgentSurfaceOwnedDropUnsupported(
  target,
  emit,
  outName,
  expectedPattern,
...
await assertAgentSurfaceOwnedDropUnsupported(
  "linux-musl-x64",
  "obj",
  "agent-surface-owned-drop-elf.o",
  /ELF64/,
  "elf",
  "zero-elf64",
);
...
await assertAgentSurfaceOwnedDropUnsupported(
  "win32-x64.exe",
  "obj",
  "agent-surface-owned-drop-coff.obj",
  /COFF/,
  "coff",
  "zero-coff-x64",
);
...
const commonPassFixtures = [
  [
    "conformance/common/pass/array-sum-min-max.0",
    "common-array-sum-min-max",
    { stdout: "array sum min max ok\n" },
  ],
  [
    "conformance/common/pass/bytes-reverse.0",
    "common-bytes-reverse",
    { stdout: "bytes reverse ok\n" },
  ],
  [
    "conformance/common/pass/cli-args.0",
    "common-cli-args",
    { stdout: "alpha\n", args: ["alpha", "beta"] },
  ],
  [
    "conformance/common/pass/count-words-lines.0",
    "common-count-words-lines",
    { stdout: "count words lines ok\n" },
  ],
  [
    "conformance/common/pass/factorial.0",
    "common-factorial",
    { stdout: "factorial ok\n" },
  ],
  [
    "conformance/common/pass/fib-iterative.0",
    "common-fib-iterative",
    { stdout: "fib iterative ok\n" },
  ],
  [
    "conformance/common/pass/fib-recursive.0",
    "common-fib-recursive",
    { stdout: "fib recursive ok\n" },
  ],
  [
    "conformance/common/pass/file-copy.0",
    "common-file-copy",
    {
      stdout: "file copy ok\n",
  ...
  [
    "conformance/common/pass/json-roundtrip.0",
    "common-json-roundtrip",
    { stdout: "json roundtrip ok\n" },
  ],
  [
    "conformance/common/pass/palindrome.0",
    "common-palindrome",
    { stdout: "palindrome ok\n" },
  ],
  ["conformance/common/pass/prime.0", "common-prime", { stdout: "prime ok\n" }],
  [
    "conformance/common/pass/sieve-small.0",
    "common-sieve-small",
    { stdout: "sieve small ok\n" },
  ],
  [
    "conformance/common/pass/sort-small.0",
    "common-sort-small",
    { stdout: "sort small ok\n" },
  ],
  [
    "conformance/common/pass/string-search.0",
    "common-string-search",
    { stdout: "string search ok\n" },
  ],
  [
    "conformance/common/pass/word-reverse.0",
    "common-word-reverse",
    { stdout: "word reverse ok\n" },
  ],
];
...
for (const [fixture, code] of [
  ...
  const result = await execFileAsync(zero, ["check", "--json", fixture]).catch(
    (error) => error,
  );
  assert.notEqual(result.code, 0);
...
const commonUnsupportedTarget = await execFileAsync(zero, [
  "check",
  "--json",
  "--target",
  "linux-musl-x64",
  "conformance/common/fail/unsupported-target-feature.0",
]).catch((error) => error);
assert.notEqual(commonUnsupportedTarget.code, 0);
assert.equal(
  JSON.parse(commonUnsupportedTarget.stdout).diagnostics[0].code,
  "TAR002",
);

const compileTimeJson = await execFileAsync(zero, [
  "check",
  "--json",
  "conformance/native/pass/compile-time-v1.0",
]);
...
assert.equal(
  compileTimeBody.compileTime.cacheKeyInputs.algorithm,
  "fnv1a64-zero-meta-v1",
);
assert.ok(compileTimeBody.compileTime.cacheKeyInputs.sourceHash);
assert.ok(
  compileTimeBody.compileTime.meta.supportedFacts.includes("target.abi"),
);
assert.ok(
  compileTimeBody.compileTime.meta.supportedFacts.includes("fieldType"),
);
assert.ok(
  compileTimeBody.compileTime.meta.supportedFacts.includes("hasEnumCase"),
);
...
const compileTimeGraph = await execFileAsync(zero, [
  "graph",
  "--json",
  "conformance/native/pass/compile-time-v1.0",
]);
...
const readGateGraph = compileTimeGraphBody.functions.find(
  (item) => item.name === "readGate",
);
assert.ok(
  readGateGraph.staticParams.some(
    (item) => item.name === "enabledFlag" && item.kind === "bool",
  ),
);
assert.ok(
  readGateGraph.staticParams.some(
    (item) => item.name === "selectedMode" && item.kind === "enum",
  ),
);
const gateGraph = compileTimeGraphBody.shapes.find(
  (item) => item.name === "Gate",
);
assert.ok(
  gateGraph.staticParams.some(
    (item) => item.name === "enabledFlag" && item.kind === "bool",
  ),
);
assert.ok(
...
const buildJsonM6 = await execFileAsync(zero, [
  "build",
  "--json",
  "--emit",
  "exe",
...
assert.equal(
  buildJsonM6Body.profileBudget.helperBudgetPolicy,
  "pay-as-used-minimum-runtime",
);
assert(
  buildJsonM6Body.profileCatalog.some(
    (item) => item.canonical === "debug" && item.debugInfo === true,
  ),
);
assert(
  buildJsonM6Body.profileCatalog.some(
    (item) =>
      item.canonical === "release-fast" &&
      item.boundsPolicy.includes("optimizer"),
  ),
);
assert(
  buildJsonM6Body.profileCatalog.some(
    (item) =>
      item.canonical === "audit" && item.runtimeMetadataPolicy === "maximum",
  ),
);
assert.equal(
  buildJsonM6Body.objectBackend.internalIr.callRepresentation,
  "same-object direct calls for supported direct subsets",
);
assert.equal(
  buildJsonM6Body.objectBackend.objectEmission.path,
  "direct-elf64-exe",
);
...
for (const [requestedProfile, canonicalProfile, profileKey] of [
  ...
  const profileBuild = await execFileAsync(zero, [
    "build",
    "--json",
    "--profile",
    requestedProfile,
    ...
    "examples/hello.0",
    "--out",
    `${outDir}/m25-${requestedProfile}-hello`,
  ]);
...
const profileSize = await execFileAsync(zero, [
  "size",
  "--json",
  "--profile",
  "debug",
...
assert(
  profileSizeBody.sizeBreakdown.functions.some(
    (item) => item.name === "main" && item.retainedBy === "entry point",
  ),
);
assert(
  profileSizeBody.sizeBreakdown.sections.some(
    (item) =>
      item.name === "text" && item.retainedBy.includes("retained functions"),
  ),
);
...
assert(
  profileSizeBody.retentionReasons.some((item) => item.kind === "function"),
);
assert(
  profileSizeBody.optimizationHints.some(
    (item) => item.id === "profile-debug-metadata",
  ),
);
assert.equal(profileSizeBody.profileBudget.debugMetadataAllowed, true);

const directObjOut = `${outDir}/direct-obj-add.o`;
const directObjJson = await execFileAsync(zero, [
  "build",
  "--json",
  "--emit",
  "obj",
...
assert.equal(
  directObjBody.objectBackend.objectEmission.path,
  "direct-elf64-object",
);
await assertElf64Object(directObjOut, "main");

const directI64ObjOut = `${outDir}/direct-i64-return.o`;
const directI64ObjJson = await execFileAsync(zero, [
  "build",
  "--json",
  "--emit",
  "obj",
...
assert.equal(
  directI64ObjBody.objectBackend.objectEmission.path,
  "direct-elf64-object",
);
assert(
  directI64ObjBytes.includes(
    Buffer.from([0x48, 0xb8, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0x7f]),
  ),
);
...
const checkJsonFailure = await execFileAsync(zero, [
  "check",
  "--json",
  "conformance/native/fail/cast-bool-to-int.0",
]).catch((error) => error);
...
assert.equal(
  checkJsonFailureBody.diagnostics[0].fixSafety,
...
for (const [fixture, message] of [
  [
    ...
    /expected '\)' after expression group/,
  ],
  ...
  const parseFailure = await execFileAsync(zero, [
    "check",
    "--json",
    fixture,
  ]).catch((error) => error);
  ...
  assert.equal(
    parseFailureBody.diagnostics[0].fixSafety,
    "requires-human-review",
  );
  assert.equal(parseFailureBody.diagnostics[0].repair.id, "repair-syntax");
}

const missingImportJson = await execFileAsync(zero, [
  "check",
  "--json",
  "conformance/check/fail/missing-import",
]).catch((error) => error);
...
assert.match(
  missingImportBody.diagnostics[0].message,
  /unknown package-local import/,
);
assert.equal(
  missingImportBody.diagnostics[0].path,
  "conformance/check/fail/missing-import/src/main.0",
);
...
assert.equal(
  missingImportBody.diagnostics[0].fixSafety,
  "requires-human-review",
);

const missingImportFixPlan = await execFileAsync(zero, [
  "fix",
  "--plan",
  "--json",
  "conformance/check/fail/missing-import",
...
assert.equal(
  missingImportFixPlanBody.diagnostics[0].path,
  "conformance/check/fail/missing-import/src/main.0",
);
...
const importLineMapJson = await execFileAsync(zero, [
  "check",
  "--json",
  "conformance/check/fail/import-line-map",
]).catch((error) => error);
...
assert.equal(
  importLineMapBody.diagnostics[0].path,
  "conformance/check/fail/import-line-map/src/main.0",
);
assert.equal(importLineMapBody.diagnostics[0].line, 4);
assert.equal(importLineMapBody.diagnostics[0].column, 26);

const importedLineMapJson = await execFileAsync(zero, [
  "check",
  "--json",
  "conformance/check/fail/imported-line-map",
]).catch((error) => error);
...
assert.equal(
  importedLineMapBody.diagnostics[0].path,
  "conformance/check/fail/imported-line-map/src/helper.0",
);
assert.equal(importedLineMapBody.diagnostics[0].line, 2);
assert.equal(importedLineMapBody.diagnostics[0].column, 3);

const importFixLineMapPatch = await execFileAsync(zero, [
  "fix",
  "--patch",
  "--json",
  "conformance/check/fail/import-fix-line-map",
]);
...
assert.equal(
  importFixLineMapPatchBody.diagnostics[0].path,
  "conformance/check/fail/import-fix-line-map/src/helper.0",
);
assert.equal(
  importFixLineMapPatchBody.patches[0].path,
  "conformance/check/fail/import-fix-line-map/src/helper.0",
);
assert.equal(importFixLineMapPatchBody.patches[0].line, 2);
assert.match(importFixLineMapPatchBody.patches[0].new, /mut values/);

const importCycleJson = await execFileAsync(zero, [
  "check",
  "--json",
  "conformance/check/fail/import-cycle",
]).catch((error) => error);
...
assert.equal(
  importCycleBody.diagnostics[0].path,
  "conformance/check/fail/import-cycle/src/b.0",
);
...
const importCycleFixPlan = await execFileAsync(zero, [
  "fix",
  "--plan",
  "--json",
  "conformance/check/fail/import-cycle",
...
assert.equal(
  importCycleFixPlanBody.diagnostics[0].path,
  "conformance/check/fail/import-cycle/src/b.0",
);
...
const duplicatePublicJson = await execFileAsync(zero, [
  "check",
  "--json",
  "conformance/check/fail/duplicate-public",
]).catch((error) => error);
...
const unresolvedNestedImportJson = await execFileAsync(zero, [
  "check",
  "--json",
  "conformance/check/fail/unresolved-nested-import",
]).catch((error) => error);
assert.notEqual(unresolvedNestedImportJson.code, 0);
const unresolvedNestedImportBody = JSON.parse(
  unresolvedNestedImportJson.stdout,
);
assert.equal(unresolvedNestedImportBody.diagnostics[0].code, "IMP001");
assert.match(
  unresolvedNestedImportBody.diagnostics[0].expected,
  /missing\.0 or missing\/mod\.0/,
);
assert.equal(
  unresolvedNestedImportBody.diagnostics[0].path,
  "conformance/check/fail/unresolved-nested-import/src/worker.0",
);
...
const duplicatePublicLargeJson = await execFileAsync(zero, [
  "check",
  "--json",
  "conformance/check/fail/duplicate-public-large",
]).catch((error) => error);
...
const genericConflictJson = await execFileAsync(zero, [
  "check",
  "--json",
  "conformance/check/fail/generic-inference-conflict.0",
]).catch((error) => error);
...
const genericStaticShadowConflictJson = await execFileAsync(zero, [
  "check",
  "--json",
  "conformance/check/fail/generic-static-shadow-conflict.0",
]).catch((error) => error);
assert.notEqual(genericStaticShadowConflictJson.code, 0);
const genericStaticShadowConflictBody = JSON.parse(
  genericStaticShadowConflictJson.stdout,
);
assert.equal(genericStaticShadowConflictBody.diagnostics[0].code, "TYP024");
assert.match(
  genericStaticShadowConflictBody.diagnostics[0].actual,
  /ref<\[4\]i32>/,
);

const genericStaticShadowExplicitJson = await execFileAsync(zero, [
  "check",
  "--json",
  "conformance/check/fail/generic-static-shadow-explicit-conflict.0",
]).catch((error) => error);
assert.notEqual(genericStaticShadowExplicitJson.code, 0);
const genericStaticShadowExplicitBody = JSON.parse(
  genericStaticShadowExplicitJson.stdout,
);
assert.equal(genericStaticShadowExplicitBody.diagnostics[0].code, "TYP001");
assert.match(
  genericStaticShadowExplicitBody.diagnostics[0].expected,
  /ref<\[N\]i32>/,
);
assert.match(
  genericStaticShadowExplicitBody.diagnostics[0].actual,
  /ref<\[4\]i32>/,
);

const genericStaticMethodShadowJson = await execFileAsync(zero, [
  "check",
  "--json",
  "conformance/check/fail/generic-static-method-shadow-conflict.0",
]).catch((error) => error);
assert.notEqual(genericStaticMethodShadowJson.code, 0);
const genericStaticMethodShadowBody = JSON.parse(
  genericStaticMethodShadowJson.stdout,
);
assert.equal(genericStaticMethodShadowBody.diagnostics[0].code, "TYP001");
assert.match(
  genericStaticMethodShadowBody.diagnostics[0].expected,
  /ref<\[N\]i32>/,
);
assert.match(
  genericStaticMethodShadowBody.diagnostics[0].actual,
  /ref<\[4\]i32>/,
);

const genericStaticShadowSignatureConstJson = await execFileAsync(zero, [
  "check",
  "--json",
  "conformance/check/fail/generic-static-shadow-signature-const.0",
]).catch((error) => error);
assert.notEqual(genericStaticShadowSignatureConstJson.code, 0);
const genericStaticShadowSignatureConstBody = JSON.parse(
  genericStaticShadowSignatureConstJson.stdout,
);
assert.equal(
  genericStaticShadowSignatureConstBody.diagnostics[0].code,
  "TYP001",
);
assert.match(
  genericStaticShadowSignatureConstBody.diagnostics[0].expected,
  /ref<\[4\]i32>/,
);
assert.match(
  genericStaticShadowSignatureConstBody.diagnostics[0].actual,
  /ref<\[N\]i32>/,
);

const genericStaticShadowSignatureConstGenericJson = await execFileAsync(zero, [
  "check",
  "--json",
  "conformance/check/fail/generic-static-shadow-signature-const-generic.0",
]).catch((error) => error);
assert.notEqual(genericStaticShadowSignatureConstGenericJson.code, 0);
const genericStaticShadowSignatureConstGenericBody = JSON.parse(
  genericStaticShadowSignatureConstGenericJson.stdout,
);
assert.equal(
  genericStaticShadowSignatureConstGenericBody.diagnostics[0].code,
  "TYP001",
);
assert.match(
  genericStaticShadowSignatureConstGenericBody.diagnostics[0].expected,
  /ref<\[4\]i32>/,
);
assert.match(
  genericStaticShadowSignatureConstGenericBody.diagnostics[0].actual,
  /ref<\[N\]i32>/,
);

const genericTypeParamShadowStaticJson = await execFileAsync(zero, [
  "check",
  "--json",
  "conformance/check/fail/generic-type-param-shadow-static-arg.0",
]).catch((error) => error);
assert.notEqual(genericTypeParamShadowStaticJson.code, 0);
const genericTypeParamShadowStaticBody = JSON.parse(
  genericTypeParamShadowStaticJson.stdout,
);
...
const genericTypeParamShadowArrayLengthJson = await execFileAsync(zero, [
  "check",
  "--json",
  "conformance/check/fail/generic-type-param-shadow-array-length.0",
]).catch((error) => error);
assert.notEqual(genericTypeParamShadowArrayLengthJson.code, 0);
const genericTypeParamShadowArrayLengthBody = JSON.parse(
  genericTypeParamShadowArrayLengthJson.stdout,
);
assert.equal(
  genericTypeParamShadowArrayLengthBody.diagnostics[0].code,
  "STC002",
);
assert.equal(genericTypeParamShadowArrayLengthBody.diagnostics[0].actual, "T");

const genericTypeParamShadowStaticAnnotationJson = await execFileAsync(zero, [
  "check",
  "--json",
  "conformance/check/fail/generic-type-param-shadow-static-annotation.0",
]).catch((error) => error);
assert.notEqual(genericTypeParamShadowStaticAnnotationJson.code, 0);
const genericTypeParamShadowStaticAnnotationBody = JSON.parse(
  genericTypeParamShadowStaticAnnotationJson.stdout,
);
assert.equal(
  genericTypeParamShadowStaticAnnotationBody.diagnostics[0].code,
  "STC002",
);
assert.equal(
  genericTypeParamShadowStaticAnnotationBody.diagnostics[0].actual,
  "T",
);

const genericTypeParamShadowStaticConstraintJson = await execFileAsync(zero, [
  "check",
  "--json",
  "conformance/check/fail/generic-type-param-shadow-static-constraint.0",
]).catch((error) => error);
assert.notEqual(genericTypeParamShadowStaticConstraintJson.code, 0);
const genericTypeParamShadowStaticConstraintBody = JSON.parse(
  genericTypeParamShadowStaticConstraintJson.stdout,
);
assert.equal(
  genericTypeParamShadowStaticConstraintBody.diagnostics[0].code,
  "STC002",
);
assert.equal(
  genericTypeParamShadowStaticConstraintBody.diagnostics[0].actual,
  "T",
);

const genericStaticParamTypeNameCollisionJson = await execFileAsync(zero, [
  "check",
  "--json",
  "conformance/check/fail/generic-static-param-type-name-collision.0",
]).catch((error) => error);
assert.notEqual(genericStaticParamTypeNameCollisionJson.code, 0);
const genericStaticParamTypeNameCollisionBody = JSON.parse(
  genericStaticParamTypeNameCollisionJson.stdout,
);
assert.equal(
  genericStaticParamTypeNameCollisionBody.diagnostics[0].code,
  "NAM004",
);
assert.match(
  genericStaticParamTypeNameCollisionBody.diagnostics[0].message,
  /generic static parameter shadows concrete type name/,
);
assert.match(
  genericStaticParamTypeNameCollisionBody.diagnostics[0].actual,
  /'Foo' already names a shape/,
);

const genericConstTypeNameCollisionMismatchJson = await execFileAsync(zero, [
  "check",
  "--json",
  "conformance/check/fail/generic-const-type-name-collision-mismatch.0",
]).catch((error) => error);
assert.notEqual(genericConstTypeNameCollisionMismatchJson.code, 0);
const genericConstTypeNameCollisionMismatchBody = JSON.parse(
  genericConstTypeNameCollisionMismatchJson.stdout,
);
assert.equal(
  genericConstTypeNameCollisionMismatchBody.diagnostics[0].code,
  "TYP001",
);
assert.match(
  genericConstTypeNameCollisionMismatchBody.diagnostics[0].expected,
  /Holder<Bar>/,
);
assert.match(
  genericConstTypeNameCollisionMismatchBody.diagnostics[0].actual,
  /Holder<Foo>/,
);

const genericArgCountJson = await execFileAsync(zero, [
  "check",
  "--json",
  "conformance/check/fail/generic-type-arg-count.0",
]).catch((error) => error);
...
assert.equal(
  genericArgCountBody.diagnostics[0].repair.id,
  "match-generic-type-arguments",
);

const genericCannotInferJson = await execFileAsync(zero, [
  "check",
  "--json",
  "conformance/check/fail/generic-cannot-infer.0",
]).catch((error) => error);
...
assert.equal(
  genericCannotInferBody.diagnostics[0].repair.id,
  "add-explicit-generic-type-arguments",
);

const constMismatchJson = await execFileAsync(zero, [
  "check",
  "--json",
  "conformance/check/fail/const-type-mismatch.0",
]).catch((error) => error);
...
const metaUnsupportedJson = await execFileAsync(zero, [
  "check",
  "--json",
  "conformance/check/fail/meta-unsupported.0",
]).catch((error) => error);
...
assert.equal(
  metaUnsupportedBody.diagnostics[0].repair.id,
  "remove-unsupported-meta-expression",
);

const metaCycleJson = await execFileAsync(zero, [
  "check",
  "--json",
  "conformance/check/fail/meta-cycle.0",
]).catch((error) => error);
...
const aliasCycleJson = await execFileAsync(zero, [
  "check",
  "--json",
  "conformance/check/fail/type-alias-cycle.0",
]).catch((error) => error);
...
const publicConstMissingTypeJson = await execFileAsync(zero, [
  "check",
  "--json",
  "conformance/check/fail/public-const-missing-type.0",
]).catch((error) => error);
assert.notEqual(publicConstMissingTypeJson.code, 0);
const publicConstMissingTypeBody = JSON.parse(
  publicConstMissingTypeJson.stdout,
);
assert.equal(publicConstMissingTypeBody.diagnostics[0].code, "PUB001");
assert.equal(
  publicConstMissingTypeBody.diagnostics[0].repair.id,
  "add-public-api-type",
);

const genericOwnedContainerJson = await execFileAsync(zero, [
  "check",
  "--json",
  "conformance/check/fail/generic-owned-container.0",
]).catch((error) => error);
...
assert.match(
  genericOwnedContainerBody.diagnostics[0].message,
  /generic containers cannot own generic payloads/,
);

const interfaceUnknownJson = await execFileAsync(zero, [
  "check",
  "--json",
  "conformance/check/fail/interface-unknown.0",
]).catch((error) => error);
...
assert.equal(
  interfaceUnknownBody.diagnostics[0].repair.id,
  "declare-or-use-static-interface",
);

const interfaceMissingMethodJson = await execFileAsync(zero, [
  "check",
  "--json",
  "conformance/check/fail/interface-missing-method.0",
]).catch((error) => error);
assert.notEqual(interfaceMissingMethodJson.code, 0);
const interfaceMissingMethodBody = JSON.parse(
  interfaceMissingMethodJson.stdout,
);
assert.equal(interfaceMissingMethodBody.diagnostics[0].code, "IFC002");
assert.match(
  interfaceMissingMethodBody.diagnostics[0].message,
  /Readable\.read/,
);
assert.match(
  interfaceMissingMethodBody.diagnostics[0].expected,
  /matching static method/,
);
assert.match(
  interfaceMissingMethodBody.diagnostics[0].actual,
  /method not found/,
);
assert.equal(
  interfaceMissingMethodBody.diagnostics[0].repair.id,
  "add-required-interface-method",
);

const interfaceArityMismatchJson = await execFileAsync(zero, [
  "check",
  "--json",
  "conformance/check/fail/interface-arity-mismatch.0",
]).catch((error) => error);
assert.notEqual(interfaceArityMismatchJson.code, 0);
const interfaceArityMismatchBody = JSON.parse(
  interfaceArityMismatchJson.stdout,
);
assert.equal(interfaceArityMismatchBody.diagnostics[0].code, "IFC003");

const interfaceReturnMismatchJson = await execFileAsync(zero, [
  "check",
  "--json",
  "conformance/check/fail/interface-return-mismatch.0",
]).catch((error) => error);
assert.notEqual(interfaceReturnMismatchJson.code, 0);
const interfaceReturnMismatchBody = JSON.parse(
  interfaceReturnMismatchJson.stdout,
);
assert.equal(interfaceReturnMismatchBody.diagnostics[0].code, "IFC004");

const interfaceParamMismatchJson = await execFileAsync(zero, [
  "check",
  "--json",
  "conformance/check/fail/interface-param-mismatch.0",
]).catch((error) => error);
assert.notEqual(interfaceParamMismatchJson.code, 0);
const interfaceParamMismatchBody = JSON.parse(
  interfaceParamMismatchJson.stdout,
);
assert.equal(interfaceParamMismatchBody.diagnostics[0].code, "IFC005");
assert.match(
  interfaceParamMismatchBody.diagnostics[0].expected,
  /ref<Counter>/,
);
...
await writeFile(
  interfaceStaticUnsupportedTypeFixture,
  `interface Bad<static N: String>
  fn value u8

pub fn main Void
`,
);
const interfaceStaticUnsupportedTypeJson = await execFileAsync(zero, [
  "check",
  "--json",
  interfaceStaticUnsupportedTypeFixture,
]).catch((error) => error);
assert.notEqual(interfaceStaticUnsupportedTypeJson.code, 0);
const interfaceStaticUnsupportedTypeBody = JSON.parse(
  interfaceStaticUnsupportedTypeJson.stdout,
);
...
await writeFile(
  interfaceMethodStaticUnsupportedTypeFixture,
  `interface Bad
...
const interfaceMethodStaticUnsupportedTypeJson = await execFileAsync(zero, [
  "check",
  "--json",
  interfaceMethodStaticUnsupportedTypeFixture,
]).catch((error) => error);
assert.notEqual(interfaceMethodStaticUnsupportedTypeJson.code, 0);
const interfaceMethodStaticUnsupportedTypeBody = JSON.parse(
  interfaceMethodStaticUnsupportedTypeJson.stdout,
);
assert.equal(
  interfaceMethodStaticUnsupportedTypeBody.diagnostics[0].code,
  "STC001",
);

const shapeMethodStaticUnsupportedTypeFixture = `${outDir}/shape-method-static-unsupported-type.0`;
await writeFile(
  shapeMethodStaticUnsupportedTypeFixture,
  `type Box
  value u8
...
const shapeMethodStaticUnsupportedTypeJson = await execFileAsync(zero, [
  "check",
  "--json",
  shapeMethodStaticUnsupportedTypeFixture,
]).catch((error) => error);
assert.notEqual(shapeMethodStaticUnsupportedTypeJson.code, 0);
const shapeMethodStaticUnsupportedTypeBody = JSON.parse(
  shapeMethodStaticUnsupportedTypeJson.stdout,
);
assert.equal(
  shapeMethodStaticUnsupportedTypeBody.diagnostics[0].code,
  "STC001",
);
...
for (const fixtureCase of methodUnknownConstraintFixtures) {
  ...
  const methodUnknownConstraintJson = await execFileAsync(zero, [
    "check",
    "--json",
    fixture,
  ]).catch((error) => error);
  assert.notEqual(methodUnknownConstraintJson.code, 0);
  const methodUnknownConstraintBody = JSON.parse(
    methodUnknownConstraintJson.stdout,
  );
...
for (const fixtureCase of duplicateStaticGenericFixtures) {
  const fixture = `${outDir}/${fixtureCase.name}.0`;
  await writeFile(fixture, fixtureCase.source);
  const duplicateStaticJson = await execFileAsync(zero, [
    "check",
    "--json",
    fixture,
  ]).catch((error) => error);
...
for (const value of ["4_", "M"]) {
  ...
  await writeFile(
    fixture,
    `interface First<T: Type, static N: usize>
...
`,
  ...
  const interfaceStaticConstraintJson = await execFileAsync(zero, [
    "check",
    "--json",
    fixture,
  ]).catch((error) => error);
  assert.notEqual(interfaceStaticConstraintJson.code, 0);
  const interfaceStaticConstraintBody = JSON.parse(
    interfaceStaticConstraintJson.stdout,
  );
...
const shapeMethodStaticParamJson = await execFileAsync(zero, [
  "check",
  "--json",
  shapeMethodStaticParamFixture,
]);
const shapeMethodStaticParamBody = JSON.parse(
  shapeMethodStaticParamJson.stdout,
);
assert.equal(shapeMethodStaticParamBody.ok, true);
const shapeMethodStaticParamGraph = await execFileAsync(zero, [
  "graph",
  "--json",
  shapeMethodStaticParamFixture,
]);
const shapeMethodStaticParamGraphBody = JSON.parse(
  shapeMethodStaticParamGraph.stdout,
);
const shapeMethodStaticBox = shapeMethodStaticParamGraphBody.shapes.find(
  (item) => item.name === "Box",
);
assert(shapeMethodStaticBox);
const shapeMethodStaticTag = shapeMethodStaticBox.methods.find(
  (item) => item.name === "tag",
);
assert(shapeMethodStaticTag);
assert(
  shapeMethodStaticTag.staticParams.some(
    (item) =>
      item.name === "N" &&
      item.type === "usize" &&
      item.staticDispatch === true,
  ),
);
...
const shapeMethodStaticCanonicalJson = await execFileAsync(zero, [
  "check",
  "--json",
  shapeMethodStaticCanonicalFixture,
]);
const shapeMethodStaticCanonicalBody = JSON.parse(
  shapeMethodStaticCanonicalJson.stdout,
);
...
await writeFile(
  interfaceMethodStaticParamFixture,
  `interface Width<T: Type>
...
const interfaceMethodStaticParamJson = await execFileAsync(zero, [
  "check",
  "--json",
  interfaceMethodStaticParamFixture,
]);
const interfaceMethodStaticParamBody = JSON.parse(
  interfaceMethodStaticParamJson.stdout,
);
assert.equal(interfaceMethodStaticParamBody.ok, true);
const interfaceMethodStaticParamGraph = await execFileAsync(zero, [
  "graph",
  "--json",
  interfaceMethodStaticParamFixture,
]);
const interfaceMethodStaticParamGraphBody = JSON.parse(
  interfaceMethodStaticParamGraph.stdout,
);
const interfaceMethodStaticWidth =
  interfaceMethodStaticParamGraphBody.interfaces.find(
    (item) => item.name === "Width",
  );
assert(interfaceMethodStaticWidth);
const interfaceMethodStaticWidthMethod =
  interfaceMethodStaticWidth.methods.find((item) => item.name === "width");
assert(interfaceMethodStaticWidthMethod);
assert(
  interfaceMethodStaticWidthMethod.staticParams.some(
    (item) =>
      item.name === "N" &&
      item.type === "usize" &&
      item.staticDispatch === true,
...
const interfaceMethodStaticBytes =
  interfaceMethodStaticParamGraphBody.shapes.find(
    (item) => item.name === "Bytes",
  );
assert(interfaceMethodStaticBytes);
const interfaceMethodStaticBytesWidth = interfaceMethodStaticBytes.methods.find(
  (item) => item.name === "width",
);
assert(interfaceMethodStaticBytesWidth);
assert(
  interfaceMethodStaticBytesWidth.staticParams.some(
    (item) =>
      item.name === "N" &&
      item.type === "usize" &&
      item.staticDispatch === true,
...
await writeFile(
  interfaceMethodStaticRenamedParamFixture,
  `interface Width<T: Type>
...
const interfaceMethodStaticRenamedParamJson = await execFileAsync(zero, [
  "check",
  "--json",
  interfaceMethodStaticRenamedParamFixture,
]);
const interfaceMethodStaticRenamedParamBody = JSON.parse(
  interfaceMethodStaticRenamedParamJson.stdout,
);
assert.equal(interfaceMethodStaticRenamedParamBody.ok, true);
const interfaceMethodStaticRenamedParamGraph = await execFileAsync(zero, [
  "graph",
  "--json",
  interfaceMethodStaticRenamedParamFixture,
]);
const interfaceMethodStaticRenamedParamGraphBody = JSON.parse(
  interfaceMethodStaticRenamedParamGraph.stdout,
);
const interfaceMethodStaticRenamedWidth =
  interfaceMethodStaticRenamedParamGraphBody.interfaces.find(
    (item) => item.name === "Width",
  );
assert(interfaceMethodStaticRenamedWidth);
const interfaceMethodStaticRenamedWidthMethod =
  interfaceMethodStaticRenamedWidth.methods.find(
    (item) => item.name === "width",
  );
assert(interfaceMethodStaticRenamedWidthMethod);
assert(
  interfaceMethodStaticRenamedWidthMethod.staticParams.some(
    (item) =>
      item.name === "N" &&
      item.type === "usize" &&
      item.staticDispatch === true,
...
const interfaceMethodStaticRenamedBytes =
  interfaceMethodStaticRenamedParamGraphBody.shapes.find(
    (item) => item.name === "Bytes",
  );
assert(interfaceMethodStaticRenamedBytes);
const interfaceMethodStaticRenamedBytesWidth =
  interfaceMethodStaticRenamedBytes.methods.find(
    (item) => item.name === "width",
  );
assert(interfaceMethodStaticRenamedBytesWidth);
assert(
  interfaceMethodStaticRenamedBytesWidth.staticParams.some(
    (item) =>
      item.name === "M" &&
      item.type === "usize" &&
      item.staticDispatch === true,
...
await writeFile(
  staticInterfaceReturnMismatchFixture,
...
const staticInterfaceReturnMismatchJson = await execFileAsync(zero, [
  "check",
  "--json",
  staticInterfaceReturnMismatchFixture,
]).catch((error) => error);
assert.notEqual(staticInterfaceReturnMismatchJson.code, 0);
const staticInterfaceReturnMismatchBody = JSON.parse(
  staticInterfaceReturnMismatchJson.stdout,
);
assert.equal(staticInterfaceReturnMismatchBody.diagnostics[0].code, "IFC004");
assert.match(
  staticInterfaceReturnMismatchBody.diagnostics[0].expected,
  /\[3\]u8/,
);
assert.match(
  staticInterfaceReturnMismatchBody.diagnostics[0].actual,
  /\[4\]u8/,
);

const shapeMethodGenericConstraintFixture = `${outDir}/shape-method-generic-constraint.0`;
await writeFile(
  shapeMethodGenericConstraintFixture,
  `interface NeedsMethod<T: Type>
...
const shapeMethodGenericConstraintJson = await execFileAsync(zero, [
  "check",
  "--json",
  shapeMethodGenericConstraintFixture,
]).catch((error) => error);
assert.notEqual(shapeMethodGenericConstraintJson.code, 0);
const shapeMethodGenericConstraintBody = JSON.parse(
  shapeMethodGenericConstraintJson.stdout,
);
...
await writeFile(
  interfaceMethodGenericConstraintFixture,
  `interface NeedsMethod<T: Type>
  fn need u8 self ref<T>
...
const interfaceMethodGenericConstraintJson = await execFileAsync(zero, [
  "check",
  "--json",
  interfaceMethodGenericConstraintFixture,
]).catch((error) => error);
assert.notEqual(interfaceMethodGenericConstraintJson.code, 0);
const interfaceMethodGenericConstraintBody = JSON.parse(
  interfaceMethodGenericConstraintJson.stdout,
);
assert.equal(
  interfaceMethodGenericConstraintBody.diagnostics[0].code,
  "IFC002",
);
...
for (const fixtureCase of interfaceMethodGenericMismatchFixtures) {
  const fixture = `${outDir}/${fixtureCase.name}.0`;
  await writeFile(fixture, fixtureCase.source);
  const interfaceMethodGenericMismatchJson = await execFileAsync(zero, [
    "check",
    "--json",
    fixture,
  ]).catch((error) => error);
  assert.notEqual(interfaceMethodGenericMismatchJson.code, 0);
  const interfaceMethodGenericMismatchBody = JSON.parse(
    interfaceMethodGenericMismatchJson.stdout,
  );
  assert.equal(
    interfaceMethodGenericMismatchBody.diagnostics[0].code,
    fixtureCase.code,
  );
  if (fixtureCase.message)
    assert.match(
      interfaceMethodGenericMismatchBody.diagnostics[0].message,
      fixtureCase.message,
    );
}

const staticUnsupportedTypeJson = await execFileAsync(zero, [
  "check",
  "--json",
  "conformance/check/fail/static-value-unsupported-type.0",
]).catch((error) => error);
...
assert.equal(
  staticUnsupportedTypeBody.diagnostics[0].repair.id,
  "use-supported-static-value-type",
);

const staticNonConstantJson = await execFileAsync(zero, [
  "check",
  "--json",
  "conformance/check/fail/static-value-non-constant.0",
]).catch((error) => error);
...
assert.equal(
  staticNonConstantBody.diagnostics[0].repair.id,
  "pass-constant-static-value",
);
...
`,
  ...
  const malformedStaticJson = await execFileAsync(zero, [
    "check",
    "--json",
    fixture,
  ]).catch((error) => error);
...
for (const value of ["4_", "4__5", "0x_1", "4_nope"]) {
  ...
  await writeFile(
    fixture,
    `fn take Void bytes [${value}]u8

pub fn main Void
`,
  );
  const malformedArrayJson = await execFileAsync(zero, [
    "check",
    "--json",
    fixture,
  ]).catch((error) => error);
...
const staticMismatchJson = await execFileAsync(zero, [
  "check",
  "--json",
  "conformance/check/fail/static-value-mismatch.0",
]).catch((error) => error);
...
assert.equal(
  staticMismatchBody.diagnostics[0].repair.id,
  "match-static-value-argument",
);

const staticAliasMismatchJson = await execFileAsync(zero, [
  "check",
  "--json",
  "conformance/check/fail/static-value-alias-mismatch.0",
]).catch((error) => error);
...
assert.match(
  staticAliasMismatchBody.diagnostics[0].actual,
  /Box<FixedVec<u8,5>>/,
);
assert.equal(
  staticAliasMismatchBody.diagnostics[0].repair.id,
  "match-static-value-argument",
);

const staticBoolEnumMismatchJson = await execFileAsync(zero, [
  "check",
  "--json",
  "conformance/check/fail/static-value-bool-enum-mismatch.0",
]).catch((error) => error);
assert.notEqual(staticBoolEnumMismatchJson.code, 0);
const staticBoolEnumMismatchBody = JSON.parse(
  staticBoolEnumMismatchJson.stdout,
);
assert.equal(staticBoolEnumMismatchBody.diagnostics[0].code, "STC003");
assert.match(
  staticBoolEnumMismatchBody.diagnostics[0].expected,
  /Gate<false,Mode\.fast>/,
);
assert.match(
  staticBoolEnumMismatchBody.diagnostics[0].actual,
  /Gate<true,Mode\.fast>/,
);
assert.equal(
  staticBoolEnumMismatchBody.diagnostics[0].repair.id,
  "match-static-value-argument",
);

const shapeMethodCannotInferJson = await execFileAsync(zero, [
  "check",
  "--json",
  "conformance/check/fail/generic-shape-method-cannot-infer.0",
]).catch((error) => error);
assert.notEqual(shapeMethodCannotInferJson.code, 0);
const shapeMethodCannotInferBody = JSON.parse(
  shapeMethodCannotInferJson.stdout,
);
assert.equal(shapeMethodCannotInferBody.diagnostics[0].code, "SHM001");
assert.equal(
  shapeMethodCannotInferBody.diagnostics[0].repair.id,
  "bind-generic-shape-method",
);

const shapeMethodSelfMismatchJson = await execFileAsync(zero, [
  "check",
  "--json",
  "conformance/check/fail/generic-shape-method-self-mismatch.0",
]).catch((error) => error);
assert.notEqual(shapeMethodSelfMismatchJson.code, 0);
const shapeMethodSelfMismatchBody = JSON.parse(
  shapeMethodSelfMismatchJson.stdout,
);
assert.equal(shapeMethodSelfMismatchBody.diagnostics[0].code, "SHM002");
assert.equal(
  shapeMethodSelfMismatchBody.diagnostics[0].repair.id,
  "match-shape-method-self",
);

const selfOutsideShapeJson = await execFileAsync(zero, [
  "check",
  "--json",
  "conformance/check/fail/self-outside-shape.0",
]).catch((error) => error);
...
const arrayLengthMismatchJson = await execFileAsync(zero, [
  "check",
  "--json",
  "conformance/check/fail/array-literal-length-mismatch.0",
]).catch((error) => error);
...
assert.match(
  arrayLengthMismatchBody.diagnostics[0].message,
  /array literal length/,
);
assert.match(arrayLengthMismatchBody.diagnostics[0].actual, /3 element/);

const arrayElementMismatchJson = await execFileAsync(zero, [
  "check",
  "--json",
  "conformance/check/fail/array-literal-element-mismatch.0",
]).catch((error) => error);
...
assert.match(
  arrayElementMismatchBody.diagnostics[0].message,
  /array literal element/,
);
assert.equal(arrayElementMismatchBody.diagnostics[0].expected, "u8");

const receiverMethodImmutableJson = await execFileAsync(zero, [
  "check",
  "--json",
  "conformance/check/fail/receiver-method-immutable.0",
]).catch((error) => error);
assert.notEqual(receiverMethodImmutableJson.code, 0);
const receiverMethodImmutableBody = JSON.parse(
  receiverMethodImmutableJson.stdout,
);
assert.equal(receiverMethodImmutableBody.diagnostics[0].code, "RCV002");
assert.equal(
  receiverMethodImmutableBody.diagnostics[0].repair.id,
  "make-receiver-addressable-or-mutable",
);

const receiverMethodUnknownJson = await execFileAsync(zero, [
  "check",
  "--json",
  "conformance/check/fail/receiver-method-unknown.0",
]).catch((error) => error);
...
assert.equal(
  receiverMethodUnknownBody.diagnostics[0].repair.id,
  "call-declared-receiver-method",
);

const shapeDefaultMissingRequiredJson = await execFileAsync(zero, [
  "check",
  "--json",
  "conformance/check/fail/shape-default-missing-required.0",
]).catch((error) => error);
assert.notEqual(shapeDefaultMissingRequiredJson.code, 0);
const shapeDefaultMissingRequiredBody = JSON.parse(
  shapeDefaultMissingRequiredJson.stdout,
);
...
const shapeDefaultTypeMismatchJson = await execFileAsync(zero, [
  "check",
  "--json",
  "conformance/check/fail/shape-default-type-mismatch.0",
]).catch((error) => error);
assert.notEqual(shapeDefaultTypeMismatchJson.code, 0);
const shapeDefaultTypeMismatchBody = JSON.parse(
  shapeDefaultTypeMismatchJson.stdout,
);
...
const matchFallbackPayloadJson = await execFileAsync(zero, [
  "check",
  "--json",
  "conformance/check/fail/match-fallback-payload.0",
]).catch((error) => error);
...
const matchDuplicateFallbackJson = await execFileAsync(zero, [
  "check",
  "--json",
  "conformance/check/fail/match-duplicate-fallback.0",
]).catch((error) => error);
assert.notEqual(matchDuplicateFallbackJson.code, 0);
const matchDuplicateFallbackBody = JSON.parse(
  matchDuplicateFallbackJson.stdout,
);
assert.equal(matchDuplicateFallbackBody.diagnostics[0].code, "MAT003");
assert.match(
  matchDuplicateFallbackBody.diagnostics[0].message,
  /duplicate match fallback/,
);

const matchBoolNonExhaustiveJson = await execFileAsync(zero, [
  "check",
  "--json",
  "conformance/check/fail/match-bool-non-exhaustive.0",
]).catch((error) => error);
assert.notEqual(matchBoolNonExhaustiveJson.code, 0);
const matchBoolNonExhaustiveBody = JSON.parse(
  matchBoolNonExhaustiveJson.stdout,
);
...
const matchU8NonExhaustiveJson = await execFileAsync(zero, [
  "check",
  "--json",
  "conformance/check/fail/match-u8-non-exhaustive.0",
]).catch((error) => error);
...
const matchGuardNonBoolJson = await execFileAsync(zero, [
  "check",
  "--json",
  "conformance/check/fail/match-guard-non-bool.0",
]).catch((error) => error);
...
const badTargetNameJson = await execFileAsync(zero, [
  "check",
  "--json",
  "--target",
  "not-a-target",
...
assert.equal(
  badTargetNameBody.diagnostics[0].fixSafety,
  "requires-human-review",
);
assert.equal(badTargetNameBody.diagnostics[0].repair.id, "manual-review");
assert.match(
  badTargetNameBody.diagnostics[0].related[0].message,
  /not-a-target/,
);

const explainTar002 = await execFileAsync(zero, [
  "explain",
  "--json",
  "TAR002",
]);
...
const genericFixPlanJson = await execFileAsync(zero, [
  "fix",
  "--plan",
  "--json",
  "conformance/check/fail/generic-cannot-infer.0",
...
assert.equal(
  genericFixPlanBody.diagnostics[0].repair.id,
  "add-explicit-generic-type-arguments",
);

const fixPatchJson = await execFileAsync(zero, [
  "fix",
  "--patch",
  "--json",
  "conformance/native/fail/mem-copy-immutable-dst.0",
...
await writeFile(
  fixApplyFixture,
  await readFile("conformance/native/fail/mem-copy-immutable-dst.0", "utf8"),
);
const fixApplyJson = await execFileAsync(zero, [
  "fix",
  "--apply",
  "--json",
  fixApplyFixture,
...
const importGraph = await execFileAsync(zero, [
  "graph",
  "--json",
  "conformance/check/pass/imports",
]);
...
assert(
  importGraphBody.sourceMaps.every((item) => item.columnUnit === "utf8-byte"),
);
assert.deepEqual(
  importGraphBody.targets.map((item) => item.name),
  ["cli"],
);
assert.deepEqual(
  importGraphBody.modules.map((item) => item.name),
  ["math", "types", "main"],
);
assert.deepEqual(
  importGraphBody.importEdges.map((item) => `${item.from}->${item.to}`),
  ["main->math", "main->types"],
);
assert.deepEqual(
  importGraphBody.importEdges.map(
    (item) =>
      `${item.from}->${item.to}:${item.sourceRange.path}:${item.sourceRange.start.line}:${item.sourceRange.start.column}:${item.sourceRange.end.column}`,
  ),
  [
...
assert.deepEqual(
  importGraphBody.useImports.map(
    (item) =>
      `${item.from}->${item.to}:${item.kind}:${item.line}:${item.column}`,
  ),
  ["main->math:package-local:1:1", "main->types:package-local:2:1"],
);
assert.deepEqual(
  importGraphBody.useImports.map((item) => item.resolvedPath),
  [
...
assert(
  importGraphBody.useImports.every(
    (item) => item.sourceRange.columnUnit === "utf8-byte",
  ),
);
assert(
  importGraphBody.symbols.some(
    (item) => item.module === "math" && item.name === "add_one",
  ),
);
assert(
  importGraphBody.functions.some(
    (item) =>
      item.name === "main" &&
      item.returnType === "Void" &&
      item.effects.includes("world"),
  ),
);
...
await writeFile(
  `${whitespaceUsePackage}/zero.json`,
  JSON.stringify(
    {
...
await writeFile(
  `${whitespaceUsePackage}/src/math.0`,
  "fn add_one i32 value i32\n  ret + value 1\n",
);
await writeFile(
  `${whitespaceUsePackage}/src/math/util.0`,
  "fn add_two i32 value i32\n  ret + value 2\n",
);
await writeFile(
  `${whitespaceUsePackage}/src/types.0`,
  "type Point\n  value i32\n",
);
await writeFile(
  `${whitespaceUsePackage}/src/main.0`,
  'use math\nuse math . util\nuse   types   as   model\n\npub fn main Void world World !\n  let point Point . value (add_two 40)\n  if == point.value (add_one 41)\n    check world.out.write "whitespace imports pass\\n"\n',
);
const whitespaceUseCheck = await execFileAsync(zero, [
  "check",
  "--json",
  whitespaceUsePackage,
]);
assert.equal(JSON.parse(whitespaceUseCheck.stdout).ok, true);
const whitespaceUseGraph = await execFileAsync(zero, [
  "graph",
  "--json",
  whitespaceUsePackage,
]);
...
assert.deepEqual(
  whitespaceUseGraphBody.useImports.map(
    (item) =>
      `${item.from}->${item.to}:${item.kind}:${item.alias ?? "null"}:${item.sourceRange.end.column}`,
  ),
  [
...
assert.deepEqual(
  whitespaceUseGraphBody.importEdges.map((item) => `${item.from}->${item.to}`),
  ["main->math", "main->math.util", "main->types"],
);

const packageUseGraph = await execFileAsync(zero, [
  "graph",
  "--json",
  "conformance/check/pass/package",
]);
...
assert.deepEqual(
  packageUseGraphBody.useImports.map(
    (item) =>
      `${item.from}->${item.to}:${item.kind}:${item.resolvedPath ?? "null"}`,
  ),
  [
...
const resourceGraph = await execFileAsync(zero, [
  "graph",
  "--json",
...
assert(
  resourceGraphBody.targets.some(
    (item) => item.name === "cli" && item.kind === "exe",
  ),
);
assert.deepEqual(
  resourceGraphBody.importEdges.map((item) => `${item.from}->${item.to}`),
  ["main->config", "main->payload"],
);
assert(resourceGraphBody.requiresCapabilities.includes("fs"));
assert(
  resourceGraphBody.functions.some(
    (item) => item.name === "outputDir" && item.effects.includes("env"),
  ),
);
const resourceMainSymbol = resourceGraphBody.symbols.find(
  (item) => item.name === "main",
);
...
const memoryGraph = await execFileAsync(zero, [
  "graph",
  "--json",
  "--target",
  "linux-musl-x64",
...
assert.deepEqual(
  memoryGraphBody.importEdges.map((item) => `${item.from}->${item.to}`),
  ["main->buffer", "main->checksum"],
);
...
assert.equal(
  memoryGraphBody.targetSupport.requiredCapabilitySupport.status,
  "supported",
);
assert.equal(memoryGraphBody.symbolCounts.public, 5);
assert(
  memoryGraphBody.stdlibHelpers.some(
    (helper) =>
      helper.name === "std.mem.copy" &&
      helper.targetSupport === "target-neutral",
  ),
);
assert(
  memoryGraphBody.stdlibHelpers.some(
    (helper) =>
      helper.name === "std.fs.createOrRaise" && helper.targetSupport === "host",
  ),
);
const memCopyHelper = memoryGraphBody.stdlibHelpers.find(
  (helper) => helper.name === "std.mem.copy",
);
...
const lexerTokens = await execFileAsync(zero, [
  "tokens",
  "--json",
  "conformance/lexer/compiler-smoke.0",
]);
const lexerTokensBody = JSON.parse(lexerTokens.stdout);
assert.equal(lexerTokensBody.schemaVersion, 1);
assert.deepEqual(
  lexerTokensBody.tokens
    .slice(0, 4)
    .map((token) => `${token.kind}:${token.text}`),
  ["word:use", "word:std", "symbol:.", "word:mem"],
);
assert.deepEqual(
  lexerTokensBody.tokens
    .filter((token) => token.kind === "number")
    .map((token) => token.text),
  ["123", "0xff", "0b101", "42_u8"],
);
assert.deepEqual(
  lexerTokensBody.tokens
    .filter((token) => token.kind === "string" || token.kind === "char")
    .map((token) => `${token.kind}:${token.text}`),
  ["string:hi", "char:120"],
);
...
const parseTree = await execFileAsync(zero, [
  "parse",
  "--json",
...
assert.deepEqual(parseTreeBody.functions[0].bodyKinds, [
  "if",
  "while",
  "check",
  "return",
...
const constGraph = await execFileAsync(zero, [
  "graph",
  "--json",
  "examples/const-arithmetic.0",
]);
...
assert(
  constGraphBody.consts.some(
    (item) => item.name === "answer" && item.type === "i32",
  ),
);
assert(
  constGraphBody.symbols.some(
    (item) =>
      item.name === "answer" && item.kind === "const" && item.public === false,
  ),
);

const genericPairGraph = await execFileAsync(zero, [
  "graph",
  "--json",
  "examples/generic-pair.0",
]);
...
assert(
  genericPairGraphBody.functions.some(
    (item) =>
      item.name === "makePair" &&
      item.generic === true &&
      item.returnType === "Pair<T,U>",
  ),
);
assert(
  genericPairGraphBody.shapes.some(
    (item) =>
      item.name === "Pair" &&
      item.generic === true &&
      item.typeParams.join(",") === "T,U",
  ),
);

const staticValueGraph = await execFileAsync(zero, [
  "graph",
  "--json",
  "examples/static-value-params.0",
]);
const staticValueGraphBody = JSON.parse(staticValueGraph.stdout);
const fixedVecShape = staticValueGraphBody.shapes.find(
  (item) => item.name === "FixedVec",
);
assert(fixedVecShape);
assert(
  fixedVecShape.staticParams.some(
    (item) =>
      item.name === "N" &&
      item.type === "usize" &&
      item.staticDispatch === true,
...
assert(
  staticValueGraphBody.functions.some(
    (item) =>
      item.name === "first" &&
      item.staticParams.some((param) => param.name === "N"),
  ),
);

const fixedVecGraph = await execFileAsync(zero, [
  "graph",
  "--json",
  "examples/fixed-vec.0",
]);
const fixedVecGraphBody = JSON.parse(fixedVecGraph.stdout);
const fixedVecMethodsShape = fixedVecGraphBody.shapes.find(
  (item) => item.name === "FixedVec",
);
assert(fixedVecMethodsShape);
const pushMethod = fixedVecMethodsShape.methods.find(
  (item) => item.name === "push",
);
assert(pushMethod);
...
assert(
  pushMethod.shapeStaticParams.some(
    (item) => item.name === "N" && item.staticDispatch === true,
  ),
);

const aliasGraph = await execFileAsync(zero, [
  "graph",
  "--json",
  "examples/type-alias.0",
]);
...
assert(
  aliasGraphBody.aliases.some(
    (item) => item.name === "BytePair" && item.target === "Pair<u8,u8>",
  ),
);
assert(
  aliasGraphBody.symbols.some(
    (item) => item.name === "BytePair" && item.kind === "type-alias",
  ),
);

const staticMethodGraph = await execFileAsync(zero, [
  "graph",
  "--json",
  "examples/static-method.0",
]);
const staticMethodGraphBody = JSON.parse(staticMethodGraph.stdout);
const counterShape = staticMethodGraphBody.shapes.find(
  (item) => item.name === "Counter",
);
assert(counterShape);
assert(
  counterShape.methods.some(
    (item) =>
      item.name === "add" &&
      item.staticDispatch === true &&
      item.returnType === "i32",
...
const staticInterfaceGraph = await execFileAsync(zero, [
  "graph",
  "--json",
  "examples/static-interface.0",
]);
...
assert(
  staticInterfaceGraphBody.interfaces.some(
    (item) => item.name === "Readable" && item.staticOnly === true,
  ),
);
assert(
  staticInterfaceGraphBody.functions.some(
    (item) =>
      item.name === "readValue" &&
      item.constraints.some(
        (constraint) =>
          constraint.interface === "Readable<T>" &&
          constraint.staticDispatch === true,
      ),
  ),
);
assert(
  staticInterfaceGraphBody.symbols.some(
    (item) => item.name === "Readable" && item.kind === "interface",
  ),
);

const memorySize = await execFileAsync(zero, [
  "size",
  "--json",
  "--target",
  "linux-musl-x64",
...
assert(
  memorySizeBody.sections.some(
    (section) =>
      section.name === "direct-size-metadata" && section.kind === "metadata",
  ),
);
...
assert(
  memorySizeBody.stdlibHelpers.some((helper) => helper.name === "std.mem.copy"),
);
assert(
  memorySizeBody.stdlibHelperAttribution.some(
    (helper) =>
      helper.name === "std.mem.copy" && helper.estimatedDirectBytes > 0,
  ),
);
assert(
  memorySizeBody.usedStdlibHelpers.every(
    (helper) =>
      helper.module &&
      helper.effects?.length &&
      helper.errorBehavior &&
      helper.ownershipNotes &&
      helper.example,
  ),
);
assert(
  memorySizeBody.runtimeShims.some(
    (shim) => shim.name === "bounds-checks" && shim.payAsUsed === true,
  ),
);

const genericPairSize = await execFileAsync(zero, [
  "size",
  "--json",
  "examples/generic-pair.0",
]);
...
assert(
  genericPairSizeBody.genericSpecializations.some(
    (item) => item.name === "z_Pair_i32_u8_",
  ),
);
assert(
  genericPairSizeBody.genericSpecializations.some(
    (item) => item.name === "z_makePair__i32__u8",
  ),
);

const genericInferredSize = await execFileAsync(zero, [
  "size",
  "--json",
  "conformance/native/pass/generic-inferred-specialized-call.0",
]);
...
assert(
  genericInferredSizeBody.genericSpecializations.some(
    (item) => item.name === "z_forward__i32",
  ),
);
assert(
  genericInferredSizeBody.genericSpecializations.some(
    (item) => item.name === "z_identity__i32",
  ),
);
assert(
  !genericInferredSizeBody.genericSpecializations.some(
    (item) => item.name === "z_identity__T",
  ),
);

const genericStaticForwardedSize = await execFileAsync(zero, [
  "size",
  "--json",
  "conformance/native/pass/generic-static-forwarded-array-specialization.0",
]);
const genericStaticForwardedSizeBody = JSON.parse(
  genericStaticForwardedSize.stdout,
);
assert(
  genericStaticForwardedSizeBody.genericSpecializations.some(
    (item) => item.name === "z_outer__4",
  ),
);
assert(
  genericStaticForwardedSizeBody.genericSpecializations.some(
    (item) => item.name === "z_inner__4",
  ),
);
assert(
  !genericStaticForwardedSizeBody.genericSpecializations.some(
    (item) => item.name === "z_inner__N",
  ),
);

const targetsJson = await execFileAsync(zero, ["targets"]);
const targetsBody = JSON.parse(targetsJson.stdout);
const linuxMuslTarget = targetsBody.targets.find(
  (item) => item.name === "linux-musl-x64",
);
const windowsMsvcTarget = targetsBody.targets.find((item) =>
  item.aliases.includes("x86_64-windows-msvc"),
);
const linuxGnuTarget = targetsBody.targets.find(
  (item) => item.name === "linux-x64",
);
const darwinArm64Target = targetsBody.targets.find(
  (item) => item.name === "darwin-arm64",
);
const linuxArm64Target = targetsBody.targets.find(
  (item) => item.name === "linux-arm64",
);
assert(
  linuxMuslTarget.capabilityFacts.some(
    (item) => item.name === "fs" && item.available === true,
  ),
);
assert.equal(linuxMuslTarget.abi, "musl");
assert.equal(linuxMuslTarget.objectFormat, "elf");
assert.equal(
  linuxMuslTarget.toolchain.crossCompiler,
  "target-capable C compiler",
);
...
assert.equal(
  linuxMuslTarget.directBackend.explicitDirectFallback,
  "never-c-bridge",
);
...
assert.equal(
  darwinArm64Target.httpRuntime.tlsVerification,
  targetsBody.host === "darwin-arm64",
);
if (targetsBody.host === "darwin-arm64") {
  assert.equal(
    darwinArm64Target.httpRuntime.customCa.env,
    "ZERO_HTTP_TEST_CA_BUNDLE",
  );
}
...
assert.match(
  linuxArm64Target.directBackend.reason,
  /direct object and executable backend available/,
);
...
const targetNetUnsupportedJson = await execFileAsync(zero, [
  "check",
  "--json",
  "--target",
  "linux-musl-x64",
  "conformance/check/fail/target-net-unsupported.0",
]).catch((error) => error);
...
const targetProcUnsupportedJson = await execFileAsync(zero, [
  "check",
  "--json",
  "--target",
  "linux-musl-x64",
  "conformance/check/fail/target-proc-unsupported.0",
]).catch((error) => error);
...
const cHeaderGraph = await execFileAsync(zero, [
  "graph",
  "--json",
  "conformance/check/pass/c-header-import.0",
]);
...
assert(
  cHeaderGraphBody.cImports.some(
    (item) =>
      item.header === "conformance/c/simple.h" &&
      item.imports.functions >= 1 &&
      item.cacheKey,
  ),
);
const simpleHeaderImport = cHeaderGraphBody.cImports.find(
  (item) => item.header === "conformance/c/simple.h",
);
assert(
  simpleHeaderImport.typedModel.functions.some(
    (item) => item.name === "zero_c_add" && item.params.length === 2,
  ),
);
assert(
  simpleHeaderImport.typedModel.constants.some(
    (item) => item.name === "ZERO_C_ANSWER" && item.value === "42",
  ),
);
assert(
  simpleHeaderImport.typedModel.structs.some(
    (item) =>
      item.name === "zero_c_point" &&
      item.fields.some((field) => field.name === "y"),
  ),
);
assert(
  simpleHeaderImport.typedModel.enums.some(
    (item) =>
      item.name === "zero_c_color" &&
      item.cases.some((entry) => entry.name === "ZERO_C_BLUE"),
  ),
);
assert(
  simpleHeaderImport.typedModel.typedefs.some(
    (item) => item.name === "zero_c_int" && item.target === "int",
  ),
);
assert.equal(typeof simpleHeaderImport.cache.target, "string");

const cInteropGraph = await execFileAsync(zero, [
  "graph",
  "--json",
  "examples/c-interop",
]);
const cInteropGraphBody = JSON.parse(cInteropGraph.stdout);
const mathLib = cInteropGraphBody.cLibraries.find(
  (item) => item.name === "math",
);
assert(mathLib);
...
const cInteropCrossGraph = await execFileAsync(zero, [
  "graph",
  "--json",
  "--target",
  "linux-musl-x64",
...
assert.equal(
  cInteropCrossGraphBody.cLibraries[0].targetValidation.pkgConfigTargetSafe,
  true,
);
assert.equal(
  cInteropCrossGraphBody.cLibraries[0].targetValidation.implicitHostDiscovery,
  false,
);
assert.equal(
  cInteropCrossGraphBody.cLibraries[0].linkPlan.hostDiscovery,
  "none",
);
const hostLeakGraph = await execFileAsync(zero, [
  "graph",
  "--json",
  "--target",
  "linux-musl-x64",
...
assert.equal(
  hostLeakGraphBody.cLibraries[0].targetValidation.hostHeaderLeakage,
  true,
);
assert.equal(
  hostLeakGraphBody.cLibraries[0].targetValidation.implicitHostDiscovery,
  true,
);
assert.equal(
  hostLeakGraphBody.cLibraries[0].targetValidation.status,
  "blocked",
);
const hostLeakReadiness = await execFileAsync(zero, [
  "check",
  "--json",
  "--target",
  "linux-musl-x64",
...
assert.equal(
  hostLeakReadinessBody.targetReadiness.diagnostics[0].code,
  "CIMP003",
);
assert.match(
  hostLeakReadinessBody.targetReadiness.diagnostics[0].help,
  /target sysroot|vendored/,
);
const hostLeakBuild = await execFileAsync(
  zero,
  [
    "build",
    "--json",
...
const depGraph = await execFileAsync(zero, [
  "graph",
  "--json",
  "--target",
  "linux-musl-x64",
...
assert.match(
  depGraphBody.package.lockfile.path,
  /\.zero\/package-locks\/[0-9a-f]+\.lock\.json/,
);
assert(
  depGraphBody.package.dependencies.some(
    (item) =>
      item.name === "dep-lib" &&
      item.status === "path-resolved" &&
      item.targetCompatible === true,
  ),
);
assert(
  depGraphBody.package.dependencies.some(
    (item) =>
      item.name === "remote-tools" &&
      item.status === "registry-reference" &&
      item.version === "1.2.3",
  ),
);
...
assert.match(
  depGraphBody.packageCache.cacheKeyInputs.dependencyGraphHash,
  /^[0-9a-f]{16}$/,
);
const depDoc = await execFileAsync(zero, [
  "doc",
  "--json",
  "conformance/packages/dep-app",
]);
...
const depBuild = await execFileAsync(zero, [
  "build",
  "--json",
  "--target",
  "linux-musl-x64",
...
assert.equal(
  depBuildBody.packageCache.invalidationReasons.includes(
    "dependency graph changed",
  ),
  true,
);
assert.match(
  depBuildBody.compilerCaches[0].dependencyGraphHash,
  /^[0-9a-f]{16}$/,
);
const targetGraph = await execFileAsync(zero, [
  "graph",
  "--json",
  "--target",
  "linux-musl-x64",
  "conformance/packages/target-incompatible-app",
]);
...
assert.equal(
  JSON.parse(targetIncompatibleDep.stdout).diagnostics[0].code,
  "PKG004",
);

const cHeaderUnsupportedJson = await execFileAsync(zero, [
  "check",
  "--json",
  "conformance/check/fail/c-header-unsupported.0",
]).catch((error) => error);
...
const errorSetFixPlan = await execFileAsync(zero, [
  "fix",
  "--plan",
  "--json",
  "conformance/native/fail/std-fs-error-set-mismatch.0",
]);
...
assert.equal(
  errorSetFixPlanBody.diagnostics[0].repair.id,
  "add-missing-error-name",
);

const parseFixPlan = await execFileAsync(zero, [
  "fix",
  "--plan",
  "--json",
  "conformance/check/fail/parse-missing-brace.0",
...
const nameFixPlan = await execFileAsync(zero, [
  "fix",
  "--plan",
  "--json",
  "conformance/check/fail/unknown-name.0",
]);
...
const typeFixPlan = await execFileAsync(zero, [
  "fix",
  "--plan",
  "--json",
  "conformance/check/fail/wrong-return-type.0",
]);
...
const ownershipFixPlan = await execFileAsync(zero, [
  "fix",
  "--plan",
  "--json",
  "conformance/native/fail/owned-use-after-move.0",
]);
...
const abiFixPlan = await execFileAsync(zero, [
  "fix",
  "--plan",
  "--json",
  "conformance/check/fail/export-c-raises.0",
]);
...
const uncheckedFallibleJson = await execFileAsync(zero, [
  "check",
  "--json",
  "conformance/native/fail/unchecked-fallible-call.0",
]).catch((error) => error);
...
const errorSetMismatchJson = await execFileAsync(zero, [
  "check",
  "--json",
  "conformance/native/fail/error-set-mismatch.0",
]).catch((error) => error);
...
assert.match(
  errorSetMismatchBody.diagnostics[0].expected,
  /caller `!\[\.\.\.\]` set/,
);
assert.equal(errorSetMismatchBody.diagnostics[0].fixSafety, "api-changing");

const stdFsErrorMismatchJson = await execFileAsync(zero, [
  "check",
  "--json",
  "conformance/native/fail/std-fs-error-set-mismatch.0",
]).catch((error) => error);
...
assert.match(
  stdFsErrorMismatchBody.diagnostics[0].actual,
  /std fs call may raise/,
);

const stdFsCreateErrorMismatchJson = await execFileAsync(zero, [
  "check",
  "--json",
  "conformance/native/fail/std-fs-create-error-set-mismatch.0",
]).catch((error) => error);
assert.notEqual(stdFsCreateErrorMismatchJson.code, 0);
const stdFsCreateErrorMismatchBody = JSON.parse(
  stdFsCreateErrorMismatchJson.stdout,
);
...
const stdFsUncheckedResourceJson = await execFileAsync(zero, [
  "check",
  "--json",
  "conformance/native/fail/std-fs-unchecked-resource-fallible.0",
]).catch((error) => error);
assert.notEqual(stdFsUncheckedResourceJson.code, 0);
const stdFsUncheckedResourceBody = JSON.parse(
  stdFsUncheckedResourceJson.stdout,
);
assert.equal(stdFsUncheckedResourceBody.diagnostics[0].code, "ERR003");
assert.equal(
  stdFsUncheckedResourceBody.diagnostics[0].fixSafety,
  "api-changing",
);
assert.equal(
  stdFsUncheckedResourceBody.diagnostics[0].repair.id,
  "check-or-rescue-fallible-call",
);

const badManifestKindJson = await execFileAsync(zero, [
  "check",
  "--json",
  "conformance/check/fail/bad-manifest-kind",
]).catch((error) => error);
assert.notEqual(badManifestKindJson.code, 0);
assert.equal(
  JSON.parse(badManifestKindJson.stdout).diagnostics[0].code,
  "BLD002",
);

const missingMainJson = await execFileAsync(zero, [
  "check",
  "--json",
  "conformance/check/fail/missing-main-src",
]).catch((error) => error);
assert.notEqual(missingMainJson.code, 0);
assert.equal(JSON.parse(missingMainJson.stdout).diagnostics[0].code, "BLD002");

const readAllInvalidAllocJson = await execFileAsync(zero, [
  "check",
  "--json",
  "conformance/native/fail/fs-readall-invalid-alloc.0",
]).catch((error) => error);
...
assert.match(
  readAllInvalidAllocBody.diagnostics[0].message,
  /readAll expects an allocator/,
);

const zeroTestRun = await execFileAsync(zero, [
  "test",
  "conformance/native/pass/test-blocks.0",
]);
assert.equal(zeroTestRun.stdout, "1 test(s) ok\n");

const zeroTestJsonRun = await execFileAsync(zero, [
  "test",
  "--json",
  "--filter",
  "addition",
...
assert.equal(
  zeroTestJsonBody.fixtures.snapshotKey,
  "zero-test-direct-frontend-v1",
);
assert.equal(zeroTestJsonBody.results[0].status, "passed");

const zeroPackageTestJsonRun = await execFileAsync(zero, [
  "test",
  "--json",
  "conformance/packages/test-app",
]);
...
assert(
  zeroPackageTestBody.fixtures.sourceFiles.some((path) =>
    path.endsWith("helper.0"),
  ),
);

const zeroExpectedFailJsonRun = await execFileAsync(zero, [
  "test",
  "--json",
  "conformance/native/pass/test-expected-fail.0",
]);
...
const zeroUnexpectedPassJsonRun = await execFileAsync(zero, [
  "test",
  "--json",
  "conformance/native/fail/test-unexpected-pass.0",
]).catch((error) => error);
...
const zeroTestFailureRun = await execFileAsync(zero, [
  "test",
  "conformance/native/fail/test-expect-runtime-fail.0",
]).catch((error) => error);
...
const fmtRun = await execFileAsync(zero, [
  "fmt",
  "conformance/native/pass/test-blocks.0",
]);
assert.equal(
  fmtRun.stdout,
  await readFile("conformance/native/pass/test-blocks.0", "utf8"),
);

const fmtFallibilityRun = await execFileAsync(zero, [
  "fmt",
  "conformance/native/pass/fallibility-check-value.0",
]);
assert.equal(
  fmtFallibilityRun.stdout,
  await readFile("conformance/native/pass/fallibility-check-value.0", "utf8"),
);

const fmtImportsRun = await execFileAsync(zero, [
  "fmt",
  "conformance/check/pass/imports/src/main.0",
]);
assert.match(fmtImportsRun.stdout, /use math/);
assert.match(fmtImportsRun.stdout, /use types/);

const fmtCoreRun = await execFileAsync(zero, [
  "fmt",
  "conformance/check/pass/fmt-core-usability.0",
]);
assert.equal(
...
const fmtMessyRun = await execFileAsync(zero, [
  "fmt",
  "conformance/format/messy.0",
]);
assert.equal(
  fmtMessyRun.stdout,
  `pub fn main Void world World !
...
const fmtSyntaxLocalRun = await execFileAsync(zero, [
  "fmt",
  "conformance/format/syntax-local.0",
]);
assert.equal(
  fmtSyntaxLocalRun.stdout,
  `pub fn main Void
...
const fmtFunctionsBlocksRun = await execFileAsync(zero, [
  "fmt",
  "conformance/format/functions-blocks.0",
]);
assert.equal(
  fmtFunctionsBlocksRun.stdout,
  `fn helper i32 value i32
  if == value 0
    ret 1
  else
    ret value
...
const fmtDataTypesRun = await execFileAsync(zero, [
  "fmt",
  "conformance/format/data-types.0",
]);
assert.equal(
  fmtDataTypesRun.stdout,
  `type Point
  x i32
  y i32
...
const fmtGenericsStaticRun = await execFileAsync(zero, [
  "fmt",
  "conformance/format/generics-static.0",
]);
assert.equal(
  fmtGenericsStaticRun.stdout,
  `type Box<T: Type, static N: usize>
  items [N]T
...
const fmtMatchRun = await execFileAsync(zero, [
  "fmt",
  "conformance/format/match-expressions.0",
]);
assert.equal(
  fmtMatchRun.stdout,
  `choice Event
...
const helloRunArgs = runnableExeArgs(
  "conformance/run/pass/hello.0",
  `${outDir}/hello`,
);
if (helloRunArgs) {
...
const packageGraphJson = await execFileAsync(zero, [
  "graph",
  "--json",
  "conformance/check/pass/package",
]);
...
for (const runtimeFixture of [
  ...
  [
    ...
    { stdout: /payload binding ok/ },
  ],
  [
    "conformance/native/pass/match-fallback.0",
    "match-fallback",
    { stdout: "match fallback ok\n" },
  ],
  [
    "conformance/native/pass/match-choice-fallback.0",
    "match-choice-fallback",
    { stdout: "choice fallback ok\n" },
  ],
  [
    "conformance/native/pass/null-maybe.0",
    "null-maybe",
    { stdout: /null maybe ok/ },
  ],
  [
    "conformance/native/pass/std-args.0",
    "std-args",
  ...
  [
    "conformance/native/pass/integer-widths.0",
    "integer-widths",
    { stdout: "integer widths ok\n" },
  ],
  [
    "conformance/native/pass/std-codec-widths.0",
    "std-codec-widths",
    { stdout: "codec widths ok\n" },
  ],
  [
    "conformance/native/pass/std-crypto-hmac32.0",
    "std-crypto-hmac32",
    { stdout: "crypto hmac32 ok\n" },
  ],
  [
    "conformance/native/pass/parse-integers.0",
    "parse-integers",
    { stdout: "parse integers ok\n" },
  ],
  [
    "conformance/native/pass/explicit-casts.0",
    "explicit-casts",
    { stdout: "explicit casts ok\n" },
  ],
  [
    "conformance/native/pass/float-char-casts.0",
    "float-char-casts",
    { stdout: "float char casts ok\n" },
  ],
  [
    "conformance/native/pass/radix-suffix-literals.0",
    "radix-suffix-literals",
    { stdout: "radix suffix literals ok\n" },
  ],
  [
    "conformance/native/pass/char-literals.0",
    "char-literals",
    { stdout: "char literals ok\n" },
  ],
  [
    "conformance/native/pass/float-primitives.0",
    "float-primitives",
    { stdout: "float primitives ok\n" },
  ],
  [
    "conformance/native/pass/recursive-fibonacci.0",
    "recursive-fibonacci",
    { stdout: "recursive fibonacci ok\n" },
  ],
  [
    "conformance/native/pass/scratch-nested-index.0",
    "scratch-nested-index",
    { stdout: "scratch nested index ok\n" },
  ],
  [
    "conformance/native/pass/checked-bounds-get.0",
    "checked-bounds-get",
    { stdout: "checked bounds get ok\n" },
  ],
  [
    "conformance/native/pass/check-maybe-fallibility.0",
    "check-maybe-fallibility",
    { stdout: "check maybe fallibility ok\n" },
  ],
  [
    "conformance/native/pass/fallibility-error-sets.0",
    "fallibility-error-sets",
    { stdout: "fallibility error sets ok\n" },
  ],
  [
    "conformance/native/pass/rescue-check.0",
    "rescue-check",
  ...
  [
    "conformance/native/pass/std-fs-fallible-resources.0",
    "std-fs-fallible-resources",
    { stdout: "fs fallible resources ok\n" },
  ],
  [
    "conformance/native/pass/std-cli-helpers.0",
    "std-cli-helpers",
    { stdout: "cli helpers ok\n" },
  ],
  [
    "conformance/native/pass/std-mem-copy-fill.0",
    "std-mem-copy-fill",
    { stdout: "mem copy fill ok\n" },
  ],
  [
    "conformance/native/pass/const-layout.0",
    "const-layout",
    { stdout: "const layout ok\n" },
  ],
  [
    "conformance/native/pass/c-abi-export.0",
    "c-abi-export",
    { stdout: "c abi export ok\n" },
  ],
]) {
  await assertDirectRuntimeOrUnsupported(...runtimeFixture);
}

await assertDirectRuntimeRequired(
  "conformance/native/pass/generic-function-basic.0",
  "generic-function-basic-required",
  { stdout: "generic function ok\n" },
);
await assertDirectRuntimeRequired(
  "conformance/native/pass/generic-nested-calls.0",
  "generic-nested-calls-required",
  { stdout: "generic nested calls ok\n" },
);
await assertDirectRuntimeRequired(
  "conformance/native/pass/generic-inferred-specialized-call.0",
  "generic-inferred-specialized-call-required",
  { stdout: "generic inferred specialized call ok\n" },
);
await assertDirectRuntimeRequired(
  "conformance/native/pass/generic-nested-local-specialization.0",
  "generic-nested-local-specialization-required",
  { stdout: "generic nested local specialization ok\n" },
);
await assertDirectRuntimeRequired(
  "conformance/native/pass/generic-static-array-specialization.0",
  "generic-static-array-specialization-required",
  { stdout: "generic static array specialization ok\n" },
);
await assertDirectRuntimeRequired(
  "conformance/native/pass/generic-static-forwarded-array-specialization.0",
  "generic-static-forwarded-array-specialization-required",
  { stdout: "generic static forwarded array specialization ok\n" },
);
await assertDirectRuntimeRequired(
  "conformance/native/pass/explicit-cast-narrow-direct.0",
  "explicit-cast-narrow-direct-required",
  { stdout: "explicit cast narrow direct ok\n" },
);

const abiDump = await execFileAsync(zero, [
  "abi",
  "dump",
  "--json",
  "conformance/native/pass/const-layout.0",
...
assert(
  abiDumpBody.externShapes.some(
    (item) =>
      item.name === "CPoint" && item.size === 8 && item.fields[1].offset === 4,
  ),
);
assert(
  abiDumpBody.primitiveLayouts.some(
    (item) => item.name === "usize" && item.size === 8,
  ),
);
const cAbiDump = await execFileAsync(zero, [
  "abi",
  "dump",
  "--json",
  "conformance/native/pass/c-abi-export.0",
...
assert(
  cAbiDumpBody.cExports.some(
    (item) => item.name === "zero_add" && item.cReturnType === "int32_t",
  ),
);
assert.equal(cAbiDumpBody.generatedHeader.available, true);
assert.match(
  cAbiDumpBody.generatedHeader.text,
  /int32_t zero_add\(int32_t a, int32_t b\);/,
);
const abiCheck = await execFileAsync(zero, [
  "abi",
  "check",
  "--json",
  "conformance/native/pass/const-layout.0",
...
for (const runtimeFixture of [
  [
    "conformance/native/pass/indexing-primitives.0",
    "indexing-primitives",
    { stdout: "indexing primitives ok\n" },
  ],
  [
    "conformance/native/pass/range-slices.0",
    "range-slices",
    { stdout: "range slices ok\n" },
  ],
  [
    "conformance/native/pass/generic-spans.0",
    "generic-spans",
    { stdout: "generic spans ok\n" },
  ],
  [
    "conformance/native/pass/open-ended-slices.0",
    "open-ended-slices",
    { stdout: "open ended slices ok\n" },
  ],
  [
    "conformance/native/pass/string-slices.0",
    "string-slices",
    { stdout: "string slices ok\n" },
  ],
  [
    "conformance/native/pass/indexed-mutation.0",
    "indexed-mutation",
    { stdout: "indexed mutation ok\n" },
  ],
  [
    "conformance/native/pass/nested-lvalues.0",
    "nested-lvalues",
    { stdout: "nested lvalues ok\n" },
  ],
  [
    "conformance/native/pass/mutable-spans.0",
    "mutable-spans",
    { stdout: "mutable spans ok\n" },
  ],
  [
    "conformance/native/pass/mutref-indexed-lvalues.0",
    "mutref-indexed-lvalues",
    { stdout: "mutref indexed lvalues ok\n" },
  ],
  [
    "conformance/native/pass/generic-mem.0",
    "generic-mem",
    { stdout: "generic mem ok\n" },
  ],
  [
    "conformance/native/pass/generic-nested-calls.0",
    "generic-nested-calls",
    { stdout: "generic nested calls ok\n" },
  ],
  [
    "conformance/native/pass/generic-multi-specialization.0",
    "generic-multi-specialization",
    { stdout: "generic multi specialization ok\n" },
  ],
  [
    "conformance/native/pass/generic-inferred-specialized-call.0",
    "generic-inferred-specialized-call",
    { stdout: "generic inferred specialized call ok\n" },
  ],
  [
    "conformance/native/pass/generic-nested-local-specialization.0",
    "generic-nested-local-specialization",
    { stdout: "generic nested local specialization ok\n" },
  ],
  [
    "conformance/native/pass/generic-static-array-specialization.0",
    "generic-static-array-specialization",
    { stdout: "generic static array specialization ok\n" },
  ],
  [
    "conformance/native/pass/generic-static-forwarded-array-specialization.0",
    "generic-static-forwarded-array-specialization",
    { stdout: "generic static forwarded array specialization ok\n" },
  ],
  [
    "conformance/native/pass/static-interface-mutref.0",
    "static-interface-mutref",
    { stdout: "static interface mutref ok\n" },
  ],
  [
    "conformance/native/pass/owned-transfer.0",
    "owned-transfer",
    { stdout: "owned transfer ok\n" },
  ],
  [
    "conformance/native/pass/owned-drop-cleanup.0",
    "owned-drop-cleanup",
    { stdout: "owned drop cleanup ok\n" },
  ],
  [
    "conformance/native/pass/owned-drop-move-suppressed.0",
    "owned-drop-move-suppressed",
    { stdout: "owned drop move suppressed ok\n" },
  ],
  [
    "conformance/native/pass/borrow-primitives.0",
    "borrow-primitives",
    { stdout: "borrow primitives ok\n" },
  ],
  [
    "conformance/native/pass/allocator-primitives.0",
    "allocator-primitives",
    { stdout: "allocator primitives ok\n" },
  ],
  [
    "conformance/native/pass/owned-byte-buffer.0",
    "owned-byte-buffer",
    { stdout: "owned byte buffer ok\n" },
  ],
]) {
  await assertDirectRuntimeOrUnsupported(...runtimeFixture);
}

await assertBoundsTrap(
  "conformance/native/fail/bounds-array-index.0",
  "bounds-array-index",
);
await assertBoundsTrap(
  "conformance/native/fail/bounds-span-index.0",
  "bounds-span-index",
);
await assertBoundsTrap(
  "conformance/native/fail/bounds-slice-end.0",
  "bounds-slice-end",
);
await assertBoundsTrap(
  "conformance/native/fail/bounds-slice-order.0",
  "bounds-slice-order",
);
await assertBoundsTrap(
  "conformance/native/fail/bounds-open-slice-start.0",
  "bounds-open-slice-start",
);
await assertBoundsTrap(
  "conformance/native/fail/index-string.0",
  "index-string",
);
await assertBoundsTrap(
  "conformance/native/fail/slice-string.0",
  "slice-string",
);
await assertBoundsTrap(
  "conformance/native/fail/indexed-mutation-oob.0",
  "indexed-mutation-oob",
);

const failed = await execFileAsync(zero, [
  "check",
  "conformance/check/fail/unknown-name.0",
]).catch((error) => error);
...
const unknownField = await execFileAsync(zero, [
  "check",
  "conformance/native/fail/unknown-field.0",
]).catch((error) => error);
assert.notEqual(unknownField.code, 0);
assert.match(unknownField.stderr, /FLD001/);

const wrongReturnType = await execFileAsync(zero, [
  "check",
  "conformance/native/fail/bad-return.0",
]).catch((error) => error);
assert.notEqual(wrongReturnType.code, 0);
assert.match(wrongReturnType.stderr, /TYP003/);

const immutableAssignment = await execFileAsync(zero, [
  "check",
  "conformance/native/fail/immutable-assignment.0",
]).catch((error) => error);
assert.notEqual(immutableAssignment.code, 0);
assert.match(immutableAssignment.stderr, /TYP009/);

const indexedMutationImmutable = await execFileAsync(zero, [
  "check",
  "conformance/native/fail/indexed-mutation-immutable.0",
]).catch((error) => error);
assert.notEqual(indexedMutationImmutable.code, 0);
assert.match(indexedMutationImmutable.stderr, /TYP009/);

const indexedMutationType = await execFileAsync(zero, [
  "check",
  "conformance/native/fail/indexed-mutation-type.0",
]).catch((error) => error);
assert.notEqual(indexedMutationType.code, 0);
assert.match(indexedMutationType.stderr, /TYP002/);

const indexedMutationNonInteger = await execFileAsync(zero, [
  "check",
  "conformance/native/fail/indexed-mutation-non-integer.0",
]).catch((error) => error);
assert.notEqual(indexedMutationNonInteger.code, 0);
assert.match(indexedMutationNonInteger.stderr, /TYP022/);

const indexedMutationSpan = await execFileAsync(zero, [
  "check",
  "conformance/native/fail/indexed-mutation-span.0",
]).catch((error) => error);
assert.notEqual(indexedMutationSpan.code, 0);
assert.match(indexedMutationSpan.stderr, /TYP021/);

const indexedMutationString = await execFileAsync(zero, [
  "check",
  "conformance/native/fail/indexed-mutation-string.0",
]).catch((error) => error);
...
const nestedLvalueImmutableField = await execFileAsync(zero, [
  "check",
  "conformance/native/fail/nested-lvalue-immutable-field.0",
]).catch((error) => error);
assert.notEqual(nestedLvalueImmutableField.code, 0);
assert.match(nestedLvalueImmutableField.stderr, /TYP009/);

const nestedLvalueFieldType = await execFileAsync(zero, [
  "check",
  "conformance/native/fail/nested-lvalue-field-type.0",
]).catch((error) => error);
assert.notEqual(nestedLvalueFieldType.code, 0);
assert.match(nestedLvalueFieldType.stderr, /TYP002/);

const nestedLvalueUnknownField = await execFileAsync(zero, [
  "check",
  "conformance/native/fail/nested-lvalue-unknown-field.0",
]).catch((error) => error);
assert.notEqual(nestedLvalueUnknownField.code, 0);
assert.match(nestedLvalueUnknownField.stderr, /FLD001/);

const nestedLvalueNonIntegerIndex = await execFileAsync(zero, [
  "check",
  "conformance/native/fail/nested-lvalue-non-integer-index.0",
]).catch((error) => error);
assert.notEqual(nestedLvalueNonIntegerIndex.code, 0);
assert.match(nestedLvalueNonIntegerIndex.stderr, /TYP022/);

const nestedLvalueSpanIndex = await execFileAsync(zero, [
  "check",
  "conformance/native/fail/nested-lvalue-span-index.0",
]).catch((error) => error);
assert.notEqual(nestedLvalueSpanIndex.code, 0);
assert.match(nestedLvalueSpanIndex.stderr, /TYP021/);

const nestedLvalueStringIndex = await execFileAsync(zero, [
  "check",
  "conformance/native/fail/nested-lvalue-string-index.0",
]).catch((error) => error);
...
const mutspanImmutableArray = await execFileAsync(zero, [
  "check",
  "conformance/native/fail/mutspan-immutable-array.0",
]).catch((error) => error);
assert.notEqual(mutspanImmutableArray.code, 0);
assert.match(mutspanImmutableArray.stderr, /TYP009/);

const memCopyImmutableDst = await execFileAsync(zero, [
  "check",
  "conformance/native/fail/mem-copy-immutable-dst.0",
]).catch((error) => error);
...
const memCopyImmutableDstJson = await execFileAsync(zero, [
  "check",
  "--json",
  "conformance/native/fail/mem-copy-immutable-dst.0",
]).catch((error) => error);
...
assert.equal(
  memCopyImmutableDstBody.diagnostics[0].repair.id,
  "make-binding-mutable",
);

const mutspanFromSpan = await execFileAsync(zero, [
  "check",
  "conformance/native/fail/mutspan-from-span.0",
]).catch((error) => error);
assert.notEqual(mutspanFromSpan.code, 0);
assert.match(mutspanFromSpan.stderr, /TYP002/);

const mutspanFromString = await execFileAsync(zero, [
  "check",
  "conformance/native/fail/mutspan-from-string.0",
]).catch((error) => error);
assert.notEqual(mutspanFromString.code, 0);
assert.match(mutspanFromString.stderr, /TYP002/);

const mutspanReadonlyIndex = await execFileAsync(zero, [
  "check",
  "conformance/native/fail/mutspan-readonly-index.0",
]).catch((error) => error);
assert.notEqual(mutspanReadonlyIndex.code, 0);
assert.match(mutspanReadonlyIndex.stderr, /TYP021/);

const mutspanAssignmentType = await execFileAsync(zero, [
  "check",
  "conformance/native/fail/mutspan-assignment-type.0",
]).catch((error) => error);
assert.notEqual(mutspanAssignmentType.code, 0);
assert.match(mutspanAssignmentType.stderr, /TYP002/);

const mutspanNonIntegerIndex = await execFileAsync(zero, [
  "check",
  "conformance/native/fail/mutspan-non-integer-index.0",
]).catch((error) => error);
assert.notEqual(mutspanNonIntegerIndex.code, 0);
assert.match(mutspanNonIntegerIndex.stderr, /TYP022/);

const duplicateFunction = await execFileAsync(zero, [
  "check",
  "conformance/native/fail/duplicate-function.0",
]).catch((error) => error);
assert.notEqual(duplicateFunction.code, 0);
assert.match(duplicateFunction.stderr, /NAM004/);

const wrongArity = await execFileAsync(zero, [
  "check",
  "conformance/native/fail/wrong-arity.0",
]).catch((error) => error);
assert.notEqual(wrongArity.code, 0);
assert.match(wrongArity.stderr, /NAM004/);

const unknownEnumCase = await execFileAsync(zero, [
  "check",
  "conformance/check/fail/unknown-enum-case.0",
]).catch((error) => error);
assert.notEqual(unknownEnumCase.code, 0);
assert.match(unknownEnumCase.stderr, /VAR001/);

const badStdCall = await execFileAsync(zero, [
  "check",
  "conformance/native/fail/bad-std-call.0",
]).catch((error) => error);
assert.notEqual(badStdCall.code, 0);
assert.match(badStdCall.stderr, /STD002/);

const stdHttpErrorRawInt = await execFileAsync(zero, [
  "check",
  "conformance/native/fail/std-http-error-raw-int.0",
]).catch((error) => error);
assert.notEqual(stdHttpErrorRawInt.code, 0);
assert.match(stdHttpErrorRawInt.stderr, /TYP002/);

const stdHttpFetchRawTimeout = await execFileAsync(zero, [
  "check",
  "conformance/native/fail/std-http-fetch-raw-timeout.0",
]).catch((error) => error);
assert.notEqual(stdHttpFetchRawTimeout.code, 0);
assert.match(stdHttpFetchRawTimeout.stderr, /STD003/);

const stdJsonParseBytesRawAlloc = await execFileAsync(zero, [
  "check",
  "conformance/native/fail/std-json-parsebytes-raw-alloc.0",
]).catch((error) => error);
assert.notEqual(stdJsonParseBytesRawAlloc.code, 0);
assert.match(stdJsonParseBytesRawAlloc.stderr, /STD003/);

const stdJsonParseBytesImmutableAlloc = await execFileAsync(zero, [
  "check",
  "conformance/native/fail/std-json-parsebytes-immutable-alloc.0",
]).catch((error) => error);
assert.notEqual(stdJsonParseBytesImmutableAlloc.code, 0);
assert.match(stdJsonParseBytesImmutableAlloc.stderr, /STD003/);

const genericMemLenNonSpan = await execFileAsync(zero, [
  "check",
  "conformance/native/fail/generic-mem-len-non-span.0",
]).catch((error) => error);
assert.notEqual(genericMemLenNonSpan.code, 0);
assert.match(genericMemLenNonSpan.stderr, /STD003/);

const genericMemEqlMismatch = await execFileAsync(zero, [
  "check",
  "conformance/native/fail/generic-mem-eql-mismatch.0",
]).catch((error) => error);
assert.notEqual(genericMemEqlMismatch.code, 0);
assert.match(genericMemEqlMismatch.stderr, /STD003/);

const genericMemEqlNonSpan = await execFileAsync(zero, [
  "check",
  "conformance/native/fail/generic-mem-eql-non-span.0",
]).catch((error) => error);
assert.notEqual(genericMemEqlNonSpan.code, 0);
assert.match(genericMemEqlNonSpan.stderr, /STD003/);

const badMemoryType = await execFileAsync(zero, [
  "check",
  "conformance/native/fail/bad-memory-type.0",
]).catch((error) => error);
assert.notEqual(badMemoryType.code, 0);
assert.match(badMemoryType.stderr, /MEM001/);

const checkerArrayElementType = await execFileAsync(zero, [
  "check",
  "conformance/check/fail/checker-array-element-type.0",
]).catch((error) => error);
assert.notEqual(checkerArrayElementType.code, 0);
assert.match(checkerArrayElementType.stderr, /TYP002/);

const checkerRefMismatch = await execFileAsync(zero, [
  "check",
  "conformance/check/fail/checker-ref-mismatch.0",
]).catch((error) => error);
assert.notEqual(checkerRefMismatch.code, 0);
assert.match(checkerRefMismatch.stderr, /TYP001/);

const checkerMutrefImmutable = await execFileAsync(zero, [
  "check",
  "conformance/check/fail/checker-mutref-immutable.0",
]).catch((error) => error);
assert.notEqual(checkerMutrefImmutable.code, 0);
assert.match(checkerMutrefImmutable.stderr, /TYP009/);

const nonexhaustiveMatch = await execFileAsync(zero, [
  "check",
  "conformance/native/fail/nonexhaustive-match.0",
]).catch((error) => error);
assert.notEqual(nonexhaustiveMatch.code, 0);
assert.match(nonexhaustiveMatch.stderr, /MAT002/);

const badChoicePayload = await execFileAsync(zero, [
  "check",
  "conformance/native/fail/bad-choice-payload.0",
]).catch((error) => error);
assert.notEqual(badChoicePayload.code, 0);
assert.match(badChoicePayload.stderr, /VAR004/);

const allocatorInvalid = await execFileAsync(zero, [
  "check",
  "conformance/native/fail/allocator-invalid.0",
]).catch((error) => error);
assert.notEqual(allocatorInvalid.code, 0);
assert.match(allocatorInvalid.stderr, /STD003/);

const allocatorImmutableFixedBuf = await execFileAsync(zero, [
  "check",
  "conformance/native/fail/allocator-immutable-fixedbuf.0",
]).catch((error) => error);
assert.notEqual(allocatorImmutableFixedBuf.code, 0);
assert.match(allocatorImmutableFixedBuf.stderr, /STD003/);

const byteBufferImmutableAlloc = await execFileAsync(zero, [
  "check",
  "conformance/native/fail/byte-buffer-immutable-alloc.0",
]).catch((error) => error);
assert.notEqual(byteBufferImmutableAlloc.code, 0);
assert.match(byteBufferImmutableAlloc.stderr, /STD003/);

const ownedUseAfterMove = await execFileAsync(zero, [
  "check",
  "conformance/native/fail/owned-use-after-move.0",
]).catch((error) => error);
...
const unsupportedDrop = await execFileAsync(zero, [
  "check",
  "conformance/native/fail/unsupported-drop.0",
]).catch((error) => error);
assert.notEqual(unsupportedDrop.code, 0);
assert.match(unsupportedDrop.stderr, /OWN002/);

const invalidDropSignature = await execFileAsync(zero, [
  "check",
  "conformance/native/fail/invalid-drop-signature.0",
]).catch((error) => error);
assert.notEqual(invalidDropSignature.code, 0);
assert.match(invalidDropSignature.stderr, /OWN002/);

const borrowMutrefImmutable = await execFileAsync(zero, [
  "check",
  "conformance/native/fail/borrow-mutref-immutable.0",
]).catch((error) => error);
assert.notEqual(borrowMutrefImmutable.code, 0);
assert.match(borrowMutrefImmutable.stderr, /TYP009/);

const borrowConflict = await execFileAsync(zero, [
  "check",
  "conformance/native/fail/borrow-conflict.0",
]).catch((error) => error);
assert.notEqual(borrowConflict.code, 0);
assert.match(borrowConflict.stderr, /BOR001/);

const borrowAssignWhileBorrowed = await execFileAsync(zero, [
  "check",
  "conformance/native/fail/borrow-assign-while-borrowed.0",
]).catch((error) => error);
assert.notEqual(borrowAssignWhileBorrowed.code, 0);
assert.match(borrowAssignWhileBorrowed.stderr, /BOR001/);

const borrowAssignThroughRef = await execFileAsync(zero, [
  "check",
  "conformance/native/fail/borrow-assign-through-ref.0",
]).catch((error) => error);
assert.notEqual(borrowAssignThroughRef.code, 0);
assert.match(borrowAssignThroughRef.stderr, /TYP009/);

const borrowReturnLocal = await execFileAsync(zero, [
  "check",
  "conformance/native/fail/borrow-return-local.0",
]).catch((error) => error);
assert.notEqual(borrowReturnLocal.code, 0);
assert.match(borrowReturnLocal.stderr, /BOR002/);

const borrowWrongType = await execFileAsync(zero, [
  "check",
  "conformance/native/fail/borrow-wrong-type.0",
]).catch((error) => error);
assert.notEqual(borrowWrongType.code, 0);
assert.match(borrowWrongType.stderr, /TYP001/);

const refIndexedAssignment = await execFileAsync(zero, [
  "check",
  "conformance/native/fail/ref-indexed-assignment.0",
]).catch((error) => error);
assert.notEqual(refIndexedAssignment.code, 0);
assert.match(refIndexedAssignment.stderr, /TYP009/);

const breakOutsideLoop = await execFileAsync(zero, [
  "check",
  "conformance/native/fail/break-outside-loop.0",
]).catch((error) => error);
assert.notEqual(breakOutsideLoop.code, 0);
assert.match(breakOutsideLoop.stderr, /TYP012/);

const continueOutsideLoop = await execFileAsync(zero, [
  "check",
  "conformance/native/fail/continue-outside-loop.0",
]).catch((error) => error);
assert.notEqual(continueOutsideLoop.code, 0);
assert.match(continueOutsideLoop.stderr, /TYP013/);

const nonBoolCondition = await execFileAsync(zero, [
  "check",
  "conformance/native/fail/non-bool-condition.0",
]).catch((error) => error);
assert.notEqual(nonBoolCondition.code, 0);
assert.match(nonBoolCondition.stderr, /TYP010/);

const badMatchPayloadBinding = await execFileAsync(zero, [
  "check",
  "conformance/native/fail/bad-match-payload-binding.0",
]).catch((error) => error);
assert.notEqual(badMatchPayloadBinding.code, 0);
assert.match(badMatchPayloadBinding.stderr, /MAT004/);

const badNull = await execFileAsync(zero, [
  "check",
  "conformance/native/fail/bad-null.0",
]).catch((error) => error);
assert.notEqual(badNull.code, 0);
assert.match(badNull.stderr, /TYP011/);

const badForRange = await execFileAsync(zero, [
  "check",
  "conformance/native/fail/bad-for-range.0",
]).catch((error) => error);
assert.notEqual(badForRange.code, 0);
assert.match(badForRange.stderr, /TYP014/);

const badStdArgs = await execFileAsync(zero, [
  "check",
  "conformance/native/fail/bad-std-args.0",
]).catch((error) => error);
assert.notEqual(badStdArgs.code, 0);
assert.match(badStdArgs.stderr, /STD002/);

const badStdFs = await execFileAsync(zero, [
  "check",
  "conformance/native/fail/bad-std-fs.0",
]).catch((error) => error);
assert.notEqual(badStdFs.code, 0);
assert.match(badStdFs.stderr, /STD003/);

const fsOpenWithoutCapability = await execFileAsync(zero, [
  "check",
  "conformance/native/fail/fs-open-without-capability.0",
]).catch((error) => error);
assert.notEqual(fsOpenWithoutCapability.code, 0);
assert.match(fsOpenWithoutCapability.stderr, /STD003/);

const fsReadWithoutMutref = await execFileAsync(zero, [
  "check",
  "conformance/native/fail/fs-read-without-mutref.0",
]).catch((error) => error);
assert.notEqual(fsReadWithoutMutref.code, 0);
assert.match(fsReadWithoutMutref.stderr, /STD003/);

const unknownStdHelper = await execFileAsync(zero, [
  "check",
  "conformance/native/fail/unknown-std-helper.0",
]).catch((error) => error);
assert.notEqual(unknownStdHelper.code, 0);
assert.match(unknownStdHelper.stderr, /STD002/);

const integerU8Overflow = await execFileAsync(zero, [
  "check",
  "conformance/native/fail/integer-u8-overflow.0",
]).catch((error) => error);
assert.notEqual(integerU8Overflow.code, 0);
assert.match(integerU8Overflow.stderr, /TYP016/);

const integerI8Overflow = await execFileAsync(zero, [
  "check",
  "conformance/native/fail/integer-i8-overflow.0",
]).catch((error) => error);
assert.notEqual(integerI8Overflow.code, 0);
assert.match(integerI8Overflow.stderr, /TYP016/);

const integerU64Overflow = await execFileAsync(zero, [
  "check",
  "conformance/native/fail/integer-u64-overflow.0",
]).catch((error) => error);
assert.notEqual(integerU64Overflow.code, 0);
assert.match(integerU64Overflow.stderr, /TYP016/);

const malformedIntegerLiteral = await execFileAsync(zero, [
  "check",
  "conformance/native/fail/malformed-integer-literal.0",
]).catch((error) => error);
assert.notEqual(malformedIntegerLiteral.code, 0);
assert.match(malformedIntegerLiteral.stderr, /TYP015/);

const integerNonliteralNarrowing = await execFileAsync(zero, [
  "check",
  "conformance/native/fail/integer-nonliteral-narrowing.0",
]).catch((error) => error);
assert.notEqual(integerNonliteralNarrowing.code, 0);
assert.match(integerNonliteralNarrowing.stderr, /TYP002/);

const stdCodecReadU16Width = await execFileAsync(zero, [
  "check",
  "conformance/native/fail/std-codec-read-u16-width.0",
]).catch((error) => error);
assert.notEqual(stdCodecReadU16Width.code, 0);
assert.match(stdCodecReadU16Width.stderr, /TYP002/);

const castStringToInt = await execFileAsync(zero, [
  "check",
  "conformance/native/fail/cast-string-to-int.0",
]).catch((error) => error);
assert.notEqual(castStringToInt.code, 0);
assert.match(castStringToInt.stderr, /TYP017/);

const castBoolToInt = await execFileAsync(zero, [
  "check",
  "conformance/native/fail/cast-bool-to-int.0",
]).catch((error) => error);
...
const castIntToSpan = await execFileAsync(zero, [
  "check",
  "conformance/native/fail/cast-int-to-span.0",
]).catch((error) => error);
assert.notEqual(castIntToSpan.code, 0);
assert.match(castIntToSpan.stderr, /TYP017/);

const castIntToString = await execFileAsync(zero, [
  "check",
  "conformance/native/fail/cast-int-to-string.0",
]).catch((error) => error);
assert.notEqual(castIntToString.code, 0);
assert.match(castIntToString.stderr, /TYP017/);

const radixInvalidBinary = await execFileAsync(zero, [
  "check",
  "conformance/native/fail/radix-invalid-binary.0",
]).catch((error) => error);
assert.notEqual(radixInvalidBinary.code, 0);
assert.match(radixInvalidBinary.stderr, /TYP015/);

const radixEmptyHex = await execFileAsync(zero, [
  "check",
  "conformance/native/fail/radix-empty-hex.0",
]).catch((error) => error);
assert.notEqual(radixEmptyHex.code, 0);
assert.match(radixEmptyHex.stderr, /TYP015/);

const literalBadSeparator = await execFileAsync(zero, [
  "check",
  "conformance/native/fail/literal-bad-separator.0",
]).catch((error) => error);
assert.notEqual(literalBadSeparator.code, 0);
assert.match(literalBadSeparator.stderr, /TYP015/);

const literalTrailingSeparator = await execFileAsync(zero, [
  "check",
  "conformance/native/fail/literal-trailing-separator.0",
]).catch((error) => error);
assert.notEqual(literalTrailingSeparator.code, 0);
assert.match(literalTrailingSeparator.stderr, /TYP015/);

const literalUnknownSuffix = await execFileAsync(zero, [
  "check",
  "conformance/native/fail/literal-unknown-suffix.0",
]).catch((error) => error);
assert.notEqual(literalUnknownSuffix.code, 0);
assert.match(literalUnknownSuffix.stderr, /TYP015/);

const literalSuffixMismatch = await execFileAsync(zero, [
  "check",
  "conformance/native/fail/literal-suffix-mismatch.0",
]).catch((error) => error);
assert.notEqual(literalSuffixMismatch.code, 0);
assert.match(literalSuffixMismatch.stderr, /TYP002/);

const literalSuffixOverflow = await execFileAsync(zero, [
  "check",
  "conformance/native/fail/literal-suffix-overflow.0",
]).catch((error) => error);
assert.notEqual(literalSuffixOverflow.code, 0);
assert.match(literalSuffixOverflow.stderr, /TYP016/);

const charEmpty = await execFileAsync(zero, [
  "check",
  "conformance/native/fail/char-empty.0",
]).catch((error) => error);
assert.notEqual(charEmpty.code, 0);
assert.match(charEmpty.stderr, /PAR100/);

const charMultiple = await execFileAsync(zero, [
  "check",
  "conformance/native/fail/char-multiple.0",
]).catch((error) => error);
assert.notEqual(charMultiple.code, 0);
assert.match(charMultiple.stderr, /PAR100/);

const charBadEscape = await execFileAsync(zero, [
  "check",
  "conformance/native/fail/char-bad-escape.0",
]).catch((error) => error);
assert.notEqual(charBadEscape.code, 0);
assert.match(charBadEscape.stderr, /PAR100/);

const charToString = await execFileAsync(zero, [
  "check",
  "conformance/native/fail/char-to-string.0",
]).catch((error) => error);
assert.notEqual(charToString.code, 0);
assert.match(charToString.stderr, /TYP002/);

const stringToChar = await execFileAsync(zero, [
  "check",
  "conformance/native/fail/string-to-char.0",
]).catch((error) => error);
assert.notEqual(stringToChar.code, 0);
assert.match(stringToChar.stderr, /TYP002/);

const charIntegerArithmetic = await execFileAsync(zero, [
  "check",
  "conformance/native/fail/char-integer-arithmetic.0",
]).catch((error) => error);
assert.notEqual(charIntegerArithmetic.code, 0);
assert.match(charIntegerArithmetic.stderr, /TYP002/);

const malformedFloatLiteral = await execFileAsync(zero, [
  "check",
  "conformance/native/fail/malformed-float-literal.0",
]).catch((error) => error);
assert.notEqual(malformedFloatLiteral.code, 0);
assert.match(malformedFloatLiteral.stderr, /TYP019/);

const floatF32Overflow = await execFileAsync(zero, [
  "check",
  "conformance/native/fail/float-f32-overflow.0",
]).catch((error) => error);
assert.notEqual(floatF32Overflow.code, 0);
assert.match(floatF32Overflow.stderr, /TYP020/);

const integerFromFloat = await execFileAsync(zero, [
  "check",
  "conformance/native/fail/integer-from-float.0",
]).catch((error) => error);
assert.notEqual(integerFromFloat.code, 0);
assert.match(integerFromFloat.stderr, /TYP002/);

const floatFromInteger = await execFileAsync(zero, [
  "check",
  "conformance/native/fail/float-from-integer.0",
]).catch((error) => error);
assert.notEqual(floatFromInteger.code, 0);
assert.match(floatFromInteger.stderr, /TYP002/);

const floatMixedWidth = await execFileAsync(zero, [
  "check",
  "conformance/native/fail/float-mixed-width.0",
]).catch((error) => error);
assert.notEqual(floatMixedWidth.code, 0);
assert.match(floatMixedWidth.stderr, /TYP002/);

const floatIntArithmetic = await execFileAsync(zero, [
  "check",
  "conformance/native/fail/float-int-arithmetic.0",
]).catch((error) => error);
assert.notEqual(floatIntArithmetic.code, 0);
assert.match(floatIntArithmetic.stderr, /TYP002/);

const indexNonInteger = await execFileAsync(zero, [
  "check",
  "conformance/native/fail/index-non-integer.0",
]).catch((error) => error);
assert.notEqual(indexNonInteger.code, 0);
assert.match(indexNonInteger.stderr, /TYP022/);

const indexNonIndexable = await execFileAsync(zero, [
  "check",
  "conformance/native/fail/index-non-indexable.0",
]).catch((error) => error);
assert.notEqual(indexNonIndexable.code, 0);
assert.match(indexNonIndexable.stderr, /TYP021/);

const checkedGetNonIndexable = await execFileAsync(zero, [
  "check",
  "conformance/native/fail/checked-get-non-indexable.0",
]).catch((error) => error);
assert.notEqual(checkedGetNonIndexable.code, 0);
assert.match(checkedGetNonIndexable.stderr, /STD003/);

const checkMaybeVoid = await execFileAsync(zero, [
  "check",
  "conformance/native/fail/check-maybe-void.0",
]).catch((error) => error);
assert.notEqual(checkMaybeVoid.code, 0);
assert.match(checkMaybeVoid.stderr, /ERR001/);

const raiseWithoutRaises = await execFileAsync(zero, [
  "check",
  "conformance/native/fail/raise-without-raises.0",
]).catch((error) => error);
assert.notEqual(raiseWithoutRaises.code, 0);
assert.match(raiseWithoutRaises.stderr, /ERR001/);

const raiseUndeclaredError = await execFileAsync(zero, [
  "check",
  "conformance/native/fail/raise-undeclared-error.0",
]).catch((error) => error);
assert.notEqual(raiseUndeclaredError.code, 0);
assert.match(raiseUndeclaredError.stderr, /ERR002/);

const uncheckedFallibleCall = await execFileAsync(zero, [
  "check",
  "conformance/native/fail/unchecked-fallible-call.0",
]).catch((error) => error);
...
const errorSetMismatch = await execFileAsync(zero, [
  "check",
  "conformance/native/fail/error-set-mismatch.0",
]).catch((error) => error);
...
const rescueFallbackTypeMismatch = await execFileAsync(zero, [
  "check",
  "conformance/native/fail/rescue-fallback-type-mismatch.0",
]).catch((error) => error);
assert.notEqual(rescueFallbackTypeMismatch.code, 0);
assert.match(rescueFallbackTypeMismatch.stderr, /TYP003|TYP002/);

const constFieldAssignment = await execFileAsync(zero, [
  "check",
  "conformance/native/fail/const-field-assignment.0",
]).catch((error) => error);
assert.notEqual(constFieldAssignment.code, 0);
assert.match(constFieldAssignment.stderr, /TYP009/);

const constMutBorrow = await execFileAsync(zero, [
  "check",
  "conformance/native/fail/const-mut-borrow.0",
]).catch((error) => error);
assert.notEqual(constMutBorrow.code, 0);
assert.match(constMutBorrow.stderr, /TYP009/);

const badExternLayout = await execFileAsync(zero, [
  "check",
  "conformance/native/fail/bad-extern-layout.0",
]).catch((error) => error);
assert.notEqual(badExternLayout.code, 0);
assert.match(badExternLayout.stderr, /ABI001/);

const badPackedLayout = await execFileAsync(zero, [
  "check",
  "conformance/native/fail/bad-packed-layout.0",
]).catch((error) => error);
assert.notEqual(badPackedLayout.code, 0);
assert.match(badPackedLayout.stderr, /ABI001/);

const badCExport = await execFileAsync(zero, [
  "check",
  "conformance/native/fail/bad-c-export.0",
]).catch((error) => error);
assert.notEqual(badCExport.code, 0);
assert.match(badCExport.stderr, /ABI001/);

const badCExportRaises = await execFileAsync(zero, [
  "check",
  "conformance/native/fail/bad-c-export-raises.0",
]).catch((error) => error);
assert.notEqual(badCExportRaises.code, 0);
assert.match(badCExportRaises.stderr, /ABI001/);

const byteBufferUseAfterMove = await execFileAsync(zero, [
  "check",
  "conformance/native/fail/byte-buffer-use-after-move.0",
]).catch((error) => error);
assert.notEqual(byteBufferUseAfterMove.code, 0);
assert.match(byteBufferUseAfterMove.stderr, /OWN001/);

const testExpectNonBool = await execFileAsync(zero, [
  "check",
  "conformance/native/fail/test-expect-non-bool.0",
]).catch((error) => error);
assert.notEqual(testExpectNonBool.code, 0);
assert.match(testExpectNonBool.stderr, /TYP001/);

const sliceNonIntegerBound = await execFileAsync(zero, [
  "check",
  "conformance/native/fail/slice-non-integer-bound.0",
]).catch((error) => error);
assert.notEqual(sliceNonIntegerBound.code, 0);
assert.match(sliceNonIntegerBound.stderr, /TYP022/);

const sliceNonSliceable = await execFileAsync(zero, [
  "check",
  "conformance/native/fail/slice-non-sliceable.0",
]).catch((error) => error);
...
for (const [fixture, code] of [
  ...
  const result = await execFileAsync(zero, [
    "check",
    `conformance/native/fail/${fixture}`,
  ]).catch((error) => error);
  assert.notEqual(result.code, 0);
...
// AI Control tests
{
  const aiControlFile = `${outDir}/test-ai-commands.json`;
  await writeFile(
    aiControlFile,
  ...
  const result = await execFileAsync(zero, [
    ...
    "--ai-control",
    aiControlFile,
    "conformance/agent-surface/fixtures/ai-control-profile-switch.0",
  ]);
  const body = JSON.parse(result.stdout);
  assert.equal(
    body.ok,
    true,
    "AI control: check should pass with profile switch",
  );
  console.log("AI control: set-profile command works");
}

{
  const aiControlStatusFile = `${outDir}/test-ai-status.json`;
  await writeFile(aiControlStatusFile, '{"command":"status","args":[]}\n');
  const result = await execFileAsync(zero, [
    "check",
    "--json",
    "--ai-control",
  ...
  assert.equal(
    body.ok,
    true,
    "AI control: check should pass with status command",
  );
  // Verify status file was written
  const statusOutFile = `${aiControlStatusFile}.out`;
  try {
    const statusContent = await readFile(statusOutFile, "utf8");
    assert.ok(
      statusContent.length > 0,
      "AI control: status output should be non-empty",
    );
    assert.match(
      statusContent,
      /ai-control-feedback/,
      "AI control: status output should contain feedback events",
    );
    console.log("AI control: status command produced feedback");
  } catch {
    console.log("AI control: status output not found (may be cleaned up)");
  }
}

{
  // Test that --ai-control with abort command stops compilation
  const aiAbortFile = `${outDir}/test-ai-abort.json`;
  await writeFile(aiAbortFile, '{"command":"abort","args":[]}\n');
  const result = await execFileAsync(zero, [
    "check",
    "--json",
    "--ai-control",
  ...
  // Abort should still produce valid output -- abort command is rejected by default whitelist
  // The compile should succeed because abort is denied by default
  if (result.stdout) {
    const body = JSON.parse(result.stdout);
    assert.equal(
      body.ok,
      true,
      "AI control: compilation should succeed because abort is denied by default",
    );
  }
  console.log("AI control: abort is correctly denied by default");
}
