...
const outDir = join(root, ".zerox", "bench");
const runtimeOutDir = join(root, ".zerox", "out");
mkdirSync(outDir, { recursive: true });
mkdirSync(runtimeOutDir, { recursive: true });
cleanStaleGeneratedCOutputs();
loadDotEnvFiles([".zerox/bench/snapshot.env", ".env", ".env.local"]);
...
const cases = [
  {
    name: "baseline-hello",
    expectedStdout: "baseline hello",
    languages: ["zerox"],
    sources: { zerox: "benchmarks/zerox/baseline-hello.0" },
  },
  {
    name: "baseline-file-cli",
    expectedStdout: "cli file ok",
    languages: ["zerox"],
    skipLanguages: ["zerox"],
    sources: { zerox: "examples/cli-file.0" },
    args: ["write"],
    env: { ZERO_CLI_FILE_MODE: "quiet" },
    skipReason:
      "hosted filesystem CLI direct lowering remains a tracked std.fs breadth gap",
  },
  {
    name: "baseline-memory-package",
    expectedStdout: "memory package ok",
    languages: ["zerox"],
    sources: { zerox: "examples/memory-package" },
  },
  ...
  { name: "readall", expectedStdout: "readall", skipLanguages: ["zerox"] },
  { name: "branches", expectedStdout: "branches" },
  {
    name: "module-package",
    expectedStdout: "module package",
    languages: ["zerox"],
    sources: { zerox: "benchmarks/zerox/module-package" },
  },
  {
    name: "owned-file",
    expectedStdout: "owned file",
    languages: ["zerox"],
    skipLanguages: ["zerox"],
  },
  { name: "rescue", expectedStdout: "rescue", languages: ["zerox"] },
  {
    name: "fs-resource",
    expectedStdout: "fs resource",
    languages: ["zerox"],
    skipLanguages: ["zerox"],
  },
  {
    name: "mem-copy-fill",
    expectedStdout: "mem copy fill",
    languages: ["zerox"],
  },
  { name: "zero-hash", expectedStdout: "zero-hash ok", languages: ["zerox"] },
  { name: "json", expectedStdout: "json", languages: ["zerox"] },
  {
    name: "networking",
    expectedStdout: "networking",
    languages: ["zerox"],
    skipLanguages: ["zerox"],
  },
];
...
const languages = [
  {
    ...
    build: (benchCase) => [
      ...
      [
        ...
        "--target",
        directZeroxTarget(),
        sourcePath(benchCase, "zerox", ".0"),
        "--out",
        `.zerox/bench/zerox-${benchCase.name}`,
      ],
    ],
    exe: (benchCase) => `.zerox/bench/zerox-${benchCase.name}`,
  },
];
...
const caseByName = new Map(
  cases.map((benchCase) => [benchCase.name, benchCase]),
);

function cleanStaleGeneratedCOutputs() {
  for (const entry of readdirSync(outDir)) {
    if (/^zerox-.+\.c$/.test(entry))
...
function zeroxDirectCapable() {
  return directZeroxTarget() !== null;
}
...
function hasTool(command) {
  if (command.includes("/")) {
    return (
      ...
      spawnSync(command, [], { stdio: "ignore" }).status !== null
    );
  }
  return (
...
function timedRun(command, commandArgs = [], env = {}) {
  ...
  if (process.platform === "darwin") {
    const started = performance.now();
    const result = spawnSync("/usr/bin/time", ["-l", command, ...commandArgs], {
      encoding: "utf8",
      env: runEnv,
    });
    const elapsedMs = performance.now() - started;
    if (
      result.status !== 0 &&
      result.stderr.includes("sysctl kern.clockrate")
    ) {
    ...
    return {
      ...
      stderr: result.stderr
        .replace(/\s*\d+\s+maximum resident set size[\s\S]*$/m, "")
        .trim(),
    };
  }
...
function allJobs() {
  ...
  for (const language of languages) {
    for (const benchCase of cases)
      jobs.push({ language: language.name, case: benchCase.name });
  }
  return jobs;
}
...
function executableJobs() {
  return allJobs().filter((job) => {
    ...
    return (
      language && benchCase && isCaseDefinedForLanguage(benchCase, language)
    );
  });
}
...
function skippedResults() {
  return allJobs().flatMap((job) => {
    ...
    if (
      !language ||
      !benchCase ||
      isCaseDefinedForLanguage(benchCase, language)
    )
      return [];
...
function sortResults(results) {
  const order = new Map(
    allJobs().map((job, index) => [`${job.language}:${job.case}`, index]),
  );
  return [...results].sort((left, right) => {
    const leftOrder =
      order.get(`${left.language}:${left.case}`) ?? Number.MAX_SAFE_INTEGER;
    const rightOrder =
      order.get(`${right.language}:${right.case}`) ?? Number.MAX_SAFE_INTEGER;
...
function runJobs(jobs) {
  ...
  for (const job of jobs) {
    const language = languageByName.get(job.language);
    const benchCase = caseByName.get(job.case);
    if (!language || !benchCase)
      throw new Error(`unknown benchmark job ${JSON.stringify(job)}`);
...
function runCase(language, benchCase) {
  ...
  if (!isCaseDefinedForLanguage(benchCase, language)) {
    return {
      ...
      case: name,
      status: "skipped",
      reason: "case is not defined for this language",
    };
  }
  ...
  if (language.name === "zerox") {
    rmSync(exe, { force: true });
    rmSync(`${exe}.c`, { force: true });
  }

  const directZerox = language.name === "zerox" && zeroxDirectCapable();
  const backendMode = directZerox ? "direct" : "native-toolchain";
  let build;
  let buildBody = null;
  if (directZerox) {
  ...
  if (build.result.status !== 0) {
    ...
    if (language.name === "zerox" && diagnosticCode === "BLD004") {
      return {
        language: language.name,
        case: name,
        status: "skipped",
        reason: `direct executable runner unavailable for this case on ${directZeroxTarget()}`,
        backendMode,
        target: directZerox ? directZeroxTarget() : null,
  ...
  const runs = [];
  for (let index = 0; index < Math.max(1, runCount); index++)
    runs.push(timedRun(exe, benchCase.args ?? [], benchCase.env ?? {}));
  const run = runs[runs.length - 1];
  const runTimes = runs.map((item) => item.elapsedMs);
  const peakRssValues = runs
    .map((item) => item.peakRssBytes)
  ...
  return {
    ...
    case: name,
    status:
      runs.every((item) => item.result.status === 0) && outputMatches
        ? "ok"
        : outputMatches
          ? "run-failed"
          : "semantic-failed",
    ...
    cBridgeFallback:
      buildBody?.selfHostRouting?.cBridge?.required ??
      buildBody?.cBridgeFallback ??
      (directZerox ? false : null),
    buildMs: build.elapsedMs,
    emitCMs: null,
    cCompileMs: null,
    ...
    generatedCBytes:
      language.name === "zerox" ? (buildBody?.generatedCBytes ?? 0) : null,
    stdout,
...
function buildSummary(results) {
  const summary = {};
  for (const language of languages) {
    const languageResults = results.filter(
      (item) => item.language === language.name,
    );
    ...
    summary[language.name] = {
      counts,
      averageBuildMs: average(
        ok
          .map((item) => item.buildMs)
          .filter((value) => typeof value === "number"),
      ),
      averageArtifactBytes: average(
        ok
          .map((item) => item.artifactBytes)
          .filter((value) => typeof value === "number"),
      ),
      averageCompressedArtifactBytes: average(
        ok
          .map((item) => item.compressedArtifactBytes)
          .filter((value) => typeof value === "number"),
      ),
      averageRunMedianMs: average(
        ok
          .map((item) => item.runMedianMs)
          .filter((value) => typeof value === "number"),
      ),
...
function zeroxResult(results, name) {
  return (
    results.find(
      (item) =>
        item.language === "zerox" && item.case === name && item.status === "ok",
    ) ?? null
  );
}

function measurementFact(results, name, measurement, status = "measured") {
  const result = zeroxResult(results, name);
  return {
    name,
    measurement,
...
function buildMeasurementCoverage(results) {
  return {
    startup: [measurementFact(results, "baseline-hello", "cli-startup")],
    memory: [
      measurementFact(results, "baseline-hello", "runtime-memory-floor"),
      measurementFact(results, "baseline-memory-package", "peak-rss"),
      {
        name: "allocation-counts",
        measurement: "allocation-counts",
        status: "instrumentation-planned",
        note: "stdlib allocation APIs expose explicit allocators; runtime allocation counters need allocator instrumentation",
      },
    ],
    operations: [
...
function buildReport(results, environment, runner) {
  const report = {
    ...
    methodology: {
      ...
      peakRssBytes:
        "maximum resident set size from /usr/bin/time -l on macOS; null when unavailable",
      compressedArtifactBytes:
        "gzip-compressed executable size in bytes for deployable-size baselines",
      generatedCBytes: "expected to be 0 for Zerox benchmark rows",
      backendMode: "direct or native-toolchain per benchmark row",
      emitCMs:
        "retained as a nullable historical field; Zerox no longer emits C",
      cCompileMs:
        "retained as a nullable historical field; Zerox no longer invokes a C compiler backend",
      semanticChecks:
        "benchmark stdout must match the per-case expected output",
      runner,
    },
    claims: {
      buildTime: "buildMs records end-to-end compiler process time",
      artifactSize:
        "artifactBytes records output executable size for successful builds",
      compressedArtifactSize:
        "compressedArtifactBytes records gzip-compressed executable size for successful builds",
      runMedian:
        "runMedianMs records median process runtime across ZERO_BENCH_RUNS executions",
      generatedCBytes: "generatedCBytes is zero for Zerox benchmarks",
      directZeroBenchmarks:
        "sandbox Linux runs use direct Zerox artifacts for Zerox benchmark cases",
      runtimeMeasurements:
        "measurements groups startup, memory, and operation benchmark facts; metadata-only entries call out runners that are not implemented yet",
    },
    limitations: [
      ...
      "skipped Zerox cases are listed explicitly with a reason",
    ],
    environment,
...
function buildBenchmarkTrendSummary(report) {
  const zeroxRows = report.results.filter(
    (row) =>
      row.language === "zerox" &&
      row.backendMode === "direct" &&
      row.status === "ok",
  );
  const artifactBytes = zeroxRows
    .map((row) => row.artifactBytes)
    .filter((value) => typeof value === "number");
  const compressedArtifactBytes = zeroxRows
    .map((row) => row.compressedArtifactBytes)
    .filter((value) => typeof value === "number");
  const buildMs = zeroxRows
    .map((row) => row.buildMs)
    .filter((value) => typeof value === "number");
  const runMedianMs = zeroxRows
    .map((row) => row.runMedianMs)
    .filter((value) => typeof value === "number");
  const startupRows = report.measurements.startup.filter(
    (row) => row.status === "measured",
  );
  const memoryRows = report.measurements.memory.filter(
    (row) => row.status === "measured",
  );
  const operationRows = report.measurements.operations.filter(
    (row) => row.status === "measured",
  );
  return {
    schemaVersion: 1,
    kind: "zerox-benchmark-trend-summary",
    generatedAt: report.generatedAt,
    sourceReport: ".zerox/bench/latest.json",
    runCount,
    ...
    directZerox: {
      rows: zeroxRows.length,
      generatedCBytes: zeroxRows.reduce(
        (total, row) => total + (row.generatedCBytes ?? 0),
        0,
      ),
      ...
      cases: zeroxRows.map((row) => row.case).sort(),
    },
    coverage: {
...
function assertBenchmarkGuardrails(report) {
  ...
  for (const name of guarded) {
    const result = report.results.find(
      (item) => item.language === "zerox" && item.case === name,
    );
    if (!result) throw new Error(`missing benchmark result for ${name}`);
    if (result.status !== "ok")
      throw new Error(
        `${name} benchmark status was ${result.status}\n${formatFailedResult(result)}`,
      );
    if (result.outputMatches !== true)
      throw new Error(
        `${name} benchmark stdout did not match\n${formatFailedResult(result)}`,
      );
    if (result.backendMode !== "direct")
      throw new Error(
        `${name} did not use the direct backend\n${formatFailedResult(result)}`,
      );
    if (result.generatedCBytes !== 0)
      throw new Error(
        `${name} direct benchmark emitted C bytes\n${formatFailedResult(result)}`,
      );
    if (result.cBridgeFallback)
      throw new Error(
        `${name} direct benchmark used C bridge fallback\n${formatFailedResult(result)}`,
      );
    if (typeof result.artifactBytes !== "number" || result.artifactBytes <= 0)
      throw new Error(
        `${name} missing artifact size\n${formatFailedResult(result)}`,
      );
    if (
      typeof result.compressedArtifactBytes !== "number" ||
      result.compressedArtifactBytes <= 0
    )
      throw new Error(
        `${name} missing compressed artifact size\n${formatFailedResult(result)}`,
      );
    if (typeof result.runMedianMs !== "number" || result.runMedianMs <= 0)
      throw new Error(
        `${name} missing startup timing\n${formatFailedResult(result)}`,
      );
    if (!("peakRssBytes" in result))
      throw new Error(
        `${name} missing peak RSS field\n${formatFailedResult(result)}`,
      );
  }
  for (const language of languages) {
    if (!report.summary[language.name]?.counts)
      throw new Error(`missing summary counts for ${language.name}`);
  }
}
...
function assertDirectBenchmarks(report) {
  ...
  for (const result of report.results.filter(
    (item) => item.language === "zerox" && item.status !== "skipped",
  )) {
    const name = result.case;
    if (!result)
      throw new Error(`missing direct Zerox benchmark result for ${name}`);
    if (result.status !== "ok")
      throw new Error(
        `${name} direct benchmark status was ${result.status}\n${formatFailedResult(result)}`,
      );
    if (result.backendMode !== "direct")
      throw new Error(
        `${name} did not use the direct backend in sandbox benchmarks\n${formatFailedResult(result)}`,
      );
    if (result.generatedCBytes !== 0)
      throw new Error(
        `${name} direct benchmark emitted C bytes\n${formatFailedResult(result)}`,
    ...
    if (result.cBridgeFallback)
      throw new Error(
        `${name} direct benchmark required a C bridge\n${formatFailedResult(result)}`,
      );
    if (!result.objectEmissionPath?.startsWith("direct-"))
      throw new Error(
        `${name} missing direct object emission path\n${formatFailedResult(result)}`,
      );
  }
}
...
function assertBenchmarkCoverage(report) {
  if (report.environment?.runner !== "vercel-sandbox") return;
  const zeroxRows = report.results.filter((item) => item.language === "zerox");
  const directRows = zeroxRows.filter(
    (item) => item.status === "ok" && item.backendMode === "direct",
  );
  if (directRows.length < directZeroMinimum) {
    throw new Error(
      `Zerox benchmark coverage requires at least ${directZeroMinimum} direct rows, found ${directRows.length}`,
    );
  }
  for (const result of directRows) {
    if (result.outputMatches !== true)
      throw new Error(
        `${result.case} benchmark stdout did not match\n${formatFailedResult(result)}`,
      );
    if (result.generatedCBytes !== 0)
      throw new Error(
        `${result.case} direct benchmark emitted C bytes\n${formatFailedResult(result)}`,
      );
    if (result.cBridgeFallback)
      throw new Error(
        `${result.case} direct benchmark required a C bridge\n${formatFailedResult(result)}`,
      );
    if (!result.objectEmissionPath?.startsWith("direct-"))
      throw new Error(
        `${result.case} missing direct object emission path\n${formatFailedResult(result)}`,
      );
  }
  const removedBackendSkips = zeroxRows.filter(
    (item) =>
      item.status === "skipped" &&
      /removed C backend|C backend output/.test(item.reason ?? ""),
  );
  if (removedBackendSkips.length > 0) {
    throw new Error(
      `removed-backend skip reasons remain: ${removedBackendSkips.map((item) => item.case).join(", ")}`,
    );
  }
}
...
function assertMeasurementCoverage(report) {
  for (const category of ["startup", "memory", "operations"]) {
    if (
      !Array.isArray(report.measurements?.[category]) ||
      report.measurements[category].length === 0
    ) {
  ...
  for (const name of ["fileio", "codec", "parse", "branches", "rescue"]) {
    const fact = report.measurements.operations.find(
      (item) => item.name === name,
    );
    if (!fact || fact.status !== "measured")
      throw new Error(`missing measured operation benchmark for ${name}`);
  }
}
...
function benchmarkGuardrailsMarkdownReport(report) {
  const rows = [
    ...
    "",
    "Timing is intentionally treated as noisy. The checked signals are semantic stdout, JSON shape, direct-output bytes, and artifact presence.",
    "",
  ];
  for (const name of ["fs-resource", "mem-copy-fill"]) {
    ...
    rows.push(
      `- ${name}: status=${result.status}, generatedCBytes=${result.generatedCBytes}, artifactBytes=${result.artifactBytes}, buildMs=${result.buildMs}, runMedianMs=${result.runMedianMs}`,
    );
  }
  const directRows = report.results.filter(
    (item) => item.language === "zerox" && item.backendMode === "direct",
  );
  const removedBackendSkips = report.results.filter(
    (item) =>
      item.language === "zerox" &&
      item.status === "skipped" &&
      /removed C backend|C backend output/.test(item.reason ?? ""),
  ...
  rows.push(
    `- direct zerox rows: ${directRows.length}, generatedCBytes=${directRows.reduce((total, row) => total + (row.generatedCBytes ?? 0), 0)}`,
  );
  rows.push(
    `- direct zerox threshold: ${directRows.length}/${directZeroMinimum}`,
  );
  rows.push(`- removed-backend skip reasons: ${removedBackendSkips.length}`);
  rows.push("");
  rows.push(
    `- zerox summary counts: ${JSON.stringify(report.summary.zerox.counts)}`,
  );
  rows.push(
    "- timings are recorded for context; semantic output and artifact facts are the regression signals.",
  );
  rows.push("");
  return `${rows.join("\n")}\n`;
}
...
function baselineMarkdownReport(report) {
  const rows = [
    "# Baseline Metrics",
    "",
    "These baselines track binary size, compressed size, startup timing, and peak RSS when available.",
    "",
  ];
  for (const name of [
    "baseline-hello",
    "baseline-file-cli",
    "baseline-memory-package",
  ]) {
    const result = report.results.find(
      (item) => item.language === "zerox" && item.case === name,
    );
    if (result.status === "skipped") {
      ...
      rows.push(
        `- ${name}: artifactBytes=${result.artifactBytes}, compressedArtifactBytes=${result.compressedArtifactBytes}, runMedianMs=${result.runMedianMs}, runMinMs=${result.runMinMs}, peakRssBytes=${result.peakRssBytes}`,
      );
    }
  }
  rows.push("");
...
function trendMarkdownReport(summary) {
  const rows = [
    ...
    "",
    "This file is a compact trend artifact. It is separate from the one-run smoke report so CI can archive it across runs.",
    "",
  ];
  rows.push(`- runner: ${summary.runner}`);
  rows.push(`- runCount: ${summary.runCount}`);
  rows.push(`- direct Zerox rows: ${summary.directZerox.rows}`);
  rows.push(`- generatedCBytes: ${summary.directZerox.generatedCBytes}`);
  rows.push(
    `- averageArtifactBytes: ${summary.directZerox.averageArtifactBytes}`,
  );
  rows.push(
    `- averageCompressedArtifactBytes: ${summary.directZerox.averageCompressedArtifactBytes}`,
  );
  rows.push(`- averageBuildMs: ${summary.directZerox.averageBuildMs}`);
  rows.push(`- averageRunMedianMs: ${summary.directZerox.averageRunMedianMs}`);
  rows.push(
    `- coverage: startup=${summary.coverage.startup}, memory=${summary.coverage.memory}, operations=${summary.coverage.operations}`,
  );
  rows.push("");
  return `${rows.join("\n")}\n`;
}
...
function assertBenchmarkTrendSummary(report) {
  ...
  if (!summary || summary.kind !== "zerox-benchmark-trend-summary")
    throw new Error("missing benchmark trend summary");
  if (
    !Array.isArray(summary.trackedMetrics) ||
    !summary.trackedMetrics.includes("compressedArtifactBytes")
  )
    throw new Error("trend summary missing tracked metrics");
  if (summary.directZerox.generatedCBytes !== 0)
    throw new Error("trend summary direct Zerox C-output bytes changed");
  if (
    summary.directZerox.rows <= 0 ||
    summary.rows.length !== summary.directZerox.rows
  )
    throw new Error("trend summary missing direct Zerox rows");
  if (
    summary.coverage.startup <= 0 ||
    summary.coverage.memory <= 0 ||
    summary.coverage.operations <= 0
  )
    throw new Error("trend summary measurement coverage incomplete");
}
...
function writeReport(report) {
  ...
  writeFileSync(
    join(dirname(outputPath), "benchmark-guardrails.md"),
    benchmarkGuardrailsMarkdownReport(report),
  );
  writeFileSync(
    join(dirname(outputPath), "baseline-metrics.md"),
    baselineMarkdownReport(report),
  );
  writeFileSync(
    join(trendDir, "latest.json"),
    `${JSON.stringify(report.trendSummary, null, 2)}\n`,
  );
  writeFileSync(
    join(trendDir, "summary.md"),
    trendMarkdownReport(report.trendSummary),
  );
...
function benchmarkJobGroups(jobs) {
  ...
  for (const language of languages) {
    ...
    if (languageJobs.length > 0)
      groups.push({ language: language.name, jobs: languageJobs });
  }
  return groups;
}
...
function sandboxProjectDir() {
  return (
    process.env.ZERO_BENCH_SANDBOX_PROJECT_DIR || "/vercel/sandbox/zerox-lang"
  );
}
...
function snapshotFromEnv() {
  if (
    ["1", "true", "yes"].includes(
      (process.env.ZERO_BENCH_SANDBOX_REFRESH ?? "").toLowerCase(),
    )
  )
    return null;
...
function writeSnapshotEnv(snapshot) {
  ...
  process.stderr.write(
    `Created Vercel Sandbox benchmark snapshot: ${snapshot.snapshotId}\n`,
  );
  process.stderr.write(`Saved reusable snapshot env to ${snapshotPath}\n`);
  process.stderr.write(
    "Future benchmark runs will reuse it automatically unless ZERO_BENCH_SANDBOX_REFRESH is set.\n",
  );
}
...
function createSourceArchive() {
  ...
  if (result.status !== 0) {
    throw new Error(
      `failed to create benchmark source archive\n${result.stderr.trim()}`,
    );
  }
  return archivePath;
}
...
async function createPreparedSnapshot(Sandbox) {
  ...
  const snapshotExpiration = parseOptionalNonNegativeInt(
    process.env.ZERO_BENCH_SANDBOX_SNAPSHOT_EXPIRATION_MS,
  );
  ...
  try {
    ...
    try {
      ...
      throw new Error(
        `benchmark sandbox snapshot failed\n${describeError(error)}`,
      );
    }
...
async function runSandboxLanguageGroup(Sandbox, snapshot, group, index) {
  ...
  try {
    await runSandboxCommand(
      sandbox,
      { cmd: "true" },
      `${group.language} benchmark sandbox warmup`,
    );
    const command = {
      cmd: "node",
      args: [
    ...
    const { stdout } = await runSandboxCommand(
      sandbox,
      command,
      `${group.language} benchmark sandbox`,
    );
    return JSON.parse(stdout);
  } finally {
    await sandbox.stop({ blocking: true }).catch(() => {});
...
async function runSandboxBench() {
  const { Sandbox } = await import("@vercel/sandbox").catch((error) => {
    throw new Error(
      `sandbox benchmark mode requires @vercel/sandbox. Run pnpm install before benchmarking.\n${error.message}`,
    );
  });
  ...
  const skippedNote =
    skipped.length === 0
      ? ""
      : ` (${skipped.length} skipped result slots recorded locally)`;
  const reusableSnapshot = snapshotFromEnv();
  if (reusableSnapshot) {
    process.stderr.write(
      `Using Vercel Sandbox benchmark snapshot ${reusableSnapshot.snapshotId} for ${jobs.length} executable jobs in ${groups.length} benchmark sandboxes${skippedNote}...\n`,
    );
  } else {
    if (process.env.ZERO_BENCH_SANDBOX_SNAPSHOT_ID)
      process.stderr.write(
        "Refreshing Vercel Sandbox benchmark snapshot because ZERO_BENCH_SANDBOX_REFRESH is set...\n",
      );
    process.stderr.write(
      `Preparing Vercel Sandbox benchmark snapshot for ${jobs.length} executable jobs in ${groups.length} benchmark sandboxes${skippedNote}...\n`,
    );
  }
  const snapshot = reusableSnapshot ?? (await createPreparedSnapshot(Sandbox));
  process.stderr.write(
    `Running ${jobs.length} executable benchmark jobs across ${groups.length} Vercel sandboxes for ${resultSlots} report rows...\n`,
  );

  const workerReports = await Promise.all(
    groups.map((group, index) =>
      runSandboxLanguageGroup(Sandbox, snapshot, group, index),
    ),
  );
  const results = sortResults([
    ...workerReports.flatMap((report) => report.results),
    ...skipped,
  ]);
  const environment = {
    ...
    sandboxShardStrategy: "zerox-benchmark",
...
async function main() {
  if (workerMode) {
    ...
    console.log(
      JSON.stringify({
        environment: buildEnvironment({ runner: "vercel-sandbox-worker" }),
        results,
      }),
    );
    return;
