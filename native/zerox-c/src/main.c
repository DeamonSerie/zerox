...
static void print_diag(const char *path, const ZDiag *diag) {
  ...
  fprintf(stderr, "  explain: zerox explain %s\n", diag_code(diag->code));
}
...
static const ZeroxEmbeddedSkill *find_embedded_skill(const char *name) {
  for (size_t i = 0; i < zerox_embedded_skill_count; i++) {
    if (strcmp(zerox_embedded_skills[i].name, name) == 0) return &zerox_embedded_skills[i];
  }
  return NULL;
}

static void append_embedded_skill_content(ZBuf *buf, const ZeroxEmbeddedSkill *skill) {
...
static void print_embedded_skill_content(const ZeroxEmbeddedSkill *skill) {
  if (!skill || !skill->content) return;
  for (size_t i = 0; skill->content[i]; i++) {
...
static int embedded_skills_list_command(bool json) {
  if (json) {
    ...
    bool first = true;
    for (size_t i = 0; i < zerox_embedded_skill_count; i++) {
      const ZeroxEmbeddedSkill *skill = &zerox_embedded_skills[i];
...
static int embedded_skills_get_command(int argc, char **argv, int subcommand_index, bool json) {
  bool get_all = false;
  const ZeroxEmbeddedSkill *targets[64];
  ...
  for (int i = subcommand_index + 1; i < argc; i++) {
    ...
    const ZeroxEmbeddedSkill *skill = find_embedded_skill(arg);
    if (!skill) {
      char message[160];
  ...
  if (get_all) {
    target_count = 0;
    for (size_t i = 0; i < zerox_embedded_skill_count && target_count < sizeof(targets) / sizeof(targets[0]); i++) {
      if (!zerox_embedded_skills[i].hidden) targets[target_count++] = &zerox_embedded_skills[i];
    }
  }

  if (target_count == 0) {
    return embedded_skills_error(json, "No skill name provided. Usage: zerox skills get <name>");
  }
...
static const char *zerox_commit(void) {
  const char *commit = getenv("ZEROX_COMMIT");
...
static bool write_runtime_compile_inputs(
  ...
  *out = (RuntimeCompileInputs){0};
  ...
  out->header_file = runtime_join_path(out->include_dir, "zerox_runtime.h");
...
static bool compile_zerox_runtime_object(const char *runtime_object_file, const ZToolchainPlan *plan, const Command *command, const ZTargetInfo *target, ZDiag *diag) {
  RuntimeCompileInputs inputs = {0};
  if (!write_runtime_compile_inputs(runtime_object_file, ".zerox_runtime.c", zerox_embedded_zerox_runtime_c, &inputs, diag)) return false;
  ...
  if (!ok && diag) {
    ...
    snprintf(diag->expected, sizeof(diag->expected), "C compiler can compile the embedded Zerox runtime source");
...
static bool compile_zerox_crypto_object(const char *runtime_object_file, const ZToolchainPlan *plan, const Command *command, const ZTargetInfo *target, ZDiag *diag) {
  RuntimeCompileInputs inputs = {0};
  if (!write_runtime_compile_inputs(runtime_object_file, ".zerox_crypto.c", zerox_embedded_zerox_crypto_c, &inputs, diag)) return false;
  /* Also write zerox_crypto.h into the include directory */
  char *crypto_header = runtime_join_path(inputs.include_dir, "zerox_crypto.h");
  if (crypto_header) write_runtime_chunks_file(crypto_header, zerox_embedded_zerox_crypto_h, diag);
  free(crypto_header);

  /* Build compile flags. Botan is a C++ library; its C FFI header is found
     either via system include paths (system install) or via ZEROX_BOTAN_DIR. */
  char extra_cflags[640];
  snprintf(extra_cflags, sizeof(extra_cflags), "-std=c11 -Wall -Wextra -Wpedantic");
  const char *botan_dir = getenv("ZEROX_BOTAN_DIR");
  if (botan_dir && botan_dir[0]) {
    size_t off = strlen(extra_cflags);
    /* Botan's configure.py generates public headers in build/include/public/ */
    snprintf(extra_cflags + off, sizeof(extra_cflags) - off, " -I'%s/build/include/public'", botan_dir);
  }
  bool ok = z_toolchain_compile_c_object(plan, command ? command->profile : NULL, target, inputs.source_file, runtime_object_file, inputs.include_dir, extra_cflags);
  runtime_compile_inputs_free(&inputs);
  if (!ok && diag) {
    diag->code = 2003;
    diag->line = 1;
    diag->column = 1;
    diag->length = 1;
    snprintf(diag->message, sizeof(diag->message), "crypto runtime object build failed");
    snprintf(diag->expected, sizeof(diag->expected), "C compiler can compile the embedded Zerox crypto source");
    snprintf(diag->actual, sizeof(diag->actual), "crypto runtime compile command failed");
    snprintf(diag->help, sizeof(diag->help), "install libbotan-3-dev or set ZEROX_BOTAN_DIR to the Botan build root");
  }
  return ok;
}

static bool compile_zerox_http_curl_object(const char *runtime_object_file, const ZToolchainPlan *plan, const Command *command, const ZTargetInfo *target, ZDiag *diag) {
  RuntimeCompileInputs inputs = {0};
  if (!write_runtime_compile_inputs(runtime_object_file, ".zerox_http_curl.c", zerox_embedded_zerox_http_curl_c, &inputs, diag)) return false;
  bool ok = z_toolchain_compile_c_object(plan, command ? command->profile : NULL, target, inputs.source_file, runtime_object_file, inputs.include_dir, "-std=c11 -Wall -Wextra -Wpedantic");
  runtime_compile_inputs_free(&inputs);
  if (!ok && diag) {
    diag->code = 2003;
...
static bool link_zerox_runtime_executable(const char *object_file, const char *runtime_object_file, const char *crypto_object_file, const char *http_object_file, const char *exe_file, const ZToolchainPlan *plan, const ZTargetInfo *target, ZDiag *diag) {
#if defined(__linux__)
  const char *linux_no_pie = " -no-pie";
#else
  const char *linux_no_pie = "";
#endif
  const char *object_files[4] = {object_file, runtime_object_file, crypto_object_file, http_object_file};
  size_t object_count = 2;
  if (crypto_object_file && crypto_object_file[0]) object_count = 3;
  if (http_object_file && http_object_file[0]) object_count = 4;
  /* Build link flags: curl if HTTP runtime, botan-3 (+ C++ stdlib) if crypto runtime */
  char link_flags[192];
  link_flags[0] = '\0';
  if (http_object_file && http_object_file[0]) {
    snprintf(link_flags, sizeof(link_flags), "-lcurl");
  }
  if (crypto_object_file && crypto_object_file[0]) {
    size_t off = strlen(link_flags);
    /* Botan is a C++ library. Determine link flags:
       - If ZEROX_BOTAN_DIR is set (local build), use -L with -lbotan-3 -lstdc++.
       - Otherwise try pkg-config, falling back to -lbotan-3 -lstdc++. */
    const char *botan_dir = getenv("ZEROX_BOTAN_DIR");
    if (botan_dir && botan_dir[0]) {
      if (off > 0) {
        snprintf(link_flags + off, sizeof(link_flags) - off,
                 " -L'%s' -lbotan-3 -lstdc++", botan_dir);
      } else {
        snprintf(link_flags, sizeof(link_flags),
                 "-L'%s' -lbotan-3 -lstdc++", botan_dir);
      }
    } else {
      if (off > 0) {
        snprintf(link_flags + off, sizeof(link_flags) - off,
                 " `pkg-config --libs botan-3 2>/dev/null || echo '-lbotan-3 -lstdc++'`");
      } else {
        snprintf(link_flags, sizeof(link_flags),
                 "`pkg-config --libs botan-3 2>/dev/null || echo '-lbotan-3 -lstdc++'`");
      }
  ...
  if (!ok && diag) {
    ...
    snprintf(diag->expected, sizeof(diag->expected), "direct object plus zerox runtime object link successfully");
    snprintf(diag->actual, sizeof(diag->actual), "runtime link command failed");
    snprintf(diag->help, sizeof(diag->help), http_object_file && http_object_file[0] ? "install libcurl or inspect the direct object, runtime objects, and host C linker diagnostics" : crypto_object_file && crypto_object_file[0] ? "inspect the direct object, crypto runtime object, and host C linker diagnostics" : "inspect the direct object, runtime object, and host C linker diagnostics");
  }
  return ok;
}
...
static int zerox_mkdir(const char *path) {
#if defined(_WIN32)
  return mkdir(path);
#else
...
static int zerox_lstat(const char *path, struct stat *st) {
#if defined(_WIN32)
  return stat(path, st);
#else
...
static bool ensure_dir(const char *path, ZDiag *diag) {
  if (zerox_mkdir(path) == 0 || errno == EEXIST) return true;
  diag->code = 2002;
  diag->path = path;
  diag->line = 1;
  diag->column = 1;
...
static bool remove_tree(const char *path, ZBuf *deleted) {
  struct stat st;
  if (zerox_lstat(path, &st) != 0) return errno == ENOENT;
...
static const char *c_abi_type_name(const char *zerox_type) {
  if (!zerox_type || strcmp(zerox_type, "Void") == 0) return "void";
  if (strcmp(zerox_type, "Bool") == 0) return "bool";
  if (strcmp(zerox_type, "u8") == 0) return "uint8_t";
  if (strcmp(zerox_type, "i8") == 0) return "int8_t";
  if (strcmp(zerox_type, "u16") == 0) return "uint16_t";
  if (strcmp(zerox_type, "i16") == 0) return "int16_t";
  if (strcmp(zerox_type, "u32") == 0) return "uint32_t";
  if (strcmp(zerox_type, "i32") == 0) return "int32_t";
  if (strcmp(zerox_type, "u64") == 0) return "uint64_t";
  if (strcmp(zerox_type, "i64") == 0) return "int64_t";
  if (strcmp(zerox_type, "usize") == 0) return "uintptr_t";
  if (strcmp(zerox_type, "isize") == 0) return "intptr_t";
  if (strcmp(zerox_type, "f32") == 0) return "float";
  if (strcmp(zerox_type, "f64") == 0) return "double";
  return "void *";
}
...
static void append_interface_fingerprints_json(ZBuf *buf, const SourceInput *input, const ZTargetInfo *target) {
  zbuf_append(buf, "{\"schemaVersion\":1,\"algorithm\":\"fnv1a64-zerox-interface-v1\",\"targetFactsHash\":\"");
...
static uint64_t compile_cache_key(const SourceInput *input, const ZTargetInfo *target, const char *profile, const char *kind) {
  ...
  hash = mix_hash_text(hash, ZEROX_VERSION);
...
static bool compiler_cache_touch(const char *kind, uint64_t key) {
  zerox_mkdir(".zerox");
  zerox_mkdir(".zerox/cache");
  zerox_mkdir(".zerox/cache/native");
  char path[256];
  snprintf(path, sizeof(path), ".zerox/cache/native/%s-%016llx.cache", kind, (unsigned long long)key);
...
#define APPEND_CACHE(label, key, hit, invalidates) do { \
    ...
    zbuf_appendf(buf, ",\"key\":\"%016llx\",\"hit\":%s,\"stored\":true,\"compilerVersion\":\"%s\",\"packageVersion\":", (unsigned long long)(key), (hit) ? "true" : "false", ZEROX_VERSION); \
...
static void append_package_metadata_json(ZBuf *buf, const SourceInput *input, const ZTargetInfo *target) {
  ...
  zbuf_appendf(buf, ",\"manifestHash\":\"%016llx\",\"dependencyGraphHash\":\"%016llx\"",
               ...
               (unsigned long long)(input ? input->dependency_graph_hash : 0));
  zbuf_append(buf, ",\"lockfile\":{\"format\":\"zerox-lock-v1\",\"path\":");
  append_json_string(buf, input && input->lockfile_path ? input->lockfile_path : "");
...
static void append_package_cache_audit_json(ZBuf *buf, const SourceInput *input, const ZTargetInfo *target, const char *profile) {
  ...
  append_json_string(buf, ZEROX_VERSION);
...
static void append_compile_time_json(ZBuf *buf, const Program *program, const SourceInput *input, const ZTargetInfo *target) {
  char *manifest = read_optional_file(input && input->manifest_path ? input->manifest_path : "zerox.json");
  zbuf_append(buf, "{");
  ...
  zbuf_append(buf, ",\"cacheKeyInputs\":{\"algorithm\":\"fnv1a64-zerox-meta-v1\"");
...
static void print_check_json_success(const char *path, SourceInput *input, const Program *program, const ZTargetInfo *target, const Command *command) {
  ...
  char *manifest = read_optional_file("zerox.json");
  zbuf_append(&buf, ",\n  \"package\": ");
...
static void append_local_runtime_plan_json(ZBuf *buf, const CapabilitySummary *caps, const ZTargetInfo *target) {
  ...
  append_json_string(buf, "zerox dev");
...
static void append_dev_plan_json(ZBuf *buf, const SourceInput *input, const Program *program, const ZTargetInfo *target, const Command *command) {
  ...
  append_json_string(buf, "zerox.json");
  ...
  zbuf_append(buf, "}],\n  \"restart\": {\"runnableCli\":");
  zbuf_append(buf, main_fun ? "true" : "false");
  zbuf_append(buf, ",\"strategy\":\"rebuild-and-restart-after-successful-checks\",\"binaryOut\":\".zerox/dev/app\"},\n  \"partialDiagnostics\": {\"stable\":true,\"whileCodegenPending\":true,\"sourceOfTruth\":\"zerox check --json\",\"schemaVersion\":1},\n  \"trace\": {\"enabled\":true,\"requested\":");
...
static const char *diag_repair_summary(int code) {
  switch (code) {
    case 100: return "Repair the syntax at the reported parser span, then rerun zerox check.";
    ...
    case 4004: return "Use zerox targets --json to choose a direct-supported target, or request --emit obj when only object emission exists.";
    ...
    case 9001: return "Create the local dependency package or update the path in zerox.json.";
    ...
    default: return code == 0 ? "Repair the syntax at the reported parser span, then rerun zerox check." : "Inspect the diagnostic fields and choose a repair manually.";
  }
}
...
static const ExplainInfo explain_infos[] = {
  {
    ...
    "Zerox keeps target names explicit so cross builds do not silently use host assumptions.",
    "Run `zerox targets` and choose one of the listed names.",
    "zerox check --target not-a-target examples/hello.0",
    "zerox check --target linux-musl-x64 examples/hello.0",
  },
  {
    "TAR002",
    "target",
    ...
    "Zerox checks required standard-library capabilities against the target manifest so native, web, and embedded builds do not silently inherit host behavior.",
    "Build for a target that provides the required capability, or move that capability behind a target-specific entry point.",
    "zerox check --target linux-musl-x64 conformance/native/fail/std-fs-target-unsupported.0",
    "zerox build --target host examples/memory-package --out .zero/out/memory-package",
  },
  {
    "BLD003",
    "build",
    "Generated C backend removed",
    "Generated C output is no longer part of the supported Zerox toolchain.",
    ...
    "zerox build --emit c examples/hello.0",
    "zerox build --emit exe examples/hello.0",
  },
  {
    "TYP009",
    "type",
  ...
  },
  {
    ...
    "Zerox keeps error flow visible in source and in function signatures.",
  ...
  },
  {
    ...
    "Zerox's first generic slice monomorphizes calls directly, so the concrete type argument list must be explicit and unambiguous.",
  ...
  },
  {
    ...
    "Zerox keeps compile-time execution sandboxed, deterministic, and bounded, so filesystem, network, process, and ambient environment access fail before code generation.",
  ...
  },
  {
    ...
    "Zerox monomorphizes generic calls directly, so growing recursive instantiations such as Maybe<T>, Maybe<Maybe<T>>, and deeper forms are rejected instead of expanding without a bound.",
  ...
  },
  {
    ...
    "Zerox does not create runtime interface values or vtables; satisfying an interface means the concrete shape already has the matching static method.",
	  ...
	  },
  {
    ...
    "Zerox does not infer through whole function bodies; static values must agree at the call boundary.",
  ...
  },
  {
    ...
    "Zerox tracks borrows lexically so agents can repair by moving code or introducing inner blocks without relying on hidden lifetime inference.",
  ...
  {"BLD004", "build", "Direct backend target not buildable", "The selected direct backend cannot build this source, target, object format, architecture, or artifact kind.", "Target-aware buildability runs before object or executable emission and reports ordinary direct-backend limitations with structured blocker facts.", "Choose a target whose `zerox targets --json` directBackend facts advertise the requested artifact, or simplify the program to the backend-supported subset.", "zerox check --json --emit obj --target linux-arm64 examples/direct-call-add.0", "zerox check --json --emit obj --target linux-x64 examples/direct-call-add.0"},
  {"CGEN004", "codegen", "Direct code generation invariant failed", "A direct emitter reached an internal code generation invariant after target buildability accepted the program.", "Ordinary unsupported targets and source features should be reported before emission with BLD004.", "Report this compiler bug with the source program and target that produced it.", "zerox build --json --emit obj --target linux-musl-x64 examples/direct-call-add.0", "zerox build --json --emit obj --target linux-x64 examples/direct-call-add.0"},
...
static void print_help(void) {
  printf("zerox %s native bootstrap\n\n", ZEROX_VERSION);
  printf("Usage:\n");
  printf("  zerox --version [--json]\n");
  printf("  zerox skills [list|get] [--json]\n");
  printf("  zerox new cli|lib|package <name>\n");
  printf("  zerox check <file.0|file.row|project|zerox.json>\n");
  printf("  zerox test <file.0|file.row|project|zerox.json>\n");
  printf("  zerox fmt <file.0|file.row|project|zerox.json>\n");
  printf("  zerox build [--json] [--emit exe|obj] [--target <target>] [--profile debug|dev|release-fast|release-small|tiny|audit] [--release <profile>] [--out <file>] [--ai-control <path>] <file.0|file.row|project|zerox.json>\n");
  printf("  zerox run [--target <target>] [--profile debug|dev|release-fast|release-small|tiny|audit] [--release <profile>] [--out <file>] [--ai-control <path>] <file.0|file.row|project|zerox.json> [-- args...]\n");
  printf("  zerox ship [--json] [--target <target>] [--profile release-small|tiny|audit] [--out <file>] <file.0|file.row|project|zerox.json>\n");
  printf("  zerox tokens --json <file.0|file.row|project|zerox.json>\n");
  printf("  zerox parse --json <file.0|file.row|project|zerox.json>\n");
  printf("  zerox graph [--json] <file.0|file.row|project|zerox.json>\n");
  printf("  zerox doc [--json] <file.0|file.row|project|zerox.json>\n");
  printf("  zerox size [--json] [--out <artifact>] <file.0|file.row|project|zerox.json>\n");
  printf("  zerox mem [--json] [--target <target>] <file.0|file.row|project|zerox.json>\n");
  printf("  zerox dev [--json] [--trace] <file.0|file.row|project|zerox.json>\n");
  printf("  zerox time --json <file.0|file.row|project|zerox.json>\n");
  printf("  zerox abi check|dump [--json] [--target <target>] <file.0|file.row|project|zerox.json>\n");
  printf("  zerox explain [--json] <code>\n");
  printf("  zerox fix --plan --json <file.0|file.row|project|zerox.json>\n");
  printf("  zerox doctor [--json]\n");
  printf("  zerox clean [--all]\n");
  printf("  zerox targets\n");
  printf("\nExamples:\n");
  printf("  zerox new cli hello\n");
  printf("  zerox run examples/add.0\n");
  printf("  zerox build --emit exe examples/hello.0 --out .zero/out/hello\n");
  printf("  zerox ship --target linux-musl-x64 examples/hello.0 --out .zero/ship/hello\n");
  printf("  zerox check --json examples/hello.0\n");
  printf("  zerox build --target linux-musl-x64 examples/memory-package\n");
}

static void print_command_help(const char *command) {
  if (strcmp(command, "new") == 0) {
    printf("Usage: zerox new cli|lib|package <name>\n\n");
    printf("Create a small Zerox project template.\n\n");
    printf("Examples:\n");
    printf("  zerox new cli hello\n");
    printf("  zerox new lib math-kit\n");
    printf("  zerox new package demo\n");
  } else if (strcmp(command, "skills") == 0) {
    printf("Usage: zerox skills [list|get] [--json]\n\n");
    ...
    printf("Usage: zerox check [--json] [--target <target>] [--emit exe|obj] <file.0|file.row|project|zerox.json>\n\n");
    printf("Parse and typecheck Zerox source without emitting artifacts. JSON output includes target readiness for the selected emit kind.\n");
  } else if (strcmp(command, "build") == 0) {
    printf("Usage: zerox build [--json] [--emit exe|obj] [--target <target>] [--profile debug|dev|release-fast|release-small|tiny|audit] [--release <profile>] [--out <file>] <input>\n\n");
    printf("Build direct native executable or object artifacts.\n\n");
    printf("Example: zerox build --release tiny --emit exe examples/hello.0 --out .zero/out/hello\n");
  } else if (strcmp(command, "run") == 0) {
    printf("Usage: zerox run [--target <target>] [--profile debug|dev|release-fast|release-small|tiny|audit] [--release <profile>] [--out <file>] <input> [-- args...]\n\n");
    printf("Build a host executable with the direct backend and run it. Program stdout and stderr are passed through unchanged.\n\n");
    printf("Example: zerox run examples/add.0\n");
  } else if (strcmp(command, "ship") == 0) {
    printf("Usage: zerox ship [--json] [--target <target>] [--profile release-small|tiny|audit] [--out <file>] <input>\n\n");
    ...
    printf("Example: zerox ship --target linux-musl-x64 examples/hello.0 --out .zero/ship/hello\n");
  } else if (strcmp(command, "test") == 0) {
    printf("Usage: zerox test [--json] [--filter <name>] [--target <target>] [--cc <path>] [--out <file>] <file.0|file.row|project|zerox.json>\n\n");
    printf("Build and run inline `test` blocks.\n");
  } else if (strcmp(command, "fmt") == 0) {
    printf("Usage: zerox fmt [--check] <file.0|file.row|project|zerox.json>\n\n");
    printf("Print deterministic bootstrap formatting for Zerox source.\n");
  } else if (strcmp(command, "targets") == 0) {
    printf("Usage: zerox targets\n\n");
    printf("Print supported target facts as JSON.\n");
  } else if (strcmp(command, "tokens") == 0) {
    printf("Usage: zerox tokens --json <file.0|file.row|project|zerox.json>\n\n");
    printf("Emit source token JSON for oracle comparisons.\n");
  } else if (strcmp(command, "parse") == 0) {
    printf("Usage: zerox parse --json <file.0|file.row|project|zerox.json>\n\n");
    printf("Emit normalized source parse JSON for oracle comparisons.\n");
  } else if (strcmp(command, "abi") == 0) {
    printf("Usage: zerox abi check|dump [--json] [--target <target>] <file.0|file.row|project|zerox.json>\n\n");
    printf("Check ABI-safe declarations or dump target-aware source layout facts.\n");
  } else if (strcmp(command, "graph") == 0) {
    printf("Usage: zerox graph [--json] [--target <target>] <file.0|file.row|project|zerox.json>\n\n");
    printf("Inspect modules, symbols, capabilities, static metadata, and stdlib helpers.\n");
  } else if (strcmp(command, "doc") == 0) {
    printf("Usage: zerox doc [--json] [--target <target>] <file.0|file.row|project|zerox.json>\n\n");
    printf("Emit package API documentation facts without emitting artifacts.\n");
  } else if (strcmp(command, "size") == 0) {
    printf("Usage: zerox size [--json] [--target <target>] [--profile debug|dev|release-fast|release-small|tiny|audit] [--release <profile>] [--out <artifact>] <file.0|file.row|project|zerox.json>\n\n");
    printf("Report direct IR size, optional artifact bytes, capabilities, and stdlib helper metadata.\n");
  } else if (strcmp(command, "mem") == 0) {
    printf("Usage: zerox mem [--json] [--target <target>] <file.0|file.row|project|zerox.json>\n\n");
    printf("Report direct stack, static, heap, buffer, and runtime memory facts.\n");
  } else if (strcmp(command, "dev") == 0) {
    printf("Usage: zerox dev [--json] [--trace] [--target <target>] <file.0|file.row|project|zerox.json>\n\n");
    printf("Emit a direct incremental watch plan, interface fingerprints, and affected-test summary.\n");
  } else if (strcmp(command, "time") == 0) {
    printf("Usage: zerox time --json [--target <target>] <file.0|file.row|project|zerox.json>\n\n");
    printf("Emit compiler phase, cache, and invalidation timing facts.\n");
  } else if (strcmp(command, "explain") == 0) {
    printf("Usage: zerox explain [--json] <diagnostic-code>\n\n");
    printf("Explain a diagnostic and its repair metadata.\n");
  } else if (strcmp(command, "fix") == 0) {
    printf("Usage: zerox fix (--plan|--patch|--apply) --json [--target <target>] <file.0|file.row|project|zerox.json>\n\n");
    ...
    printf("Usage: zerox --version [--json]\n\n");
...
static char *direct_find_stdlib_root(const char *root) {
  const char *checks[] = {"../zeroStandardLibrary", "zeroStandardLibrary", NULL};
  for (int i = 0; checks[i]; i++) {
    char *candidate = direct_join_path(root, checks[i]);
    char *test = direct_join_path(candidate, "std/crypto/cipher.0");
    bool found = direct_file_exists(test);
    free(test);
...
static bool direct_row_resolve_file(const char *path, const char *root, SourceInput *input, ZBuf *combined, ZDiag *diag, char ***stack, size_t *stack_len, const char *stdlib_root) {
  ...
  const char *module_root = root;
  char *tmp_stdlib_root = NULL;
  if (stdlib_root && strncmp(path, stdlib_root, strlen(stdlib_root)) == 0) {
    module_root = stdlib_root;
  } else {
    tmp_stdlib_root = direct_find_stdlib_root(root);
    if (tmp_stdlib_root) module_root = tmp_stdlib_root;
  }
  char *module = direct_row_module_name_from_path(module_root, path);
  if (tmp_stdlib_root) free(tmp_stdlib_root);
  *stack = z_checked_reallocarray(*stack, *stack_len + 1, sizeof(char *));
  ...
  for (size_t i = 0; i < tree.len && diag->code == 0; i++) {
    ...
    bool is_stdlib = strncmp(import_name, "std.", 4) == 0;
    char *import_root = NULL;
    const char *resolve_root = root;
    if (is_stdlib) {
      if (stdlib_root) {
        resolve_root = stdlib_root;
      } else {
        import_root = direct_find_stdlib_root(root);
        if (import_root) resolve_root = import_root;
      }
    }
    direct_input_push_import(input, import_name);
    char *import_path = direct_row_module_path(resolve_root, import_name);
    if (import_root) free(import_root);
    if (!import_path) {
      if (!is_stdlib) {
    ...
    bool ok = direct_row_resolve_file(import_path, resolve_root, input, combined, diag, stack, stack_len, stdlib_root);
    free(import_path);
    free(import_name);
    if (!ok) break;
  }
...
static bool resolve_direct_row_source_at_root(const char *path, const char *root, SourceInput *input, ZDiag *diag) {
  ...
  char **stack = NULL;
  size_t stack_len = 0;
  char *stdlib_root = direct_find_stdlib_root(root);
  bool ok = direct_row_resolve_file(path, root, input, &combined, diag, &stack, &stack_len, stdlib_root);
  free(stdlib_root);
  free(stack);
...
static char *direct_manifest_path_for_input(const char *input_path) {
  if (strcmp(input_path, "zerox.json") == 0 || has_suffix(input_path, "/zerox.json")) return z_strdup(input_path);
  char *manifest_path = direct_join_path(input_path, "zerox.json");
...
static bool resolve_direct_row_package_source(const char *input_path, SourceInput *input, ZDiag *diag, bool *handled) {
  ...
  *handled = true;
  ...
  if (!parsed_manifest.main_path || !is_row_source_path(parsed_manifest.main_path)) {
    ...
    diag->column = 1;
    diag->length = 1;
    snprintf(diag->message, sizeof(diag->message), "target main source must use current Zerox syntax");
    ...
    snprintf(diag->help, sizeof(diag->help), "set targets.cli.main to a current Zerox source file");
...
static void set_source_input_diag(const char *input_path, ZDiag *diag) {
  ...
  diag->column = 1;
  diag->length = 1;
  snprintf(diag->message, sizeof(diag->message), "expected Zerox source file or package");
  snprintf(diag->expected, sizeof(diag->expected), ".0 source file, .row source file, zerox.json, or package directory");
  snprintf(diag->actual, sizeof(diag->actual), "%s", input_path ? input_path : "");
  snprintf(diag->help, sizeof(diag->help), "pass a current Zerox source file or a package with targets.cli.main");
}
...
typedef struct {
  ...
  bool path_rename;
  bool zerox_json_parse_bytes;
  bool zerox_http_fetch_result;
  bool zerox_http_result_ok;
  bool zerox_http_result_status;
  bool zerox_http_result_body_len;
  bool zerox_http_result_error;
  bool zerox_http_response_len;
  bool zerox_http_response_headers_len;
  bool zerox_http_response_body_offset;
  bool zerox_http_header_value;
  bool zerox_http_header_found;
  bool zerox_http_header_offset;
  bool zerox_http_header_len;
  bool zerox_crypto_sha256;
  bool zerox_crypto_hmac_sha256;
  bool zerox_crypto_aes_encrypt;
  bool zerox_crypto_aes_decrypt;
...
static void runtime_import_audit_value(const IrValue *value, RuntimeImportAudit *audit) {
  ...
  switch (value->kind) {
    ...
    case IR_VALUE_JSON_STREAM_TOKENS_BYTES:
      audit->zerox_json_parse_bytes = true;
      break;
    case IR_VALUE_HTTP_FETCH:
      audit->zerox_http_fetch_result = true;
      break;
    case IR_VALUE_HTTP_RESULT_OK:
      audit->zerox_http_result_ok = true;
      break;
    case IR_VALUE_HTTP_RESULT_STATUS:
      audit->zerox_http_result_status = true;
      break;
    case IR_VALUE_HTTP_RESULT_BODY_LEN:
      audit->zerox_http_result_body_len = true;
      break;
    case IR_VALUE_HTTP_RESULT_ERROR:
      audit->zerox_http_result_error = true;
      break;
    case IR_VALUE_HTTP_RESPONSE_LEN:
      audit->zerox_http_response_len = true;
      break;
    case IR_VALUE_HTTP_RESPONSE_HEADERS_LEN:
      audit->zerox_http_response_headers_len = true;
      break;
    case IR_VALUE_HTTP_RESPONSE_BODY_OFFSET:
      audit->zerox_http_response_body_offset = true;
      break;
    case IR_VALUE_HTTP_HEADER_VALUE:
      audit->zerox_http_header_value = true;
      break;
    case IR_VALUE_HTTP_HEADER_FOUND:
      audit->zerox_http_header_found = true;
      break;
    case IR_VALUE_HTTP_HEADER_OFFSET:
      audit->zerox_http_header_offset = true;
      break;
    case IR_VALUE_HTTP_HEADER_LEN:
      audit->zerox_http_header_len = true;
      break;
    case IR_VALUE_CRYPTO_HELPER:
      switch (value->error_code) {
        case CRYPTO_SHA256: audit->zerox_crypto_sha256 = true; break;
        case CRYPTO_HMAC_SHA256: audit->zerox_crypto_hmac_sha256 = true; break;
        case CRYPTO_AES_ENCRYPT: audit->zerox_crypto_aes_encrypt = true; break;
        case CRYPTO_AES_DECRYPT: audit->zerox_crypto_aes_decrypt = true; break;
...
static bool ir_value_needs_zerox_runtime_object(const IrValue *value) {
  ...
  if (value->kind == IR_VALUE_JSON_PARSE_BYTES ||
      ...
      value->kind == IR_VALUE_CRYPTO_HELPER) return true;
  if (ir_value_needs_zerox_runtime_object(value->index) ||
      ir_value_needs_zerox_runtime_object(value->left) ||
      ir_value_needs_zerox_runtime_object(value->right)) {
    return true;
  }
  for (size_t i = 0; i < value->arg_len; i++) {
    if (ir_value_needs_zerox_runtime_object(value->args[i])) return true;
  }
  return false;
}

static bool ir_instrs_need_zerox_runtime_object(const IrInstr *instrs, size_t len) {
  for (size_t i = 0; instrs && i < len; i++) {
    const IrInstr *instr = &instrs[i];
    if (ir_value_needs_zerox_runtime_object(instr->value) ||
        ir_value_needs_zerox_runtime_object(instr->index) ||
        ir_instrs_need_zerox_runtime_object(instr->then_instrs, instr->then_len) ||
        ir_instrs_need_zerox_runtime_object(instr->else_instrs, instr->else_len)) {
      return true;
    }
  }
  return false;
...
static bool ir_needs_zerox_runtime_object(const IrProgram *ir) {
  for (size_t i = 0; ir && i < ir->function_len; i++) {
    if (ir_instrs_need_zerox_runtime_object(ir->functions[i].instrs, ir->functions[i].instr_len)) return true;
  }
  return false;
}

static size_t native_zerox_runtime_import_count(const RuntimeImportAudit *audit) {
  if (!audit) return 0;
  size_t count = 0;
  if (audit->zerox_json_parse_bytes) count++;
  if (audit->zerox_http_fetch_result) count++;
  if (audit->zerox_http_result_ok) count++;
  if (audit->zerox_http_result_status) count++;
  if (audit->zerox_http_result_body_len) count++;
  if (audit->zerox_http_result_error) count++;
  if (audit->zerox_http_response_len) count++;
  if (audit->zerox_http_response_headers_len) count++;
  if (audit->zerox_http_response_body_offset) count++;
  if (audit->zerox_http_header_value) count++;
  if (audit->zerox_http_header_found) count++;
  if (audit->zerox_http_header_offset) count++;
  if (audit->zerox_http_header_len) count++;
  if (audit->zerox_crypto_sha256) count++;
  if (audit->zerox_crypto_hmac_sha256) count++;
  if (audit->zerox_crypto_aes_encrypt) count++;
  if (audit->zerox_crypto_aes_decrypt) count++;
...
static bool runtime_import_audit_uses_zerox_runtime(const RuntimeImportAudit *audit) {
  return native_zerox_runtime_import_count(audit) > 0;
}

static bool runtime_import_audit_uses_http_provider(const RuntimeImportAudit *audit) {
  return audit && audit->zerox_http_fetch_result;
}

static bool runtime_import_audit_uses_crypto(const RuntimeImportAudit *audit) {
...
static void append_runtime_import_module_json(ZBuf *buf, const RuntimeImportAudit *audit, size_t import_count) {
  ...
  bool uses_zerox_runtime = runtime_import_audit_uses_zerox_runtime(audit);
  append_json_string(buf, uses_zerox_runtime ? "zerox_runtime" : "native-target-runtime");
}

static bool append_runtime_import_functions_json(ZBuf *buf, const RuntimeImportAudit *audit) {
  bool first = true;
  zbuf_append(buf, "[");
  if (audit && audit->zerox_json_parse_bytes) append_json_string_array_item(buf, &first, "zerox_json_parse_bytes");
  if (audit && audit->zerox_http_fetch_result) append_json_string_array_item(buf, &first, "zerox_http_fetch_result");
  if (audit && audit->zerox_http_result_ok) append_json_string_array_item(buf, &first, "zerox_http_result_ok");
  if (audit && audit->zerox_http_result_status) append_json_string_array_item(buf, &first, "zerox_http_result_status");
  if (audit && audit->zerox_http_result_body_len) append_json_string_array_item(buf, &first, "zerox_http_result_body_len");
  if (audit && audit->zerox_http_result_error) append_json_string_array_item(buf, &first, "zerox_http_result_error");
  if (audit && audit->zerox_http_response_len) append_json_string_array_item(buf, &first, "zerox_http_response_len");
  if (audit && audit->zerox_http_response_headers_len) append_json_string_array_item(buf, &first, "zerox_http_response_headers_len");
  if (audit && audit->zerox_http_response_body_offset) append_json_string_array_item(buf, &first, "zerox_http_response_body_offset");
  if (audit && audit->zerox_http_header_value) append_json_string_array_item(buf, &first, "zerox_http_header_value");
  if (audit && audit->zerox_http_header_found) append_json_string_array_item(buf, &first, "zerox_http_header_found");
  if (audit && audit->zerox_http_header_offset) append_json_string_array_item(buf, &first, "zerox_http_header_offset");
  if (audit && audit->zerox_http_header_len) append_json_string_array_item(buf, &first, "zerox_http_header_len");
  zbuf_append(buf, "]");
  return !first;
}
...
static void append_runtime_import_audit_json(ZBuf *buf, const IrProgram *ir, const SourceInput *input, const ZTargetInfo *target) {
  ...
  size_t import_count = native_zerox_runtime_import_count(&audit);
...
static void append_portable_runtime_json(ZBuf *buf, const IrProgram *ir, const SourceInput *input, const ZTargetInfo *target, const CapabilitySummary *caps) {
  ...
  zbuf_append(buf, uses_zerox_runtime ? "true" : "false");
  zbuf_append(buf, ",\"module\":");
  append_runtime_import_module_json(buf, &audit, import_count);
  zbuf_append(buf, ",\"functions\":");
  append_runtime_import_functions_json(buf, &audit);
  zbuf_appendf(buf, ",\"functionCount\":%zu", import_count);
  zbuf_append(buf, ",\"adapter\":");
  if (uses_zerox_runtime) append_json_string(buf, "native-zerox-runtime-object");
...
static void append_object_backend_json(ZBuf *buf, const SourceInput *input, const ZTargetInfo *target, const Command *command, const char *emit_kind) {
  ...
  if (metadata_only_direct || runtime_linked_exe || (direct_emitter && strcmp(direct_emitter, "none") != 0 &&
    ...
    bool uses_zerox_runtime = input && input->direct_host_runtime_import_count > 0;
    bool uses_http_runtime = input && input->direct_http_runtime_import_count > 0;
    bool links_zerox_runtime = uses_zerox_runtime;
    ...
    const char *runtime_external_toolchain = links_zerox_runtime ? public_compiler_label(&runtime_toolchain) : "none";
    ...
    zbuf_appendf(buf, ",\"objectEmission\":{\"path\":\"%s\",\"functions\":true,\"dataSections\":%s,\"symbols\":%s,\"relocations\":\"%s\",\"symbolCount\":%zu,\"internalHelperCount\":%zu}",
                 ...
                 "true",
                 uses_zerox_runtime ? "patched-runtime-import-relocations" : (direct_has_data ? "patched-internal-calls-and-data-relocations" : "patched-internal-calls-or-none-in-mvp"),
                 direct_symbol_count,
    ...
    append_json_string(buf, links_http_runtime ? "zerox-runtime,curl" : (uses_zerox_runtime ? "zerox-runtime" : "none"));
    ...
    append_json_string(buf, uses_zerox_runtime ? "direct-backend-runtime-link-plan" : "direct-backend");
    ...
    zbuf_append(buf, links_http_runtime ? "[\"zerox_runtime.o\",\"zerox_http_curl.o\"]" : (links_zerox_runtime ? "[\"zerox_runtime.o\"]" : "[]"));
    ...
    append_json_string(buf, links_zerox_runtime ? runtime_toolchain.libc_mode : "none");
...
static bool write_ship_artifacts(const Command *command, const SourceInput *input, const ZTargetInfo *target, const ZBuf *exe, const char *exe_file, ShipArtifacts *artifacts, ZDiag *diag) {
  ...
  artifacts->archive_path = path_with_suffix(exe_file, ".zeroxar");
  ...
  ZBuf debug;
  zbuf_init(&debug);
  zbuf_append(&debug, "{\n  \"schemaVersion\": 1,\n  \"kind\": \"zerox-debug-symbol-metadata\",\n  \"sourceFile\": ");
  ...
  ZBuf size;
  zbuf_init(&size);
  zbuf_append(&size, "{\n  \"schemaVersion\": 1,\n  \"kind\": \"zerox-release-size-report\",\n  \"sourceFile\": ");
  ...
  ZBuf archive;
  zbuf_init(&archive);
  zbuf_append(&archive, "zerox archive manifest v1\n");
...
static void print_ship_json(const Command *command, const SourceInput *input, const Program *program, const ZTargetInfo *target, const ShipArtifacts *artifacts, long long elapsed_ms) {
  ...
  zbuf_append(&buf, ", ");
  append_ship_artifact_json(&buf, "archive", artifacts ? artifacts->archive_path : "", artifacts ? artifacts->archive_bytes : 0, "zerox-archive-manifest-v1");
  zbuf_append(&buf, ", ");
...
static int print_version_command(bool json) {
  ...
  const char *zerox_cc = getenv("ZEROX_CC");
  bool target_cc_override = zerox_cc && zerox_cc[0];
  ...
  char *target_cc_version = target_cc_override ? z_strdup("configured via ZEROX_CC") : (bundled_target_cc_available ? command_first_line("zig version 2>/dev/null") : z_strdup(""));
  ...
  append_json_string(&buf, ZEROX_VERSION);
  zbuf_append(&buf, ",\n  \"commit\": ");
  append_json_string(&buf, zerox_commit());
  zbuf_append(&buf, ",\n  \"host\": ");
  append_json_string(&buf, z_host_target());
  zbuf_append(&buf, ",\n  \"backend\": \"zerox-c\",\n  \"targets\": ");
...
static bool create_cli_template(const char *root, const char *name, ZDiag *diag) {
  ...
  if (!write_project_buf(root, "zerox.json", &manifest, diag)) return false;
  ...
  if (!write_project_file(root, "README.md",
    ...
    "zerox run .\n"
    "zerox dev --json .\n"
    "zerox build --target linux-musl-x64 --out .zero/out/app .\n"
    "zerox ship --target linux-musl-x64 --out .zero/ship/app .\n"
    "```\n\n"
    ...
    diag)) return false;
  return write_project_file(root, ".gitignore", ".zerox/\n", diag);
}
...
static bool create_lib_template(const char *root, const char *name, ZDiag *diag) {
  ...
  if (!write_project_file(root, "README.md",
    "# Zerox Library\n\n"
    "This small package exposes one public function, docs metadata in `zerox.json`, and an inline test.\n\n"
    "Try:\n\n"
    "```sh\n"
    "zerox check .\n"
    "zerox test .\n"
...
static int new_command(const Command *command) {
  ...
  if (!command->kind || !command->input) {
    fprintf(stderr, "usage: zerox new cli|lib|package <name>\n");
    return 1;
  }
  if (path_exists(command->input)) {
    fprintf(stderr, "zerox new: '%s' already exists\n", command->input);
    return 1;
  }
  ...
  if (strcmp(command->kind, "cli") == 0) ok = create_cli_template(command->input, command->input, &diag);
    ...
    fprintf(stderr, "zerox new: unknown template '%s'\n", command->kind);
  ...
  if (strcmp(command->kind, "lib") == 0) printf("next: cd %s && zerox check . && zerox test .\n", command->input);
  else printf("next: cd %s && zerox check . && zerox test . && zerox run .\n", command->input);
  return 0;
}

static int clean_command(const Command *command) {
  const char *default_paths[] = {".zerox/out", NULL};
  const char *all_paths[] = {
    ".zerox/out",
    ".zerox/native-test",
    ".zerox/conformance",
    ".zerox/command-contracts",
    ".zerox/generated-c-audit",
    ".zerox/bench",
    NULL
  };
  ...
  for (size_t i = 0; paths[i]; i++) {
    if (!remove_tree(paths[i], &deleted)) {
      fprintf(stderr, "zerox clean: failed to remove %s: %s\n", paths[i], strerror(errno));
      zbuf_free(&deleted);
      return 1;
    }
  }
  if (deleted.len == 0) {
    ...
    printf("removed:\n%s\n", deleted.data);
    if (command->all) printf("preserved .zero/bin so the local zerox wrapper still works\n");
  }
  zbuf_free(&deleted);
  return 0;
}
...
static void doctor_target_toolchain_status(const ZTargetInfo *target, const ZToolchainPlan *plan, bool cc_ok, bool target_cc_ok, const char **status, ZBuf *message) {
  ...
  if (strcmp(plan->driver_kind, "host-cc") == 0 && !cc_ok) {
    *status = "error";
    zbuf_append(message, "host cc is missing; install a native C compiler or pass --cc/ZEROX_CC");
    return;
  }

  if (strcmp(plan->driver_kind, "zig-cc") == 0 && !target_cc_ok) {
    *status = "warning";
    zbuf_appendf(message, "target-capable C compiler is required for %s; pass --cc/ZEROX_CC or install the bundled-toolchain default", target->name);
    return;
  }
...
static int doctor_command(bool json) {
  ...
  bool zerox_dir_ok = zerox_mkdir(".zerox") == 0 || errno == EEXIST;
  bool write_ok = false;
  if (zerox_dir_ok) {
    ZDiag diag = {0};
    write_ok = z_write_file(".zerox/doctor.tmp", "ok\n", &diag);
    unlink(".zerox/doctor.tmp");
  }
  ...
  overall = worse_status(overall, (zerox_dir_ok && write_ok) ? "ok" : "error");
  ...
  char *target_cc_message = target_cc_override ? z_strdup("configured via ZEROX_CC") : (bundled_target_cc_ok ? command_first_line("zig version 2>/dev/null") : z_strdup("target-capable C compiler not found; cross-target executable builds may be unavailable"));
  ...
  if (json) {
    ...
    append_doctor_check_json(&buf, &first, "cross-executable-builds", target_cc_ok ? "ok" : "warning", target_cc_ok ? "target-capable C compiler available for non-host executable builds" : "pass --cc/ZEROX_CC or install a target-capable C compiler for non-host executable builds");
    ...
    append_doctor_check_json(&buf, &first, ".zerox-write", (zerox_dir_ok && write_ok) ? "ok" : "error", (zerox_dir_ok && write_ok) ? ".zerox is writable" : ".zerox is not writable");
    append_doctor_check_json(&buf, &first, "targets", target_ok ? "ok" : "warning", "run `zerox targets` for the supported target list");
    ...
    zbuf_appendf(&target_toolchain_message, "%zu ready, %zu warnings, %zu errors; run `zerox doctor --json` for per-target details", target_toolchains_ok, target_toolchains_warning, target_toolchains_error);
    printf("zerox doctor: %s\n", overall);
    ...
    printf("cross executable builds: %s (%s)\n", target_cc_ok ? "ok" : "warning", target_cc_ok ? "target-capable C compiler available" : "pass --cc/ZEROX_CC or install a target-capable C compiler");
    ...
    printf(".zerox write: %s\n", (zerox_dir_ok && write_ok) ? "ok" : "error");
    printf("targets: %s (run `zerox targets`)\n", target_ok ? "ok" : "warning");
...
static void test_fail(TestRunFailure *failure, const Expr *expr, const char *message) {
  ...
  snprintf(failure->message, sizeof(failure->message), "%s%s%s",
           message ? message : "zerox test expectation failed",
      ...
      *returned = true;
      ...
      if (stmt->expr && stmt->expr->kind == EXPR_CALL) {
        ...
        if (named && strcmp(name.data ? name.data : "", "expect") == 0 && stmt->expr->args.len == 1) {
          ...
          zbuf_free(&name);
          if (!passed) {
            test_fail(failure, stmt->expr, "zerox test expectation failed");
            return false;
          }
          continue;
        }
      ...
      test_fail(failure, NULL, "zerox test direct runner does not support this statement yet");
      return false;
    }
  }
  return true;
...
static bool test_eval_function(const Program *program, const Function *fun, const TestValue *args, size_t arg_len, TestValue *out, TestRunFailure *failure) {
  if (!fun) {
    test_fail(failure, NULL, "zerox test unknown function");
    return false;
  }
  if (fun->params.len != arg_len) {
    test_fail(failure, NULL, "zerox test function argument count mismatch");
    return false;
  }
  TestEnv local = {0};
...
static bool test_eval_expr(const Program *program, TestEnv *env, const Expr *expr, TestValue *out, TestRunFailure *failure) {
  ...
  switch (expr->kind) {
    ...
    case EXPR_IDENT:
      ...
      test_fail(failure, expr, "zerox test unknown identifier");
      return false;
    ...
    case EXPR_MEMBER: {
      ...
      test_value_free(&base);
      test_fail(failure, expr, "zerox test unknown field");
      return false;
    }
    case EXPR_BINARY: {
      TestValue left = {0};
      ...
      if (strcmp(op, "==") == 0 || strcmp(op, "!=") == 0) {
        ...
        if (strcmp(op, "+") == 0) out->int_value = a + b;
          ...
          test_fail(failure, expr, "zerox test unsupported operator");
          test_value_free(&left);
          test_value_free(&right);
          return false;
        }
    ...
    case EXPR_CALL: {
      ...
      if (!test_expr_name(expr->left, &name)) {
        zbuf_free(&name);
        test_fail(failure, expr, "zerox test unsupported callee");
        return false;
      }
    ...
    default:
      test_fail(failure, expr, "zerox test direct runner does not support this expression yet");
      return false;
  }
}
...
static int run_tests_direct(const Command *command, const SourceInput *input, const Program *program, const ZTargetInfo *target) {
  ...
  for (size_t i = 0; program && i < program->functions.len; i++) {
    ...
    if (expected_failure && ok) {
      ...
      if (!first_failure.message[0]) {
        ...
        snprintf(first_failure.message, sizeof(first_failure.message), "zerox test expected failure unexpectedly passed: %s", failure.current_test ? failure.current_test : fun->name);
      }
      ...
      status = "failed";
      failed++;
      failure_message = failure.message[0] ? failure.message : "zerox test failed";
  ...
  if (command && command->json) {
    ...
    zbuf_append(&buf, ",\"snapshotKey\":\"zerox-test-direct-frontend-v1\"}");
    CapabilitySummary caps = program_capabilities(program);
...
static const char *function_retention_reason(const Function *fun) {
  ...
  if (fun->is_test) return "zerox test discovery";
...
static void manifest_path_for_input(const SourceInput *input, char *manifest_path, size_t manifest_path_len) {
  ...
  if (src_dir) snprintf(manifest_path, manifest_path_len, "%.*s/zerox.json", (int)(src_dir - src), src);
  else snprintf(manifest_path, manifest_path_len, "zerox.json");
}
...
static void append_self_host_routing_json(ZBuf *buf, const char *command_name, const char *emit_kind, const Program *program, const CapabilitySummary *caps, const ZTargetInfo *target) {
  ...
  zbuf_append(buf, ",\"phases\":{\"parse\":\"zerox-c\",\"check\":\"zerox-c\",\"lower\":\"zerox-c\",\"emit\":\"zerox-c\"}");
...
static bool target_readiness_buildability_check(const Command *command, const ZTargetInfo *target, const IrProgram *ir, ZDiag *diag) {
  EmitKind emit = command ? command->emit : EMIT_EXE;
  const char *emit_kind = (emit == EMIT_EXE && ir && ir_needs_zerox_runtime_object(ir)) ? "obj" : emit_kind_name(emit);
...
static bool target_readiness_select_diag(const Command *command, const SourceInput *input, const Program *program, const ZTargetInfo *target, const IrProgram *ir, ZDiag *diag) {
  ...
  if (ir && ir_needs_zerox_runtime_object(ir)) {
    RuntimeImportAudit audit = runtime_import_audit_from_ir(ir);
...
static void append_target_readiness_json(ZBuf *buf, SourceInput *input, const Program *program, const ZTargetInfo *target, const Command *command) {
  ...
  if (!ready && input) {
    /* C library validation points at zerox.json; source mapping would erase that. */
...
int main(int argc, char **argv) {
  ...
  if (command.unknown_flag) {
    ...
    fprintf(stderr, "help: run `zerox %s --help`", command.command);
  ...
  if (!z_is_known_target(command.target) || !target) {
    ...
    snprintf(diag.expected, sizeof(diag.expected), "one of zerox targets");
    snprintf(diag.actual, sizeof(diag.actual), "%s", command.target);
    snprintf(diag.help, sizeof(diag.help), "run zerox targets and choose a supported target name");
    if (command.json) print_diag_json(command.input, &diag);
    else print_diag(command.input, &diag);
    return 1;
  }
  ...
  if (command.legacy_backend || command.emit == EMIT_C) {
    ...
    snprintf(diag.expected, sizeof(diag.expected), "zerox build --emit exe|obj <input>");
  ...
  if (strcmp(command.command, "run") == 0) {
    if (command.json) {
      ...
      diag.column = 1;
      diag.length = 1;
      snprintf(diag.message, sizeof(diag.message), "zerox run does not support --json");
      snprintf(diag.expected, sizeof(diag.expected), "zerox run <input>");
      snprintf(diag.actual, sizeof(diag.actual), "zerox run --json");
      snprintf(diag.help, sizeof(diag.help), "program stdout belongs to the program; use zerox build --json to inspect the artifact before running it");
    ...
    if (command.emit != EMIT_EXE) {
      ...
      diag.column = 1;
      diag.length = 1;
      snprintf(diag.message, sizeof(diag.message), "zerox run only supports executable output");
      snprintf(diag.expected, sizeof(diag.expected), "zerox run <input>");
      snprintf(diag.actual, sizeof(diag.actual), "--emit obj");
      snprintf(diag.help, sizeof(diag.help), "use zerox build --emit obj when you need a non-executable artifact");
      print_diag(command.input, &diag);
      return 1;
    }
    if (!z_target_is_host(target)) {
      ...
      diag.column = 1;
      diag.length = 1;
      snprintf(diag.message, sizeof(diag.message), "zerox run requires the host target");
      ...
      snprintf(diag.help, sizeof(diag.help), "use zerox build --target %s for cross-target artifacts, then run them on a matching host", target && target->name ? target->name : "<target>");
      print_diag(command.input, &diag);
      return 1;
    }
  }
  ...
  if (strcmp(command.command, "fix") == 0 && !command.plan && !command.apply && !command.patch) {
    ...
    snprintf(diag.expected, sizeof(diag.expected), "zerox fix --plan, --patch, or --apply --json <input>");
    snprintf(diag.actual, sizeof(diag.actual), "zerox fix without a mode");
  ...
  if (command.emit == EMIT_OBJ) {
    if (strcmp(command.command, "build") != 0) {
      fprintf(stderr, "--emit obj is currently supported only by zerox build\n");
      z_free_ir_program(&ir);
      z_free_program(&program);
      z_free_source(&input);
      return 1;
  ...
  bool needs_zerox_runtime = artifact_command && command.emit == EMIT_EXE && ir_needs_zerox_runtime_object(&ir);
  if (needs_zerox_runtime) {
    ...
    bool needs_crypto_runtime = runtime_import_audit_uses_crypto(&runtime_audit);
    ZDirectBackend object_backend = z_direct_object_backend(target);
    bool runtime_object_emitter_supported = z_direct_backend_supports_runtime_object(object_backend);
    if (ship_command) {
      int rc = return_direct_backend_error(&command, &input, target, "exe", "host runtime link plan is not wired into ship yet; use zerox build or zerox run", &ir, &program);
      z_free_source(&input);
      AI_RETURN(rc);
    }
    ...
    free(base_exe_file);
    char *object_file = path_with_suffix(exe_file, ".zerox.o");
    char *runtime_object_file = path_with_suffix(exe_file, ".zerox-runtime.o");
    char *crypto_object_file = needs_crypto_runtime ? path_with_suffix(exe_file, ".zerox-crypto.o") : NULL;
    char *http_object_file = needs_http_runtime ? path_with_suffix(exe_file, ".zerox-http-curl.o") : NULL;
    ...
    if (wrote_object && needs_http_runtime) wrote_object = compile_zerox_http_curl_object(http_object_file, &runtime_toolchain, &command, target, &diag);
    input.object_ms = now_ms() - phase_started;
    if (!wrote_object) {
    ...
    phase_started = now_ms();
    bool linked = link_zerox_runtime_executable(object_file, runtime_object_file, crypto_object_file, http_object_file, exe_file, &runtime_toolchain, target, &diag);
    if (linked) chmod(exe_file, 0755);
    input.link_ms = now_ms() - phase_started;
    remove(object_file);
    remove(runtime_object_file);
    if (crypto_object_file) remove(crypto_object_file);
