  ...
  switch (value->kind) {
    ...
    case IR_VALUE_BINARY:
      ...
      if (value->binary_op != IR_BIN_ADD && value->binary_op != IR_BIN_SUB && value->binary_op != IR_BIN_MUL &&
        ...
        return macho_diag_at(diag, "direct AArch64 Mach-O binary operator is unsupported", value->line, value->column, "unsupported operator");
    ...
    case IR_VALUE_CRYPTO_HELPER: {
      MachORuntimeHelper helper;
      switch (value->error_code) {
        case CRYPTO_SHA256: helper = MACHO_RUNTIME_CRYPTO_SHA256; break;
        case CRYPTO_HMAC_SHA256: helper = MACHO_RUNTIME_CRYPTO_HMAC_SHA256; break;
        case CRYPTO_AES_ENCRYPT: helper = MACHO_RUNTIME_CRYPTO_AES_ENCRYPT; break;
        case CRYPTO_AES_DECRYPT: helper = MACHO_RUNTIME_CRYPTO_AES_DECRYPT; break;
        case CRYPTO_CHACHA20: helper = MACHO_RUNTIME_CRYPTO_CHACHA20; break;
        case CRYPTO_PBKDF2: helper = MACHO_RUNTIME_CRYPTO_PBKDF2; break;
        case CRYPTO_RANDOM_BYTES: helper = MACHO_RUNTIME_CRYPTO_RANDOM_BYTES; break;
        case CRYPTO_SHA512: helper = MACHO_RUNTIME_CRYPTO_SHA512; break;
        case CRYPTO_SALSA20: helper = MACHO_RUNTIME_CRYPTO_SALSA20; break;
        case CRYPTO_CHACHA20_POLY1305_ENCRYPT: helper = MACHO_RUNTIME_CRYPTO_CHACHA20_POLY1305_ENCRYPT; break;
        case CRYPTO_CHACHA20_POLY1305_DECRYPT: helper = MACHO_RUNTIME_CRYPTO_CHACHA20_POLY1305_DECRYPT; break;
        case CRYPTO_DES_ENCRYPT: helper = MACHO_RUNTIME_CRYPTO_DES_ENCRYPT; break;
        case CRYPTO_DES_DECRYPT: helper = MACHO_RUNTIME_CRYPTO_DES_DECRYPT; break;
        case CRYPTO_TDES_ENCRYPT: helper = MACHO_RUNTIME_CRYPTO_TDES_ENCRYPT; break;
        case CRYPTO_TDES_DECRYPT: helper = MACHO_RUNTIME_CRYPTO_TDES_DECRYPT; break;
        case CRYPTO_BLOWFISH_ENCRYPT: helper = MACHO_RUNTIME_CRYPTO_BLOWFISH_ENCRYPT; break;
        case CRYPTO_BLOWFISH_DECRYPT: helper = MACHO_RUNTIME_CRYPTO_BLOWFISH_DECRYPT; break;
        case CRYPTO_TWOFISH_ENCRYPT: helper = MACHO_RUNTIME_CRYPTO_TWOFISH_ENCRYPT; break;
        case CRYPTO_TWOFISH_DECRYPT: helper = MACHO_RUNTIME_CRYPTO_TWOFISH_DECRYPT; break;
        case CRYPTO_SERPENT_ENCRYPT: helper = MACHO_RUNTIME_CRYPTO_SERPENT_ENCRYPT; break;
        case CRYPTO_SERPENT_DECRYPT: helper = MACHO_RUNTIME_CRYPTO_SERPENT_DECRYPT; break;
        case CRYPTO_CAMELLIA_ENCRYPT: helper = MACHO_RUNTIME_CRYPTO_CAMELLIA_ENCRYPT; break;
        case CRYPTO_CAMELLIA_DECRYPT: helper = MACHO_RUNTIME_CRYPTO_CAMELLIA_DECRYPT; break;
        case CRYPTO_RSA_GENERATE_KEYPAIR: helper = MACHO_RUNTIME_CRYPTO_RSA_GENERATE_KEYPAIR; break;
        case CRYPTO_RSA_ENCRYPT: helper = MACHO_RUNTIME_CRYPTO_RSA_ENCRYPT; break;
        case CRYPTO_RSA_DECRYPT: helper = MACHO_RUNTIME_CRYPTO_RSA_DECRYPT; break;
        case CRYPTO_RSA_SIGN: helper = MACHO_RUNTIME_CRYPTO_RSA_SIGN; break;
        case CRYPTO_RSA_VERIFY: helper = MACHO_RUNTIME_CRYPTO_RSA_VERIFY; break;
        case CRYPTO_ECC_GENERATE_KEYPAIR: helper = MACHO_RUNTIME_CRYPTO_ECC_GENERATE_KEYPAIR; break;
        case CRYPTO_ECC_SIGN: helper = MACHO_RUNTIME_CRYPTO_ECC_SIGN; break;
        case CRYPTO_ECC_VERIFY: helper = MACHO_RUNTIME_CRYPTO_ECC_VERIFY; break;
        case CRYPTO_ECC_ECDH: helper = MACHO_RUNTIME_CRYPTO_ECC_ECDH; break;
        case CRYPTO_ECC_ED25519_SIGN: helper = MACHO_RUNTIME_CRYPTO_ECC_ED25519_SIGN; break;
        case CRYPTO_ECC_ED25519_VERIFY: helper = MACHO_RUNTIME_CRYPTO_ECC_ED25519_VERIFY; break;
        default: return macho_diag_at(diag, "direct AArch64 Mach-O unsupported crypto operation", value->line, value->column, "unknown crypto kind");
      }
      ...
      for (size_t i = 0; i < value->arg_len && reg_idx < 8; i++) {
        ...
        if (arg->type == IR_TYPE_BYTE_VIEW || arg->type == IR_TYPE_MAYBE_BYTE_VIEW) {
          ...
          if (!macho_emit_byte_view_ptr_at(text, fun, arg, (unsigned)reg_idx, frame_size, scratch_slot + (unsigned)reg_idx, ctx, diag)) return false;
          if ((unsigned)reg_idx != reg_idx) {} /* cast warning guard */
          reg_idx++;
          if (reg_idx >= 8) break;
          if (!macho_emit_byte_view_len_at(text, fun, arg, (unsigned)reg_idx, frame_size, scratch_slot + (unsigned)reg_idx, ctx, diag)) return false;
          reg_idx++;
        } else {
          /* Scalar: load value into reg */
          if (!macho_emit_value_to_reg_at(text, fun, arg, (unsigned)reg_idx, frame_size, scratch_slot + (unsigned)reg_idx, ctx, diag)) return false;
          reg_idx++;
        }
      }
      size_t patch = z_aarch64_emit_bl_placeholder(text);
