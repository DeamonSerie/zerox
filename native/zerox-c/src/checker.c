  ...
  switch (expr->kind) {
    ...
    case EXPR_BINARY: {
      ...
      if (!((is_int_type(left_type) && is_int_type(right_type)) || (is_float_type(left_type) && is_float_type(right_type)))) {
        bool bitwise = strcmp(expr->text, "^") == 0 || strcmp(expr->text, "<<") == 0 || strcmp(expr->text, ">>") == 0 || strcmp(expr->text, "<<<") == 0 || strcmp(expr->text, ">>>") == 0;
        return set_diag_detail(diag, 3006, bitwise ? "bitwise operators require integer operands" : "arithmetic operators require matching numeric operands", expr->line, expr->column, "matching integer or float operands", is_char_type(left_type) || is_char_type(right_type) ? "char operand" : "mixed or non-numeric operand", "keep integers, floats, and chars separate");
      }
