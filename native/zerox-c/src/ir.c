...
static bool ir_binary_op(const char *text, IrBinaryOp *out) {
  ...
  else if (strcmp(text, "^") == 0) *out = IR_BIN_XOR;
  else if (strcmp(text, "<<") == 0) *out = IR_BIN_SHL;
  else if (strcmp(text, ">>") == 0) *out = IR_BIN_SHR;
  else if (strcmp(text, "<<<") == 0) *out = IR_BIN_ROL;
  else if (strcmp(text, ">>>") == 0) *out = IR_BIN_ROR;
  else return false;
  return true;
}
...
static bool ir_lower_expr(const Program *program, IrProgram *ir, const IrFunction *fun, const Expr *expr, IrValue **out) {
  ...
  switch (expr->kind) {
    ...
    case EXPR_CALL: {
      ...
      /* std.crypto.hash.sha256: (data: Span<u8>, out: MutSpan<u8>, world: World) -> u32 */
      if (strcmp(callee_name, "std.crypto.hash.sha256") == 0 && expr->args.len == 3) {
        IrValue *view = NULL;
        ...
        value->error_code = CRYPTO_SHA256;
      ...
      /* std.crypto.hash.sha512: (data: Span<u8>, out: MutSpan<u8>, world: World) -> u32 */
      if (strcmp(callee_name, "std.crypto.hash.sha512") == 0 && expr->args.len == 3) {
        IrValue *view = NULL;
        IrValue *out_view = NULL;
        if (!ir_lower_byte_view(program, ir, fun, expr->args.items[0], &view)) {
          ir_free_value(view); free(callee_name); return false;
        ...
        IrValue *value = ir_new_value(ir, IR_VALUE_CRYPTO_HELPER, IR_TYPE_U32, expr->line, expr->column);
        value->error_code = CRYPTO_SHA512;
        ir_value_push_arg(ir, value, view);
        ir_value_push_arg(ir, value, out_view);
        free(callee_name);
        *out = value;
      ...
      if (strcmp(callee_name, "std.crypto.hash.hmacSha256") == 0 && expr->args.len == 4) {
        ...
        IrValue *value = ir_new_value(ir, IR_VALUE_CRYPTO_HELPER, IR_TYPE_U32, expr->line, expr->column);
        value->error_code = CRYPTO_HMAC_SHA256;
        ir_value_push_arg(ir, value, key_view);
        ir_value_push_arg(ir, value, data_view);
        ir_value_push_arg(ir, value, out_view);
        free(callee_name);
      ...
      /* std.crypto.chacha.chacha20: (key, nonce, data, counter, out, world) -> u32 */
      if (strcmp(callee_name, "std.crypto.chacha.chacha20") == 0 && expr->args.len == 6) {
        IrValue *key_view = NULL, *nonce_view = NULL, *data_view = NULL, *counter_val = NULL, *out_view = NULL;
        if (!ir_lower_byte_view(program, ir, fun, expr->args.items[0], &key_view) ||
            !ir_lower_byte_view(program, ir, fun, expr->args.items[1], &nonce_view) ||
            !ir_lower_byte_view(program, ir, fun, expr->args.items[2], &data_view) ||
            !ir_lower_expr(program, ir, fun, expr->args.items[3], &counter_val) ||
            !ir_lower_byte_view(program, ir, fun, expr->args.items[4], &out_view)) {
          ir_free_value(key_view); ir_free_value(nonce_view); ir_free_value(data_view);
          ir_free_value(counter_val); ir_free_value(out_view);
          free(callee_name); return false;
        }
        IrValue *value = ir_new_value(ir, IR_VALUE_CRYPTO_HELPER, IR_TYPE_U32, expr->line, expr->column);
        value->error_code = CRYPTO_CHACHA20;
        ir_value_push_arg(ir, value, key_view);
        ir_value_push_arg(ir, value, nonce_view);
        ir_value_push_arg(ir, value, data_view);
        ir_value_push_arg(ir, value, counter_val);
        ir_value_push_arg(ir, value, out_view);
        free(callee_name);
        *out = value;
        return true;
      }
      /* std.crypto.chacha.salsa20: (key, nonce, data, counter, out, world) -> u32 */
      if (strcmp(callee_name, "std.crypto.chacha.salsa20") == 0 && expr->args.len == 6) {
        IrValue *key_view = NULL, *nonce_view = NULL, *data_view = NULL, *counter_val = NULL, *out_view = NULL;
        if (!ir_lower_byte_view(program, ir, fun, expr->args.items[0], &key_view) ||
            !ir_lower_byte_view(program, ir, fun, expr->args.items[1], &nonce_view) ||
            !ir_lower_byte_view(program, ir, fun, expr->args.items[2], &data_view) ||
        ...
        IrValue *value = ir_new_value(ir, IR_VALUE_CRYPTO_HELPER, IR_TYPE_U32, expr->line, expr->column);
        value->error_code = CRYPTO_SALSA20;
        ir_value_push_arg(ir, value, key_view);
        ir_value_push_arg(ir, value, nonce_view);
        ir_value_push_arg(ir, value, data_view);
        ir_value_push_arg(ir, value, counter_val);
      ...
      /* std.crypto.chacha.chacha20Poly1305Encrypt: (key, nonce, data, aad, out, world) -> u32 */
      if (strcmp(callee_name, "std.crypto.chacha.chacha20Poly1305Encrypt") == 0 && expr->args.len == 6) {
        IrValue *key_view = NULL, *nonce_view = NULL, *data_view = NULL, *aad_view = NULL, *out_view = NULL;
        if (!ir_lower_byte_view(program, ir, fun, expr->args.items[0], &key_view) ||
            !ir_lower_byte_view(program, ir, fun, expr->args.items[1], &nonce_view) ||
            !ir_lower_byte_view(program, ir, fun, expr->args.items[2], &data_view) ||
        ...
        IrValue *value = ir_new_value(ir, IR_VALUE_CRYPTO_HELPER, IR_TYPE_U32, expr->line, expr->column);
        value->error_code = CRYPTO_CHACHA20_POLY1305_ENCRYPT;
        ir_value_push_arg(ir, value, key_view);
        ir_value_push_arg(ir, value, nonce_view);
        ir_value_push_arg(ir, value, data_view);
      ...
      /* std.crypto.chacha.chacha20Poly1305Decrypt: (key, nonce, data, aad, out, world) -> u32 */
      if (strcmp(callee_name, "std.crypto.chacha.chacha20Poly1305Decrypt") == 0 && expr->args.len == 6) {
        IrValue *key_view = NULL, *nonce_view = NULL, *data_view = NULL, *aad_view = NULL, *out_view = NULL;
        if (!ir_lower_byte_view(program, ir, fun, expr->args.items[0], &key_view) ||
            !ir_lower_byte_view(program, ir, fun, expr->args.items[1], &nonce_view) ||
            !ir_lower_byte_view(program, ir, fun, expr->args.items[2], &data_view) ||
        ...
        IrValue *value = ir_new_value(ir, IR_VALUE_CRYPTO_HELPER, IR_TYPE_U32, expr->line, expr->column);
        value->error_code = CRYPTO_CHACHA20_POLY1305_DECRYPT;
        ir_value_push_arg(ir, value, key_view);
        ir_value_push_arg(ir, value, nonce_view);
        ir_value_push_arg(ir, value, data_view);
        ir_value_push_arg(ir, value, aad_view);
      ...
      /* std.crypto.aes.encrypt / decrypt: (key, iv, data, mode, out, world) -> u32 */
      if ((strcmp(callee_name, "std.crypto.aes.encrypt") == 0 || strcmp(callee_name, "std.crypto.aes.decrypt") == 0) && expr->args.len == 6) {
        IrValue *key_view = NULL, *iv_view = NULL, *data_view = NULL, *mode_val = NULL, *out_view = NULL;
        if (!ir_lower_byte_view(program, ir, fun, expr->args.items[0], &key_view) ||
            !ir_lower_byte_view(program, ir, fun, expr->args.items[1], &iv_view) ||
            !ir_lower_byte_view(program, ir, fun, expr->args.items[2], &data_view) ||
            !ir_lower_expr(program, ir, fun, expr->args.items[3], &mode_val) ||
            !ir_lower_byte_view(program, ir, fun, expr->args.items[4], &out_view)) {
          ir_free_value(key_view); ir_free_value(iv_view); ir_free_value(data_view);
          ir_free_value(mode_val); ir_free_value(out_view);
          free(callee_name); return false;
        }
        IrValue *value = ir_new_value(ir, IR_VALUE_CRYPTO_HELPER, IR_TYPE_U32, expr->line, expr->column);
        value->error_code = strcmp(callee_name, "std.crypto.aes.encrypt") == 0 ? CRYPTO_AES_ENCRYPT : CRYPTO_AES_DECRYPT;
        ir_value_push_arg(ir, value, key_view);
      ...
      /* std.crypto.key.randomBytes: (size, out, world) -> u32 */
      if (strcmp(callee_name, "std.crypto.key.randomBytes") == 0 && expr->args.len == 3) {
        IrValue *size = NULL, *out_view = NULL;
        if (!ir_lower_expr(program, ir, fun, expr->args.items[0], &size) ||
            !ir_lower_byte_view(program, ir, fun, expr->args.items[1], &out_view)) {
          ir_free_value(size); ir_free_value(out_view); free(callee_name); return false;
        }
        IrValue *value = ir_new_value(ir, IR_VALUE_CRYPTO_HELPER, IR_TYPE_U32, expr->line, expr->column);
        value->error_code = CRYPTO_RANDOM_BYTES;
        ir_value_push_arg(ir, value, out_view);
        free(callee_name);
        *out = value;
        return true;
      }
      /* std.crypto.key.derive: (password, salt, iterations, outLen, out, world) -> u32 */
      if (strcmp(callee_name, "std.crypto.key.derive") == 0 && expr->args.len == 6) {
        IrValue *pass_view = NULL, *salt_view = NULL, *iterations = NULL, *out_len = NULL, *out_view = NULL;
        if (!ir_lower_byte_view(program, ir, fun, expr->args.items[0], &pass_view) ||
            !ir_lower_byte_view(program, ir, fun, expr->args.items[1], &salt_view) ||
            !ir_lower_expr(program, ir, fun, expr->args.items[2], &iterations) ||
            !ir_lower_expr(program, ir, fun, expr->args.items[3], &out_len) ||
            !ir_lower_byte_view(program, ir, fun, expr->args.items[4], &out_view)) {
          ir_free_value(pass_view); ir_free_value(salt_view); ir_free_value(iterations);
          ir_free_value(out_len); ir_free_value(out_view);
          free(callee_name); return false;
        }
        IrValue *value = ir_new_value(ir, IR_VALUE_CRYPTO_HELPER, IR_TYPE_U32, expr->line, expr->column);
        value->error_code = CRYPTO_PBKDF2;
        ir_value_push_arg(ir, value, pass_view);
        ir_value_push_arg(ir, value, salt_view);
        ir_value_push_arg(ir, value, iterations);
        ir_value_push_arg(ir, value, out_len);
        ir_value_push_arg(ir, value, out_view);
        free(callee_name);
        *out = value;
        return true;
      }
      /* std.crypto.des.encrypt / decrypt / tripleEncrypt / tripleDecrypt: (key, data, mode, out, world) -> u32 */
      if ((strcmp(callee_name, "std.crypto.des.encrypt") == 0 ||
           strcmp(callee_name, "std.crypto.des.decrypt") == 0 ||
           strcmp(callee_name, "std.crypto.des.tripleEncrypt") == 0 ||
           strcmp(callee_name, "std.crypto.des.tripleDecrypt") == 0) && expr->args.len == 5) {
        IrValue *key_view = NULL, *data_view = NULL, *mode_val = NULL, *out_view = NULL;
        if (!ir_lower_byte_view(program, ir, fun, expr->args.items[0], &key_view) ||
            !ir_lower_byte_view(program, ir, fun, expr->args.items[1], &data_view) ||
            !ir_lower_expr(program, ir, fun, expr->args.items[2], &mode_val) ||
            !ir_lower_byte_view(program, ir, fun, expr->args.items[3], &out_view)) {
          ir_free_value(key_view); ir_free_value(data_view); ir_free_value(mode_val); ir_free_value(out_view);
          free(callee_name); return false;
        }
        IrValue *value = ir_new_value(ir, IR_VALUE_CRYPTO_HELPER, IR_TYPE_U32, expr->line, expr->column);
        if (strcmp(callee_name, "std.crypto.des.encrypt") == 0) value->error_code = CRYPTO_DES_ENCRYPT;
        else if (strcmp(callee_name, "std.crypto.des.decrypt") == 0) value->error_code = CRYPTO_DES_DECRYPT;
        else if (strcmp(callee_name, "std.crypto.des.tripleEncrypt") == 0) value->error_code = CRYPTO_TDES_ENCRYPT;
        else value->error_code = CRYPTO_TDES_DECRYPT;
        ir_value_push_arg(ir, value, key_view);
        ir_value_push_arg(ir, value, data_view);
        ir_value_push_arg(ir, value, mode_val);
        ir_value_push_arg(ir, value, out_view);
      ...
      /* std.crypto.blowfish/twofish/serpent/camellia encrypt/decrypt: (key, iv, data, mode, out, world) -> u32 */
      if ((strcmp(callee_name, "std.crypto.blowfish.encrypt") == 0 ||
           strcmp(callee_name, "std.crypto.blowfish.decrypt") == 0 ||
           strcmp(callee_name, "std.crypto.twofish.encrypt") == 0 ||
           strcmp(callee_name, "std.crypto.twofish.decrypt") == 0 ||
           strcmp(callee_name, "std.crypto.serpent.encrypt") == 0 ||
           strcmp(callee_name, "std.crypto.serpent.decrypt") == 0 ||
           strcmp(callee_name, "std.crypto.camellia.encrypt") == 0 ||
           strcmp(callee_name, "std.crypto.camellia.decrypt") == 0) && expr->args.len == 6) {
        IrValue *key_view = NULL, *iv_view = NULL, *data_view = NULL, *mode_val = NULL, *out_view = NULL;
        if (!ir_lower_byte_view(program, ir, fun, expr->args.items[0], &key_view) ||
            !ir_lower_byte_view(program, ir, fun, expr->args.items[1], &iv_view) ||
            !ir_lower_byte_view(program, ir, fun, expr->args.items[2], &data_view) ||
        ...
        unsigned crypto_kind = 0;
        if (strcmp(callee_name, "std.crypto.blowfish.encrypt") == 0) crypto_kind = CRYPTO_BLOWFISH_ENCRYPT;
        else if (strcmp(callee_name, "std.crypto.blowfish.decrypt") == 0) crypto_kind = CRYPTO_BLOWFISH_DECRYPT;
        else if (strcmp(callee_name, "std.crypto.twofish.encrypt") == 0) crypto_kind = CRYPTO_TWOFISH_ENCRYPT;
        else if (strcmp(callee_name, "std.crypto.twofish.decrypt") == 0) crypto_kind = CRYPTO_TWOFISH_DECRYPT;
        else if (strcmp(callee_name, "std.crypto.serpent.encrypt") == 0) crypto_kind = CRYPTO_SERPENT_ENCRYPT;
        else if (strcmp(callee_name, "std.crypto.serpent.decrypt") == 0) crypto_kind = CRYPTO_SERPENT_DECRYPT;
        else if (strcmp(callee_name, "std.crypto.camellia.encrypt") == 0) crypto_kind = CRYPTO_CAMELLIA_ENCRYPT;
        else if (strcmp(callee_name, "std.crypto.camellia.decrypt") == 0) crypto_kind = CRYPTO_CAMELLIA_DECRYPT;
        IrValue *value = ir_new_value(ir, IR_VALUE_CRYPTO_HELPER, IR_TYPE_U32, expr->line, expr->column);
        value->error_code = crypto_kind;
        ir_value_push_arg(ir, value, key_view);
        ir_value_push_arg(ir, value, iv_view);
        ir_value_push_arg(ir, value, data_view);
        ir_value_push_arg(ir, value, mode_val);
      ...
      /* std.crypto.rsa.generateKeyPair: (keyBits, pubOut, privOut, world) -> u32 */
      if (strcmp(callee_name, "std.crypto.rsa.generateKeyPair") == 0 && expr->args.len == 4) {
        IrValue *key_bits = NULL, *pub_view = NULL, *priv_view = NULL;
        if (!ir_lower_expr(program, ir, fun, expr->args.items[0], &key_bits) ||
            !ir_lower_byte_view(program, ir, fun, expr->args.items[1], &pub_view) ||
            !ir_lower_byte_view(program, ir, fun, expr->args.items[2], &priv_view)) {
          ir_free_value(key_bits); ir_free_value(pub_view); ir_free_value(priv_view);
          free(callee_name); return false;
        }
        IrValue *value = ir_new_value(ir, IR_VALUE_CRYPTO_HELPER, IR_TYPE_U32, expr->line, expr->column);
        value->error_code = CRYPTO_RSA_GENERATE_KEYPAIR;
        ir_value_push_arg(ir, value, key_bits);
      ...
      /* std.crypto.rsa.encrypt / decrypt: (key, data, padding, out, world) -> u32 */
      if ((strcmp(callee_name, "std.crypto.rsa.encrypt") == 0 ||
           strcmp(callee_name, "std.crypto.rsa.decrypt") == 0) && expr->args.len == 5) {
        IrValue *key_view = NULL, *data_view = NULL, *padding_val = NULL, *out_view = NULL;
        if (!ir_lower_byte_view(program, ir, fun, expr->args.items[0], &key_view) ||
            !ir_lower_byte_view(program, ir, fun, expr->args.items[1], &data_view) ||
            !ir_lower_expr(program, ir, fun, expr->args.items[2], &padding_val) ||
            !ir_lower_byte_view(program, ir, fun, expr->args.items[3], &out_view)) {
          ir_free_value(key_view); ir_free_value(data_view); ir_free_value(padding_val); ir_free_value(out_view);
          free(callee_name); return false;
        }
        IrValue *value = ir_new_value(ir, IR_VALUE_CRYPTO_HELPER, IR_TYPE_U32, expr->line, expr->column);
        value->error_code = strcmp(callee_name, "std.crypto.rsa.encrypt") == 0 ? CRYPTO_RSA_ENCRYPT : CRYPTO_RSA_DECRYPT;
        ir_value_push_arg(ir, value, key_view);
        ir_value_push_arg(ir, value, data_view);
        ir_value_push_arg(ir, value, padding_val);
        ir_value_push_arg(ir, value, out_view);
        free(callee_name);
        *out = value;
        return true;
      }
      /* std.crypto.rsa.sign: (key, data, hashAlgo, padding, out, world) -> u32 */
      if (strcmp(callee_name, "std.crypto.rsa.sign") == 0 && expr->args.len == 6) {
        IrValue *key_view = NULL, *data_view = NULL, *hash_algo = NULL, *padding_val = NULL, *out_view = NULL;
        if (!ir_lower_byte_view(program, ir, fun, expr->args.items[0], &key_view) ||
            !ir_lower_byte_view(program, ir, fun, expr->args.items[1], &data_view) ||
            !ir_lower_expr(program, ir, fun, expr->args.items[2], &hash_algo) ||
            !ir_lower_expr(program, ir, fun, expr->args.items[3], &padding_val) ||
            !ir_lower_byte_view(program, ir, fun, expr->args.items[4], &out_view)) {
          ir_free_value(key_view); ir_free_value(data_view); ir_free_value(hash_algo);
          ir_free_value(padding_val); ir_free_value(out_view);
          free(callee_name); return false;
        }
        IrValue *value = ir_new_value(ir, IR_VALUE_CRYPTO_HELPER, IR_TYPE_U32, expr->line, expr->column);
        value->error_code = CRYPTO_RSA_SIGN;
        ir_value_push_arg(ir, value, key_view);
        ir_value_push_arg(ir, value, data_view);
        ir_value_push_arg(ir, value, hash_algo);
        ir_value_push_arg(ir, value, padding_val);
        ir_value_push_arg(ir, value, out_view);
        free(callee_name);
        *out = value;
      ...
      /* std.crypto.rsa.verify: (key, data, signature, hashAlgo, padding, world) -> Bool */
      if (strcmp(callee_name, "std.crypto.rsa.verify") == 0 && expr->args.len == 6) {
        IrValue *key_view = NULL, *data_view = NULL, *sig_view = NULL, *hash_algo = NULL, *padding_val = NULL;
        if (!ir_lower_byte_view(program, ir, fun, expr->args.items[0], &key_view) ||
            !ir_lower_byte_view(program, ir, fun, expr->args.items[1], &data_view) ||
            !ir_lower_byte_view(program, ir, fun, expr->args.items[2], &sig_view) ||
            !ir_lower_expr(program, ir, fun, expr->args.items[3], &hash_algo) ||
            !ir_lower_expr(program, ir, fun, expr->args.items[4], &padding_val)) {
          ir_free_value(key_view); ir_free_value(data_view); ir_free_value(sig_view);
          ir_free_value(hash_algo); ir_free_value(padding_val);
          free(callee_name); return false;
        }
        IrValue *value = ir_new_value(ir, IR_VALUE_CRYPTO_HELPER, IR_TYPE_BOOL, expr->line, expr->column);
        value->error_code = CRYPTO_RSA_VERIFY;
        ir_value_push_arg(ir, value, key_view);
        ir_value_push_arg(ir, value, data_view);
      ...
      /* std.crypto.ecc.generateKeyPair: (curve, pubOut, privOut, world) -> u32 */
      if (strcmp(callee_name, "std.crypto.ecc.generateKeyPair") == 0 && expr->args.len == 4) {
        IrValue *curve = NULL, *pub_view = NULL, *priv_view = NULL;
        if (!ir_lower_expr(program, ir, fun, expr->args.items[0], &curve) ||
            !ir_lower_byte_view(program, ir, fun, expr->args.items[1], &pub_view) ||
            !ir_lower_byte_view(program, ir, fun, expr->args.items[2], &priv_view)) {
          ir_free_value(curve); ir_free_value(pub_view); ir_free_value(priv_view);
          free(callee_name); return false;
        }
        IrValue *value = ir_new_value(ir, IR_VALUE_CRYPTO_HELPER, IR_TYPE_U32, expr->line, expr->column);
        value->error_code = CRYPTO_ECC_GENERATE_KEYPAIR;
        ir_value_push_arg(ir, value, curve);
        ir_value_push_arg(ir, value, pub_view);
        ir_value_push_arg(ir, value, priv_view);
        free(callee_name);
        *out = value;
      ...
      /* std.crypto.ecc.sign: (key, data, curve, out, world) -> u32 */
      if (strcmp(callee_name, "std.crypto.ecc.sign") == 0 && expr->args.len == 5) {
        IrValue *key_view = NULL, *data_view = NULL, *curve = NULL, *out_view = NULL;
        if (!ir_lower_byte_view(program, ir, fun, expr->args.items[0], &key_view) ||
            !ir_lower_byte_view(program, ir, fun, expr->args.items[1], &data_view) ||
            !ir_lower_expr(program, ir, fun, expr->args.items[2], &curve) ||
            !ir_lower_byte_view(program, ir, fun, expr->args.items[3], &out_view)) {
          ir_free_value(key_view); ir_free_value(data_view); ir_free_value(curve); ir_free_value(out_view);
          free(callee_name); return false;
        }
        IrValue *value = ir_new_value(ir, IR_VALUE_CRYPTO_HELPER, IR_TYPE_U32, expr->line, expr->column);
        value->error_code = CRYPTO_ECC_SIGN;
        ir_value_push_arg(ir, value, key_view);
        ir_value_push_arg(ir, value, data_view);
        ir_value_push_arg(ir, value, curve);
        ir_value_push_arg(ir, value, out_view);
      ...
      /* std.crypto.ecc.verify: (key, data, signature, curve, world) -> Bool */
      if (strcmp(callee_name, "std.crypto.ecc.verify") == 0 && expr->args.len == 5) {
        IrValue *key_view = NULL, *data_view = NULL, *sig_view = NULL, *curve = NULL;
        if (!ir_lower_byte_view(program, ir, fun, expr->args.items[0], &key_view) ||
            !ir_lower_byte_view(program, ir, fun, expr->args.items[1], &data_view) ||
            !ir_lower_byte_view(program, ir, fun, expr->args.items[2], &sig_view) ||
            !ir_lower_expr(program, ir, fun, expr->args.items[3], &curve)) {
          ir_free_value(key_view); ir_free_value(data_view); ir_free_value(sig_view); ir_free_value(curve);
          free(callee_name); return false;
        }
        IrValue *value = ir_new_value(ir, IR_VALUE_CRYPTO_HELPER, IR_TYPE_BOOL, expr->line, expr->column);
        value->error_code = CRYPTO_ECC_VERIFY;
        ir_value_push_arg(ir, value, key_view);
        ir_value_push_arg(ir, value, data_view);
        ir_value_push_arg(ir, value, sig_view);
        ir_value_push_arg(ir, value, curve);
      ...
      /* std.crypto.ecc.ecdh: (key, peerPublic, curve, out, world) -> u32 */
      if (strcmp(callee_name, "std.crypto.ecc.ecdh") == 0 && expr->args.len == 5) {
        IrValue *key_view = NULL, *peer_view = NULL, *curve = NULL, *out_view = NULL;
        if (!ir_lower_byte_view(program, ir, fun, expr->args.items[0], &key_view) ||
            !ir_lower_byte_view(program, ir, fun, expr->args.items[1], &peer_view) ||
            !ir_lower_expr(program, ir, fun, expr->args.items[2], &curve) ||
            !ir_lower_byte_view(program, ir, fun, expr->args.items[3], &out_view)) {
          ir_free_value(key_view); ir_free_value(peer_view); ir_free_value(curve); ir_free_value(out_view);
          free(callee_name); return false;
        }
        IrValue *value = ir_new_value(ir, IR_VALUE_CRYPTO_HELPER, IR_TYPE_U32, expr->line, expr->column);
        value->error_code = CRYPTO_ECC_ECDH;
        ir_value_push_arg(ir, value, key_view);
      ...
      /* std.crypto.ecc.ed25519Sign: (key, data, out, world) -> u32 */
      if (strcmp(callee_name, "std.crypto.ecc.ed25519Sign") == 0 && expr->args.len == 4) {
        IrValue *key_view = NULL, *data_view = NULL, *out_view = NULL;
        if (!ir_lower_byte_view(program, ir, fun, expr->args.items[0], &key_view) ||
            !ir_lower_byte_view(program, ir, fun, expr->args.items[1], &data_view) ||
            !ir_lower_byte_view(program, ir, fun, expr->args.items[2], &out_view)) {
        ...
        IrValue *value = ir_new_value(ir, IR_VALUE_CRYPTO_HELPER, IR_TYPE_U32, expr->line, expr->column);
        value->error_code = CRYPTO_ECC_ED25519_SIGN;
        ir_value_push_arg(ir, value, key_view);
        ir_value_push_arg(ir, value, data_view);
        ir_value_push_arg(ir, value, out_view);
        free(callee_name);
      ...
      /* std.crypto.ecc.ed25519Verify: (key, data, signature, world) -> Bool */
      if (strcmp(callee_name, "std.crypto.ecc.ed25519Verify") == 0 && expr->args.len == 4) {
        IrValue *key_view = NULL, *data_view = NULL, *sig_view = NULL;
        if (!ir_lower_byte_view(program, ir, fun, expr->args.items[0], &key_view) ||
            !ir_lower_byte_view(program, ir, fun, expr->args.items[1], &data_view) ||
            !ir_lower_byte_view(program, ir, fun, expr->args.items[2], &sig_view)) {
          ir_free_value(key_view); ir_free_value(data_view); ir_free_value(sig_view);
        ...
        IrValue *value = ir_new_value(ir, IR_VALUE_CRYPTO_HELPER, IR_TYPE_BOOL, expr->line, expr->column);
        value->error_code = CRYPTO_ECC_ED25519_VERIFY;
        ir_value_push_arg(ir, value, key_view);
        ir_value_push_arg(ir, value, data_view);
        ir_value_push_arg(ir, value, sig_view);
        free(callee_name);
