  ...
  switch (value->kind) {
    ...
    case IR_VALUE_BINARY:
      ...
      if (value->binary_op == IR_BIN_ADD) {
        ...
        z_x64_emit_shl_rax_cl(text, false);
      } else if (value->binary_op == IR_BIN_SHR) {
        z_x64_emit_shr_rax_cl(text, false);
      } else if (value->binary_op == IR_BIN_ROL) {
        z_x64_emit_rol_rax_cl(text, false);
      } else if (value->binary_op == IR_BIN_ROR) {
        z_x64_emit_ror_rax_cl(text, false);
      } else {
    ...
    case IR_VALUE_CRYPTO_HELPER: {
      CoffRuntimeHelper helper;
      switch (value->error_code) {
        case CRYPTO_SHA256: helper = COFF_RUNTIME_CRYPTO_SHA256; break;
        case CRYPTO_HMAC_SHA256: helper = COFF_RUNTIME_CRYPTO_HMAC_SHA256; break;
        case CRYPTO_AES_ENCRYPT: helper = COFF_RUNTIME_CRYPTO_AES_ENCRYPT; break;
        case CRYPTO_AES_DECRYPT: helper = COFF_RUNTIME_CRYPTO_AES_DECRYPT; break;
        case CRYPTO_CHACHA20: helper = COFF_RUNTIME_CRYPTO_CHACHA20; break;
        case CRYPTO_PBKDF2: helper = COFF_RUNTIME_CRYPTO_PBKDF2; break;
        case CRYPTO_RANDOM_BYTES: helper = COFF_RUNTIME_CRYPTO_RANDOM_BYTES; break;
        case CRYPTO_SHA512: helper = COFF_RUNTIME_CRYPTO_SHA512; break;
        case CRYPTO_SALSA20: helper = COFF_RUNTIME_CRYPTO_SALSA20; break;
        case CRYPTO_CHACHA20_POLY1305_ENCRYPT: helper = COFF_RUNTIME_CRYPTO_CHACHA20_POLY1305_ENCRYPT; break;
        case CRYPTO_CHACHA20_POLY1305_DECRYPT: helper = COFF_RUNTIME_CRYPTO_CHACHA20_POLY1305_DECRYPT; break;
        case CRYPTO_DES_ENCRYPT: helper = COFF_RUNTIME_CRYPTO_DES_ENCRYPT; break;
        case CRYPTO_DES_DECRYPT: helper = COFF_RUNTIME_CRYPTO_DES_DECRYPT; break;
        case CRYPTO_TDES_ENCRYPT: helper = COFF_RUNTIME_CRYPTO_TDES_ENCRYPT; break;
        case CRYPTO_TDES_DECRYPT: helper = COFF_RUNTIME_CRYPTO_TDES_DECRYPT; break;
        case CRYPTO_BLOWFISH_ENCRYPT: helper = COFF_RUNTIME_CRYPTO_BLOWFISH_ENCRYPT; break;
        case CRYPTO_BLOWFISH_DECRYPT: helper = COFF_RUNTIME_CRYPTO_BLOWFISH_DECRYPT; break;
        case CRYPTO_TWOFISH_ENCRYPT: helper = COFF_RUNTIME_CRYPTO_TWOFISH_ENCRYPT; break;
        case CRYPTO_TWOFISH_DECRYPT: helper = COFF_RUNTIME_CRYPTO_TWOFISH_DECRYPT; break;
        case CRYPTO_SERPENT_ENCRYPT: helper = COFF_RUNTIME_CRYPTO_SERPENT_ENCRYPT; break;
        case CRYPTO_SERPENT_DECRYPT: helper = COFF_RUNTIME_CRYPTO_SERPENT_DECRYPT; break;
        case CRYPTO_CAMELLIA_ENCRYPT: helper = COFF_RUNTIME_CRYPTO_CAMELLIA_ENCRYPT; break;
        case CRYPTO_CAMELLIA_DECRYPT: helper = COFF_RUNTIME_CRYPTO_CAMELLIA_DECRYPT; break;
        case CRYPTO_RSA_GENERATE_KEYPAIR: helper = COFF_RUNTIME_CRYPTO_RSA_GENERATE_KEYPAIR; break;
        case CRYPTO_RSA_ENCRYPT: helper = COFF_RUNTIME_CRYPTO_RSA_ENCRYPT; break;
        case CRYPTO_RSA_DECRYPT: helper = COFF_RUNTIME_CRYPTO_RSA_DECRYPT; break;
        case CRYPTO_RSA_SIGN: helper = COFF_RUNTIME_CRYPTO_RSA_SIGN; break;
        case CRYPTO_RSA_VERIFY: helper = COFF_RUNTIME_CRYPTO_RSA_VERIFY; break;
        case CRYPTO_ECC_GENERATE_KEYPAIR: helper = COFF_RUNTIME_CRYPTO_ECC_GENERATE_KEYPAIR; break;
        case CRYPTO_ECC_SIGN: helper = COFF_RUNTIME_CRYPTO_ECC_SIGN; break;
        case CRYPTO_ECC_VERIFY: helper = COFF_RUNTIME_CRYPTO_ECC_VERIFY; break;
      ...
      static const unsigned param_regs[] = {1, 2, 8, 9}; /* rcx, rdx, r8, r9 */
      for (size_t i = 0; i < reg_count; i++) {
        z_x64_emit_pop_reg64(text, param_regs[3 - i]);
      }
      if (expanded > 4) {
        z_x64_emit_sub_rsp(text, 32);
      }
      ...
      return z_coff_record_instr_runtime_patch(ctx, helper, patch, NULL, diag);
    }
    default: {
      char actual[64];
...
bool z_emit_coff_x64_object_from_ir(const IrProgram *program, ZBuf *out, ZDiag *diag) {
  ...
  for (size_t i = 0; i < program->function_len; i++) {
    symbols[i] = (ZCoffSymbol){
      .name = program->functions[i].name ? program->functions[i].name : "zerox_fn",
