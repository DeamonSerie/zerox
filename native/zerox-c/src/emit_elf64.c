...
static ElfRuntimeHelper elf_crypto_kind_to_runtime(unsigned crypto_kind) {
  switch (crypto_kind) {
    case CRYPTO_SHA256: return ELF_RUNTIME_CRYPTO_SHA256;
    case CRYPTO_HMAC_SHA256: return ELF_RUNTIME_CRYPTO_HMAC_SHA256;
    case CRYPTO_AES_ENCRYPT: return ELF_RUNTIME_CRYPTO_AES_ENCRYPT;
    case CRYPTO_AES_DECRYPT: return ELF_RUNTIME_CRYPTO_AES_DECRYPT;
    case CRYPTO_CHACHA20: return ELF_RUNTIME_CRYPTO_CHACHA20;
    case CRYPTO_PBKDF2: return ELF_RUNTIME_CRYPTO_PBKDF2;
    case CRYPTO_RANDOM_BYTES: return ELF_RUNTIME_CRYPTO_RANDOM_BYTES;
    case CRYPTO_SHA512: return ELF_RUNTIME_CRYPTO_SHA512;
    case CRYPTO_SALSA20: return ELF_RUNTIME_CRYPTO_SALSA20;
    case CRYPTO_CHACHA20_POLY1305_ENCRYPT: return ELF_RUNTIME_CRYPTO_CHACHA20_POLY1305_ENCRYPT;
    case CRYPTO_CHACHA20_POLY1305_DECRYPT: return ELF_RUNTIME_CRYPTO_CHACHA20_POLY1305_DECRYPT;
    case CRYPTO_DES_ENCRYPT: return ELF_RUNTIME_CRYPTO_DES_ENCRYPT;
    case CRYPTO_DES_DECRYPT: return ELF_RUNTIME_CRYPTO_DES_DECRYPT;
    case CRYPTO_TDES_ENCRYPT: return ELF_RUNTIME_CRYPTO_TDES_ENCRYPT;
    case CRYPTO_TDES_DECRYPT: return ELF_RUNTIME_CRYPTO_TDES_DECRYPT;
    case CRYPTO_BLOWFISH_ENCRYPT: return ELF_RUNTIME_CRYPTO_BLOWFISH_ENCRYPT;
    case CRYPTO_BLOWFISH_DECRYPT: return ELF_RUNTIME_CRYPTO_BLOWFISH_DECRYPT;
    case CRYPTO_TWOFISH_ENCRYPT: return ELF_RUNTIME_CRYPTO_TWOFISH_ENCRYPT;
    case CRYPTO_TWOFISH_DECRYPT: return ELF_RUNTIME_CRYPTO_TWOFISH_DECRYPT;
    case CRYPTO_SERPENT_ENCRYPT: return ELF_RUNTIME_CRYPTO_SERPENT_ENCRYPT;
    case CRYPTO_SERPENT_DECRYPT: return ELF_RUNTIME_CRYPTO_SERPENT_DECRYPT;
    case CRYPTO_CAMELLIA_ENCRYPT: return ELF_RUNTIME_CRYPTO_CAMELLIA_ENCRYPT;
    case CRYPTO_CAMELLIA_DECRYPT: return ELF_RUNTIME_CRYPTO_CAMELLIA_DECRYPT;
    case CRYPTO_RSA_GENERATE_KEYPAIR: return ELF_RUNTIME_CRYPTO_RSA_GENERATE_KEYPAIR;
    case CRYPTO_RSA_ENCRYPT: return ELF_RUNTIME_CRYPTO_RSA_ENCRYPT;
    case CRYPTO_RSA_DECRYPT: return ELF_RUNTIME_CRYPTO_RSA_DECRYPT;
    case CRYPTO_RSA_SIGN: return ELF_RUNTIME_CRYPTO_RSA_SIGN;
    case CRYPTO_RSA_VERIFY: return ELF_RUNTIME_CRYPTO_RSA_VERIFY;
    case CRYPTO_ECC_GENERATE_KEYPAIR: return ELF_RUNTIME_CRYPTO_ECC_GENERATE_KEYPAIR;
    case CRYPTO_ECC_SIGN: return ELF_RUNTIME_CRYPTO_ECC_SIGN;
    case CRYPTO_ECC_VERIFY: return ELF_RUNTIME_CRYPTO_ECC_VERIFY;
...
static bool elf_emit_crypto_value(ZBuf *code, const IrFunction *fun, const IrValue *value, ElfEmitContext *ctx, ZDiag *diag) {
  if (value->kind != IR_VALUE_CRYPTO_HELPER) {
    return elf_diag(diag, "direct ELF64 crypto value expected", value->line, value->column, "invalid value");
  }
  ElfRuntimeHelper helper = elf_crypto_kind_to_runtime(value->error_code);
  if (helper == ELF_RUNTIME_HELPER_COUNT) {
    return elf_diag(diag, "direct ELF64 unsupported crypto operation", value->line, value->column, "unknown crypto kind");
  }
  /* Push all arguments in reverse order, expanding byte views into ptr/len pairs */
  for (size_t i = value->arg_len; i > 0; i--) {
    IrValue *arg = value->args[i - 1];
    if (arg->type == IR_TYPE_BYTE_VIEW || arg->type == IR_TYPE_MAYBE_BYTE_VIEW) {
      /* Byte view: push ptr, then len */
      if (!elf_emit_byte_view_ptr(code, fun, arg, ctx, diag)) return false;
      elf_emit_push_rax(code);
      if (!elf_emit_byte_view_len(code, fun, arg, ctx, diag)) return false;
      elf_emit_push_rax(code);
    } else {
      /* Scalar: push value */
  ...
  /* Pop into x64 calling convention registers (rcx, rdx, r8, r9) */
  unsigned regs[4] = {1, 2, 8, 9};
  /* Count expanded args by scanning the arg list */
  size_t expanded = 0;
  ...
  /* Pop into r9, r8, rdx, rcx */
  size_t reg_count = expanded < 4 ? expanded : 4;
  for (size_t i = 0; i < reg_count; i++) {
    z_x64_emit_pop_reg64(code, regs[3 - i]);
  }
  /* Shadow space for x64 calling convention */
  if (expanded > 4) {
    z_x64_emit_sub_rsp(code, 32);
  }
  size_t patch = z_x64_emit_call32_placeholder(code);
  if (expanded > 4) {
    z_x64_emit_add_rsp(code, 32);
  }
  ...
  switch (value->kind) {
    ...
    case IR_VALUE_BINARY: {
      ...
      if (value->binary_op == IR_BIN_ADD) {
        ...
        z_x64_emit_xor_rax_rcx(code, wide);
      } else if (value->binary_op == IR_BIN_SHL) {
        z_x64_emit_shl_rax_cl(code, wide);
      } else if (value->binary_op == IR_BIN_SHR) {
        z_x64_emit_shr_rax_cl(code, wide);
      } else if (value->binary_op == IR_BIN_ROL) {
        z_x64_emit_rol_rax_cl(code, wide);
      } else if (value->binary_op == IR_BIN_ROR) {
        z_x64_emit_ror_rax_cl(code, wide);
...
static bool elf_emit_value(ZBuf *code, const IrFunction *fun, const IrValue *value, ElfEmitContext *ctx, ZDiag *diag) {
  ...
  switch (value->kind) {
    ...
    case IR_VALUE_CRYPTO_HELPER:
      return elf_emit_crypto_value(code, fun, value, ctx, diag);
    case IR_VALUE_BYTE_VIEW_INDEX_LOAD:
