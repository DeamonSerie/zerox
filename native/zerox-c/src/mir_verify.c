...
static bool mir_verify_direct_helper_value_contract(IrProgram *ir, const IrFunction *fun, const MirVerifierState *state, const IrValue *value, MirHelperRequirements *requirements) {
  ...
  switch (value->kind) {
    ...
    case IR_VALUE_CRYPTO_HELPER: {
      const char *name = "std.crypto";
      unsigned expected = 0;
      switch (value->error_code) {
        case CRYPTO_SHA256: name = "std.crypto.hash.sha256"; expected = 32; break;
        case CRYPTO_HMAC_SHA256: name = "std.crypto.hash.hmacSha256"; expected = 32; break;
        case CRYPTO_AES_ENCRYPT: name = "std.crypto.aes.encrypt"; expected = 0; break;
        case CRYPTO_AES_DECRYPT: name = "std.crypto.aes.decrypt"; expected = 0; break;
      ...
      if (!mir_verify_helper_result_type(ir, value, IR_TYPE_U32, "Crypto helper result")) return false;
      /* Verify that arg types are valid (each arg must be a byte view or integer value) */
      for (size_t i = 0; i < value->arg_len; i++) {
        if (value->args[i]->type != IR_TYPE_BYTE_VIEW &&
            value->args[i]->type != IR_TYPE_MAYBE_BYTE_VIEW &&
            !mir_type_is_value(value->args[i]->type)) {
          mir_verify_mark_unsupported(ir, "MIR verifier found invalid crypto helper arg type", value->line, value->column, "non-byte-view, non-value arg");
          return false;
        }
      }
      return true;
...
static bool mir_verify_binary_value_contract(IrProgram *ir, const IrValue *value) {
  ...
  if ((value->binary_op == IR_BIN_ADD || value->binary_op == IR_BIN_SUB ||
       ...
       value->binary_op == IR_BIN_XOR ||
       value->binary_op == IR_BIN_SHL ||
       value->binary_op == IR_BIN_SHR ||
       value->binary_op == IR_BIN_ROL ||
       value->binary_op == IR_BIN_ROR) &&
