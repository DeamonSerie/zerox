...
static bool row_three_char_symbol(const char *source, size_t offset) {
  const char *symbols[] = {"<<<", ">>>", NULL};
  for (int i = 0; symbols[i]; i++) {
    if (source[offset] == symbols[i][0] && source[offset + 1] == symbols[i][1] && source[offset + 2] == symbols[i][2]) return true;
  }
  return false;
}
...
ZRowTokenVec z_row_tokenize(const char *source, ZDiag *diag) {
  ...
  while (source && source[offset]) {
    ...
    if (row_three_char_symbol(source, offset)) {
      row_push_token(&tokens, row_token(Z_ROW_TOKEN_SYMBOL, z_strndup(source + offset, 3), line, column, offset, 3));
      offset += 3;
      column += 3;
      continue;
    }
